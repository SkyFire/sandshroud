...
public:
	...
	// push onto the Lua stack a userdata containing a  *to T object
	static int push(lua_State *L, shared_ptr<T> obj, bool gc=false) {
		...
		if (ud) {
			ud->pT = obj.get();  // store  *to object in userdata
			lua_pushvalue(L, mt);
		...
		lua_replace(L, mt);
		lua_settop(L, mt);
		return mt;  // index of userdata containing  *to T object
	}
	static int push(lua_State *L, T * obj, bool gc=false) {
		...
		if (ud) {
			ud->pT = obj;  // store  *to object in userdata
			lua_pushvalue(L, mt);
			lua_setmetatable(L, -2);
			if (gc == false) {
				lua_checkstack(L, 3);
	...
	// get userdata from Lua stack and return  *to T object
...
private:
	...
	static int new_T(lua_State *L) {
		...
		return 1;           // userdata containing  *to T object
	}
