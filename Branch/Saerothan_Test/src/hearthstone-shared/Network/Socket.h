...
/* Sending Operations */
	...
	HEARTHSTONE_INLINE void BurstBegin() { m_writeMutex.Acquire(); }
	...
	HEARTHSTONE_INLINE void BurstEnd() { m_writeMutex.Release(); }

/* Client Operations */

	// Get the client's ip in numerical form.
	string GetRemoteIP();
	HEARTHSTONE_INLINE uint32 GetRemotePort() { return ntohs(m_client.sin_port); }
	HEARTHSTONE_INLINE SOCKET GetFd() { return m_fd; }
...
/* Platform-specific methods */
	...
	HEARTHSTONE_INLINE bool IsConnected() { return m_connected; }
	HEARTHSTONE_INLINE sockaddr_in & GetRemoteStruct() { return m_client; }
	HEARTHSTONE_INLINE CircularBuffer& GetReadBuffer() { return readBuffer; }
	HEARTHSTONE_INLINE CircularBuffer& GetWriteBuffer() { return writeBuffer; }

/* Deletion */
	void Delete();

	HEARTHSTONE_INLINE in_addr GetRemoteAddress() { return m_client.sin_addr; }
...
public:
	...
	HEARTHSTONE_INLINE void SetCompletionPort(HANDLE cp) { m_completionPort = cp; }
	
	// Atomic wrapper functions for increasing read/write locks
	HEARTHSTONE_INLINE void IncSendLock() { InterlockedIncrement(&m_writeLock); }
	HEARTHSTONE_INLINE void DecSendLock() { InterlockedDecrement(&m_writeLock); }
	HEARTHSTONE_INLINE bool AcquireSendLock()
	{
		if(m_writeLock)
			return false;
		else
...
public:
	...
	// Atomic wrapper functions for increasing read/write locks
	HEARTHSTONE_INLINE void IncSendLock() { __sync_add_and_fetch(&m_writeLock, 1); }
	HEARTHSTONE_INLINE void DecSendLock() { __sync_sub_and_fetch(&m_writeLock, 1); }
	HEARTHSTONE_INLINE bool HasSendLock() { return (m_writeLock != 0); }
