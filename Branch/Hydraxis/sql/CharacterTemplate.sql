...
-- ----------------------------
-- Table structure for playercurrency
-- ----------------------------
CREATE TABLE `playercurrency` (
  `playerid` int(32) NOT NULL DEFAULT '0',
  `currencyid` int(32) NOT NULL DEFAULT '0',
  `amount` int(4) NOT NULL DEFAULT '0',
  UNIQUE KEY `Unique` (`playerid`,`currencyid`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COMMENT='Player currency system';
...
) ENGINE=MyISAM AUTO_INCREMENT=6303 DEFAULT CHARSET=latin1;
...
CREATE TABLE `playertalents` (
  ...
  UNIQUE KEY `Unique` (`guid`,`spec`,`tid`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 CHECKSUM=1 DELAY_KEY_WRITE=1 ROW_FORMAT=DYNAMIC;
