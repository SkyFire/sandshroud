...
#include "feathermoonemu_packet.h"
#include "feathermoonemu_opcodes.h"
#include "voice_channel.h"
#include "voice_channelmgr.h"

#define FEATHERMOONEMUSOCKET_RBUF_LEN 5000
...
typedef struct  
{
	...
	network_socket * sock;
	int channelcount;
} feathermoonemu_socket;

feathermoonemu_socket * feathermoonemusocket_allocate(network_socket *s)
{
	feathermoonemu_socket * ret = (feathermoonemu_socket*)vc_malloc(sizeof(feathermoonemu_socket));
	ret->read_buf = (char*)vc_malloc(FEATHERMOONEMUSOCKET_RBUF_LEN);
	ret->read_buf_len = 0;
	ret->read_buf_sz = FEATHERMOONEMUSOCKET_RBUF_LEN;
	ret->sock = s;
	ret->channelcount = 0;
	++g_serverCount;
	return ret;
...
void feathermoonemusocket_free(feathermoonemu_socket* s)
{
	if( s->channelcount > 0 )
...
void feathermoonemusocket_send(feathermoonemu_socket *s, feathermoonemu_packet *p)
{
...
int voicechat_feathermoonemu_listen_socket_read_handler(network_socket *s, int act)
{
	...
	ns->event_handler = voicechat_feathermoonemu_socket_read_handler;
	ns->miscdata = (void*)feathermoonemusocket_allocate(ns);		// used for voicechat struct later
	...
	if( network_add_socket(ns) < 0 )
	{
		...
		feathermoonemusocket_free((feathermoonemu_socket*)ns->miscdata);
		network_close(ns);
	}
...
// client handlers
void vc_handler_createchannel(feathermoonemu_socket *s, feathermoonemu_packet *p)
{
	uint8 type;
	uint32 request_id;
	feathermoonemu_packet reply;
	voice_channel * chn;
	uint8 error;
	uint16 reply_id;

	type = feathermoonemupacket_readu8(p);
	request_id = feathermoonemupacket_readu32(p);
	...
	if( chn == NULL )
	{
		...
		feathermoonemupacket_init(VOICECHAT_SMSG_CHANNEL_CREATED, 5, &reply);
		feathermoonemupacket_writeu32(&reply, request_id);
		feathermoonemupacket_writeu8(&reply, error);

		// send reply
		feathermoonemusocket_send(s, &reply);

		// free buffer
		feathermoonemupacket_free(&reply);
	}
	else
	{
		...
		feathermoonemupacket_init(VOICECHAT_SMSG_CHANNEL_CREATED, 7, &reply);
		feathermoonemupacket_writeu32(&reply, request_id);
		feathermoonemupacket_writeu8(&reply, error);
		feathermoonemupacket_writeu16(&reply, reply_id);
...
void vc_handler_deletechannel(feathermoonemu_socket *s, feathermoonemu_packet *p)
{
	uint16 channel_id;
	voice_channel *ch;

	channel_id = feathermoonemupacket_readu16(p);
...
void vc_handler_addmember(feathermoonemu_socket *s, feathermoonemu_packet *p)
{
	uint16 channel_id;
	uint8 member_id;
	voice_channel *ch;
	...
	channel_id = feathermoonemupacket_readu16(p);
	member_id = feathermoonemupacket_readu8(p);
...
void vc_handler_deletemember(feathermoonemu_socket *s, feathermoonemu_packet *p)
{
	uint16 channel_id;
	uint8 member_id;
	voice_channel *ch;
...
void vc_handler_ping(feathermoonemu_socket *s, feathermoonemu_packet *p)
{
	feathermoonemu_packet bp;
	uint32 pi;

	printf("ping!\n");
	pi = feathermoonemupacket_readu32(p);

	feathermoonemupacket_init(VOICECHAT_SMSG_PONG, 4, &bp);
	feathermoonemupacket_writeu32(&bp, pi);
	feathermoonemusocket_send(s, &bp);
	feathermoonemupacket_free(&bp);
}

// handler table
typedef void(*vc_handler)(feathermoonemu_socket*,feathermoonemu_packet*);
...
{
	feathermoonemu_socket * mysock = (feathermoonemu_socket*)s->miscdata;
	uint16 opcode;
	uint16 len;
	int readcount;
	feathermoonemu_packet * pck;
	...
	{
		feathermoonemusocket_free(mysock);
	...
	if( readcount <= 0 )
	{
		log_write(DEBUG, "feathermoonemu socket %u failed reading. probably dead.", s->fd);
	...
	for(;;)
	{
		...
		// create feathermoonemupacket structure
		pck = feathermoonemupacket_create(opcode, len);
