...
public:
	...
	FEATHERMOONEMU_INLINE void SetLength(const unsigned int sz)
	{
		i_nodes.resize( sz );
	}

	FEATHERMOONEMU_INLINE size_t Size(void) const { return i_nodes.size(); }
	FEATHERMOONEMU_INLINE void Resize(unsigned int sz) { i_nodes.resize(sz); }
	FEATHERMOONEMU_INLINE void Clear(void) { i_nodes.clear(); }
	FEATHERMOONEMU_INLINE PathNode* GetNodes(void) { return static_cast<PathNode *>(&i_nodes[0]); }
...
public:
	...
	FEATHERMOONEMU_INLINE void AddPlayer(Player* pPlayer) { mPassengers[pPlayer->GetLowGUID()] = pPlayer; }
	FEATHERMOONEMU_INLINE void RemovePlayer(Player* pPlayer) {mPassengers.erase(pPlayer->GetLowGUID()); }
	FEATHERMOONEMU_INLINE bool HasPlayer(Player* pPlayer) { return mPassengers.find(pPlayer->GetLowGUID()) != mPassengers.end(); }
	FEATHERMOONEMU_INLINE void SetPeriod(uint32 val) { m_period = val; }
