...
public:
	...
	FEATHERMOONEMU_INLINE Unit* getUnitToFollow() { return UnitToFollow; }
	void setCreatureState(CreatureState state){ m_creatureState = state; }
	FEATHERMOONEMU_INLINE uint8 getAIState() { return m_AIState; }
	FEATHERMOONEMU_INLINE uint8 getAIType() { return m_AIType; }
	FEATHERMOONEMU_INLINE uint8 getCurrentAgent() { return m_aiCurrentAgent; }
	...
	FEATHERMOONEMU_INLINE AssistTargetSet GetAssistTargets() { return m_assistTargets; }
	FEATHERMOONEMU_INLINE TargetMap *GetAITargets() { return &m_aiTargets; }
	...
	Unit* getTauntedBy();
	bool GetIsTaunted();
	FEATHERMOONEMU_INLINE size_t getAITargetsCount() { return m_aiTargets.size(); }
	FEATHERMOONEMU_INLINE uint32 getOutOfCombatRange() { return m_outOfCombatRange; }
	...
	FEATHERMOONEMU_INLINE bool hasWaypoints() { return m_waypoints!=NULL; }
	FEATHERMOONEMU_INLINE void setMoveType(uint32 movetype) { m_moveType = movetype; }
	FEATHERMOONEMU_INLINE uint32 getMoveType() { return m_moveType; }
	FEATHERMOONEMU_INLINE void setMoveRunFlag(bool f) { m_moveRun = f; }
	FEATHERMOONEMU_INLINE bool getMoveRunFlag() { return m_moveRun; }
	FEATHERMOONEMU_INLINE void setCanMove(bool canmove) { m_canMove = canmove; }
	...
	FEATHERMOONEMU_INLINE bool GetAllowedToEnterCombat(void) { return m_AllowedToEnterCombat; }

	void CheckTarget(Unit* target);
	FEATHERMOONEMU_INLINE void SetAIState(AI_State newstate) { m_AIState = newstate; }
	...
	FEATHERMOONEMU_INLINE void AddStopTime(uint32 Time) { m_moveTimer += Time; }
	FEATHERMOONEMU_INLINE void SetNextSpell(AI_Spell*sp) { m_nextSpell = sp; }
	FEATHERMOONEMU_INLINE Unit* GetNextTarget() { return m_nextTarget; }
	FEATHERMOONEMU_INLINE void SetNextTarget (Unit* nextTarget) 
	{ 
	...
	/*FEATHERMOONEMU_INLINE void ResetProcCounts()
	{
		AI_Spell*sp;
	...
	FEATHERMOONEMU_INLINE void SetPetOwner(Unit* owner) { m_PetOwner = owner; }
	...
	FEATHERMOONEMU_INLINE void SetWaypointMap(WayPointMap * m) { m_waypoints = m; }
	bool m_isGuard;
	void CallGuards();
