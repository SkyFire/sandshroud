				...
				if( ip->ItemLimitCategory > 0 )
				{
					...
					if( ile )
					{
						...
						if( ( gCount + gemCount ) > ile->maxAmount )
							return INV_ERR_ITEM_MAX_COUNT_EQUIPPED_SOCKETED;
					}
				}
			}
		}
...
bool ItemInterface::HasItem(uint32 itemid)
{
	for( uint32 x = EQUIPMENT_SLOT_START; x < INVENTORY_KEYRING_END; ++x )
	{
		if( m_pItems[x] != NULL )
			if( m_pItems[x]->GetProto()->ItemId == itemid )
				return true;
	...
	if(it)
	{
		...
		while( (count > maxStack) && freeslots )
		{
			...
			if(it->Bonding==ITEM_BIND_ON_PICKUP)
				if(it->Flags & ITEM_FLAG_BINDONACC) // don't "Soulbind" account-bound items
					item->AccountBind();
				else
					item->SoulBind();
			if(randomprop!=0)
...
void ItemInterface::ItemSets(ItemSetEntry * itemset)
{
	ItemSetEntry *is;
	if(itemset != NULL)
	{
		...
		case 878:
			{
				itemset->itemid[1] = 48627;
				itemset->itemid[2] = 48628;
				itemset->itemid[3] = 48629;
				itemset->itemid[4] = 48630;
				itemset->itemid[5] = 48631;
			}break;

		case 766:
			{
				itemset->itemid[1] = 40780;
				itemset->itemid[2] = 40798;
				itemset->itemid[3] = 40818;
				itemset->itemid[4] = 40838;
				itemset->itemid[5] = 40858;
			}
		}
	}
	uint32 cnt = (uint32)dbcItemSet.GetNumRows();
	for(uint32 x = 0; x < cnt; x++)
	{
		is = dbcItemSet.LookupRow(x);
		ItemSets(is);
	}
	for(list<ItemSetEntry*>::iterator itr = sWorld.dummysets.begin(); itr != sWorld.dummysets.end(); ++itr)
		ItemSets(*itr);
}
