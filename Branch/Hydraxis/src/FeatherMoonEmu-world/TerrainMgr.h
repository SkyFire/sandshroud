...
protected:
	...
	/* Gets the offset for the specified cell from the cached offset index.
	  ...
	  */
	FEATHERMOONEMU_INLINE uint32 GetCellInformationOffset(uint32 x, uint32 y)
	{
	...
	/* Gets a cell information pointer so that another function can access its data.
	  ...
	  */
	FEATHERMOONEMU_INLINE CellTerrainInformation* GetCellInformation(uint32 x, uint32 y)
	{
	...
	/* Converts a global x co-ordinate into a cell x co-ordinate.
	  ...
	  */
	FEATHERMOONEMU_INLINE uint32 ConvertGlobalXCoordinate(float x)
	{
	...
	*/
	FEATHERMOONEMU_INLINE uint32 ConvertGlobalYCoordinate(float y)
	{
	...
	*/
	FEATHERMOONEMU_INLINE float ConvertInternalXCoordinate(float x, uint32 cellx)
	{
		float X = (_maxX - x);
		X -= (cellx * _cellSize);
		return X;
	...
	*/
	FEATHERMOONEMU_INLINE float ConvertInternalYCoordinate(float y, uint32 celly)
	{
		float Y = (_maxY - y);
		Y -= (celly * _cellSize);
		return Y;
	...
	/* Checks whether a cell information is loaded or not.
	  */
	FEATHERMOONEMU_INLINE bool CellInformationLoaded(uint32 x, uint32 y)
	{
	...
	/* Converts the internal co-ordinate to an index in the 
	  ...
	  */
	FEATHERMOONEMU_INLINE uint32 ConvertTo2dArray(float c)
	{
	...
	/* Checks that the co-ordinates are within range.
	  */
	FEATHERMOONEMU_INLINE bool AreCoordinatesValid(float x, float y)
	{
		if(x > _maxX || x < _minX)
			return false;
		if(y > _maxY || y < _minY)
