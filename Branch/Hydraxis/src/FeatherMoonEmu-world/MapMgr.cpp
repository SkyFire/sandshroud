		...
		cx >= _sizeX || cy >= _sizeY)
	{
		if( plObj != NULL )
		{
			if(plObj->GetBindMapId() != GetMapId())
			{
				...
				plObj->GetSession()->SystemMessage("Teleported you to your FeathermoonEmu location as you ended up on the wrong map.");
				return;
			}
			else
			{
				obj->GetPositionV()->ChangeCoords(plObj->GetBindPositionX(),plObj->GetBindPositionY(),plObj->GetBindPositionZ(),0);
				plObj->GetSession()->SystemMessage("Teleported you to your FeathermoonEmu location as you were out of the map boundaries.");
		...
		obj->GetPositionY() >= _maxY || obj->GetPositionY() <= _minY)
	{
		if(obj->IsPlayer())
		{
			...
			if(plr->GetBindMapId() != GetMapId())
			{
				...
				plr->GetSession()->SystemMessage("Teleported you to your FeathermoonEmu location as you were out of the map boundaries.");
				return;
			}
			else
			{
...
{
    VehicleEntry const *vehInfo = dbcVehicle.LookupEntry(entry);
    if(!vehInfo)
        return false;
...
void MapMgr::AreaIDScripts(Player* plr, uint16 GetAreaID, uint32 spellId, uint32)
{
	{
		if(plr->GetAreaID() == 4676) //Sunreaver(Anti Alliance)
		{
			{
				if(plr->GetTeam() == ALLIANCE) // Alliance!!!
				{
					CastSpellOnPlayers( 1, 63991 );
				}
			}
		}
	}
	{
		if(plr->GetAreaID() == 4677) //Silver Covenant(Anti Horde)
		{
			{
				if(plr->GetTeam() == HORDE) // Horde
				{
					CastSpellOnPlayers( 0, 63990 );
				}
			}
		}
	}
	if(plr->GetAreaID() == 4658) // The Argent Tournament
	{
		{
			CastSpellOnPlayers( -1, 64373 );
		}
	}
};
