...
bool ChatHandler::HandleReviveCommand(const char* args, WorldSession *m_session)
{
	...
	{
		SelectedPlayer->ResurrectPlayer(SelectedPlayer);
	}
...
bool ChatHandler::HandleRestartCommand(const char* args, WorldSession* m_session)
{
	...
	sGMLog.writefromsession(m_session, "Initiated server restart timer %u sec", restarttime);
...
void SendItemLinkToPlayer(ItemPrototype * iProto, WorldSession * pSession, bool ItemCount, Player * owner = NULL)
{
	...
	if(ItemCount)
		sChatHandler.SystemMessage(pSession,"Item %u |%s|Hitem:%u:0:0:0:0:0:0:0|h[%s]|h|r  count %u", iProto->ItemId, q.c_str(), iProto->ItemId, iProto->Name, owner->GetItemInterface()->GetItemCount(iProto->ItemId, true));
	else
		sChatHandler.SystemMessage(pSession,"Item %u |%s|Hitem:%u:0:0:0:0:0:0:0|h[%s]|h|r", iProto->ItemId, q.c_str(), iProto->ItemId, iProto->Name);
}
		...
		if(FindXinYString(x, y))
 		{
			...
			if(count == 40)
			{
				RedSystemMessage(m_session, "More than 40 results returned. aborting.");
				break;
			}
		}
	}
...
bool ChatHandler::HandleShowItems(const char * args, WorldSession * m_session)
{
	...
	{
		SystemMessage(m_session, "Item %s count %u", (*itr)->GetProto()->Name, (*itr)->GetUInt32Value(ITEM_FIELD_STACK_COUNT));
	}
	itr.EndSearch();
...
bool ChatHandler::HandleCollisionTestIndoor(const char * args, WorldSession * m_session)
{
	...
	{
		SystemMessage(m_session, "FeathermoonEmu was not compiled with collision support.");
		return true;
	}
}
...
bool ChatHandler::HandleWhoCommand(const char *args, WorldSession *m_session)
{
	return false;
}