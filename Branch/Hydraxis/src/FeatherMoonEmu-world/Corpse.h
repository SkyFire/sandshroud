...
public:
   ...
   // void Create();
	...
	void SaveToDB();
	void DeleteFromDB();
	FEATHERMOONEMU_INLINE void SetCorpseState(uint32 state) { m_state = state; }
	FEATHERMOONEMU_INLINE uint32 GetCorpseState() { return m_state; }
	void Despawn();
	
	FEATHERMOONEMU_INLINE void SetLoadedFromDB(bool value) { _loadedfromdb = value; }
	FEATHERMOONEMU_INLINE bool GetLoadedFromDB(void) { return _loadedfromdb; }
