...
void WorldSession::HandleRepopRequestOpcode( WorldPacket & recv_data )
{
	...
	{
		printf("Recieved unhandled repop value %u.", repop);
	}
}
...
void WorldSession::HandleWhoOpcode( WorldPacket & recv_data )
{
	...
	recv_data >> chatname >> guild_name >> race_mask >> class_mask;
...
void WorldSession::HandleLogoutCancelOpcode( WorldPacket & recv_data )
{
	...
	OUT_DEBUG( "WORLD: Recvd CMSG_LOGOUT_CANCEL Message" );
...
void WorldSession::HandleUpdateAccountData(WorldPacket &recv_data)
{
	...
	{
		SKIP_READ_PACKET(recv_data); // Reduce some error overload :D
		return;
	}
...
void WorldSession::HandleSetActionButtonOpcode(WorldPacket& recv_data)
{
	...
	// Re-enable for new patch opcode change testing
	//printf( "BUTTON: %u ACTION: %u TYPE: %u MISC: %u\n", button, action, type, misc );
...
void WorldSession::HandleAmmoSetOpcode(WorldPacket & recv_data)
{
	...
	if(xproto->Class != ITEM_CLASS_PROJECTILE || GetPlayer()->GetItemInterface()->GetItemCount(ammoId) == 0)
	{
		...
		printf("Disconnecting account %u, tried to use incorrect ammo.\n", _socket->GetSession()->GetAccountId());
...
void WorldSession::HandleTutorialFlag( WorldPacket & recv_data )
{
	...
	if(wInt >= 7)
	{
		printf("Account %u, recieved an Oversized tutorial, size %u.\n", _socket->GetSession()->GetAccountId(), wInt);
		return;
	}
	...
	if(wInt >= 100)
	{
		printf("Disconnecting account %u, Oversized tutorial.\n", _socket->GetSession()->GetAccountId());
		Disconnect();
		return;
	}
...
void WorldSession::HandlePlayedTimeOpcode( WorldPacket & recv_data )
{
	...
	uint8 sendtime; // 1 if player asked for time, 0 if he didn't.
	recv_data >> sendtime; // See if its a 1 or a 0.
...
void WorldSession::HandleSetActionBarTogglesOpcode(WorldPacket &recv_data)
{
	...
	GetPlayer()->SetByte(PLAYER_FIELD_BYTES, 2, ActionBar);
}
...
void WorldSession::HandleAcknowledgementOpcodes( WorldPacket & recv_data )
{
	...
	// Individual opcodes :|
	if(opcode == CMSG_FORCE_RUN_SPEED_CHANGE_ACK || CMSG_FORCE_RUN_BACK_SPEED_CHANGE_ACK || CMSG_FORCE_SWIM_SPEED_CHANGE_ACK || CMSG_FORCE_FLIGHT_SPEED_CHANGE_ACK)
	{
		recv_data >> GUID;
		...
		{
			recv_data >> d_time;
			recv_data >> X >> Y >> Z >> O; // LocVec
			recv_data >> unk1;
		}
		recv_data >> speed; // Current Speed
	}
	...
	case CMSG_FORCE_PITCH_RATE_CHANGE_ACK:
		{
			...
			{
				if(opcode == CMSG_FORCE_WALK_SPEED_CHANGE_ACK)
					_player->SetPlayerSpeed( WALK, speed);
				if(opcode == CMSG_FORCE_RUN_SPEED_CHANGE_ACK)
					_player->SetPlayerSpeed( RUN, speed);
				if(opcode == CMSG_FORCE_RUN_BACK_SPEED_CHANGE_ACK)
					_player->SetPlayerSpeed( RUNBACK, speed);
				if(opcode == CMSG_FORCE_SWIM_SPEED_CHANGE_ACK)
					_player->SetPlayerSpeed( SWIM, speed);
				if(opcode == CMSG_FORCE_SWIM_BACK_SPEED_CHANGE_ACK)
					_player->SetPlayerSpeed( SWIMBACK, speed);
				if(opcode == CMSG_FORCE_TURN_RATE_CHANGE_ACK)
					_player->SetPlayerSpeed( TURN, speed);
				if(opcode == CMSG_FORCE_FLIGHT_SPEED_CHANGE_ACK)
					_player->SetPlayerSpeed( FLY, speed);
				if(opcode == CMSG_FORCE_FLIGHT_BACK_SPEED_CHANGE_ACK)
					_player->SetPlayerSpeed( FLYBACK, speed);
				if(opcode == CMSG_FORCE_PITCH_RATE_CHANGE_ACK)
					_player->SetPlayerSpeed( PITCH_RATE, speed);
			}
		}break;
	}
}
...
void WorldSession::HandleCalendarGetCalendar(WorldPacket &recv_data)
{
    ...
    data << (uint32) 1135753200;                            //wtf?? (28.12.2005 8:00)
    data << (uint32) 0;                                     //  unk counter 4
    data << (uint32) 0;                                     // unk counter 5
    //OUT_DEBUG("Sending calendar");
    //data.hexlike();
    SendPacket(&data);
}
...
{
	printf("Calling HandleCalendarGetEvent");
	recv_data.hexlike();
...
{
	printf("Calling HandleCalendarGuildFilter");
	recv_data.hexlike();
...
{
	printf("Calling HandleCalendarArenaTeam");
	recv_data.hexlike();
