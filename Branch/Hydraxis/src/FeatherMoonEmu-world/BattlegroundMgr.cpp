...
static uint32 ChooseRandom5v5()
{
	uint32 mapid = (rand()%4 + 1);
	switch(mapid)
	{
	case 1:
		return 559;
...
void CBattlegroundManager::HandleBattlegroundListPacket(WorldSession * m_session, uint32 BattlegroundType, bool battlemaster)
{
	...
	data << uint32(0);		// Number of battlegrounds
	...
	if(!IS_ARENA(BattlegroundType))
	{
		...
		{
			if( itr->second->HasStarted() && itr->second->GetLevelGroup() == LevelGroup  && !itr->second->HasEnded() )
			{
				data << itr->first;
				++Count;
			}
...
void CBattlegroundManager::HandleBattlegroundJoin(WorldSession * m_session, WorldPacket & pck)
{
	...
	{
		printf("Disconnecting account %u, unknown Battleground type.\n", m_session->GetAccountId());
...
void CBattlegroundManager::DeleteBattleground(CBattleground* bg)
{
	...
	bg = NULLBATTLEGROUND; //close mem leak
	delete bg;
}
...
void CBattlegroundManager::SendBattlegroundQueueStatus(Player* plr, uint32 queueSlot)
{
	...
	if( !plr->m_bgIsQueued[queueSlot] && !plr->m_pendingBattleground[queueSlot])
	{
		...
		data << uint16(0) << uint32(queueSlot) << uint64(0);
		plr->GetSession()->SendPacket(&data);
		return;
	}
	...
	if( plr->m_pendingBattleground[queueSlot] )
	{
		data << uint32(2); // 0 null, 1 Pending, 2 available.
	...
	data << uint32(1); // 0 null, 1 Pending, 2 available.
...
Creature* CBattleground::SpawnSpiritGuide(float x, float y, float z, float o, uint32 horde)
{
	...
	pCreature->Create(pInfo->Name, m_mapMgr->GetMapId(), -1, x, y, z, o);
	...
	pCreature->SetUInt32Value(UNIT_FIELD_FLAGS, UNIT_FLAG_PLUS_MOB | UNIT_FLAG_OOC_NOT_ATTACKABLE | UNIT_FLAG_UNKNOWN_10); // 4928
