...
public:
	...
	/// Creation
	void Create ( const char* creature_name, uint32 vehId, uint32 mapid, float x, float y, float z, float ang);	
	...
	FEATHERMOONEMU_INLINE uint32 GetSQL_id() { return spawnid; };

	/// Creature inventory
	FEATHERMOONEMU_INLINE uint32 GetItemIdBySlot(uint32 slot) { return m_SellItems->at(slot).itemid; }
	FEATHERMOONEMU_INLINE uint32 GetItemAmountBySlot(uint32 slot) { return m_SellItems->at(slot).amount; }

	FEATHERMOONEMU_INLINE bool HasItems() { return ((m_SellItems != NULL) ? true : false); }
	...
	FEATHERMOONEMU_INLINE void GetSellItemBySlot(uint32 slot, CreatureItem &ci)
	{
	...
	FEATHERMOONEMU_INLINE std::vector<CreatureItem>::iterator GetSellItemBegin() { return m_SellItems->begin(); }
	FEATHERMOONEMU_INLINE std::vector<CreatureItem>::iterator GetSellItemEnd()   { return m_SellItems->end(); }
	FEATHERMOONEMU_INLINE size_t GetSellItemCount() { return m_SellItems->size(); }
	...
	AIInterface * AI() const { return (AIInterface*)i_AI; }
	...
	FEATHERMOONEMU_INLINE void SetSheatheForAttackType(uint8 type) { SetByte(UNIT_FIELD_BYTES_2, 0, type); }
	FEATHERMOONEMU_INLINE bool isQuestGiver() { return HasFlag( UNIT_NPC_FLAGS, UNIT_NPC_FLAG_QUESTGIVER ); };
	...
	bool RemoveEnslave();
	void HandleMirrorImageCreature(Player* player);

	//General vars for summoned creatures
	Unit* GetSummonOwner();
	FEATHERMOONEMU_INLINE int32 GetSummonSlot() { return SummonSlot; }
	FEATHERMOONEMU_INLINE bool IsSummon() { return SummonOwner && SummonSlot > -1; }
	FEATHERMOONEMU_INLINE void SetSummonOwnerSlot(uint64 ownerguid, int8 slot) { SummonOwner = ownerguid; SummonSlot = slot;}

	//Special summons, Totems
	FEATHERMOONEMU_INLINE bool IsTotem() { return Totem;}
	FEATHERMOONEMU_INLINE void SetTotem(bool totem) { Totem = totem;}

	FEATHERMOONEMU_INLINE bool IsPickPocketed() { return m_PickPocketed; }
	FEATHERMOONEMU_INLINE void SetPickPocketed(bool val = true) { m_PickPocketed = val; }

	FEATHERMOONEMU_INLINE CreatureAIScript * GetScript() { return _myScriptClass; }
	void LoadScript();
	...
	FEATHERMOONEMU_INLINE Trainer* GetTrainer() { return mTrainer; }
	...
	FEATHERMOONEMU_INLINE CreatureProto *GetProto() { return proto; }
	FEATHERMOONEMU_INLINE CreatureProtoHeroic * GetProtoHeroic() { return proto_heroic; }
	FEATHERMOONEMU_INLINE CreatureInfo *GetCreatureInfo() {	return creature_info; }
	FEATHERMOONEMU_INLINE CreatureInfo *GetCreatureName() {	return creature_info; }
	FEATHERMOONEMU_INLINE void SetCreatureName(CreatureInfo *ci) { creature_info = ci; }
	...
	{
	//	return GetCreatureProto()-> = CREATURE_FLAG_EXTRA_TRIGGER;
		return false;
	}
protected:
...
public:
	...
	// Crow: End Vehicle Crap

	//	custom functions for scripting
	FEATHERMOONEMU_INLINE uint32 GetProtoItemDisplayId(uint8 i) { return GetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID + i); }
...
class SERVER_DECL TempSummon : public Creature
{
public:
...
//	void UpdateDamagePhysical(uint32 attType);
