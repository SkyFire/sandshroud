...
void World::Rehash(bool load)
{
	...
	{
		#ifdef WIN32
		Config.MainConfig.SetSource("FeathermoonEmu-world.conf", true);
		#else
		Config.MainConfig.SetSource((char*)CONFDIR "/FeathermoonEmu-world.conf", true);
		#endif
	}
	...
	GoldStartAmount = Config.MainConfig.GetIntDefault("Server", "StartingGold", 0);
	...
	SetRequiredBuild(Config.MainConfig.GetIntDefault("Server", "RequiredBuild", 10958));

	SetMotd(Config.MainConfig.GetStringDefault("Server", "Motd", "FeathermoonEmu Default MOTD").c_str());
	SetMotd2(Config.MainConfig.GetStringDefault("Server", "Motd2", "FeathermoonEmu Default MOTD").c_str());
...
void World::QueueShutdown(uint32 delay, uint32 type)
{
	...
	// send message
	char buf[1000];
	snprintf(buf, 1000, "Server %s initiated. Server will save and %s in approx. %u seconds.", type == SERVER_SHUTDOWN_TYPE_RESTART ? "restart" : "shutdown", type == SERVER_SHUTDOWN_TYPE_RESTART ? "restart" : "shut down", delay);
