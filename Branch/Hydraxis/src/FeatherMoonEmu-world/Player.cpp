...
void Player::OnLogin(Player* plr)
{
...
FEATHERMOONEMU_INLINE uint32 GetSpellForLanguage(uint32 SkillID)
{
	switch(SkillID)
	{
...
//	SetUInt32Value(PLAYER_BYTES_2, (facialHair | (0xEE << 8) | (0x01 << 16) | (0x02 << 24)));
	...
	SetUInt32Value(PLAYER_FIELD_COINAGE, sWorld.GoldStartAmount *10000);
...
void Player::InitVisibleUpdateBits()
{
	...
	for(uint16 i = PLAYER_QUEST_LOG_1_1; i < PLAYER_QUEST_LOG_25_5; i += 5)
...
void Player::RemovePendingPlayer()
{
	if(m_session)
	{
		uint8 respons = 0x4F;		// CHAR_LOGIN_NO_CHARACTER
...
void Player::OnPushToWorld()
{
	...
	*data << uint32(0);
	...
	if(flying_aura && m_mapId != 530 && (m_mapId != 571 || !HasSpellwithNameHash(SPELL_HASH_COLD_WEATHER_FLYING)) && !GetSession()->CanUseCommand('a') &&
		flying_aura != 55164 && flying_aura != 55173 )
	{
...
void Player::_ApplyItemMods(Item* item, int8 slot, bool apply, bool justdrokedown /* = false */, bool skip_stat_apply /* = false  */)
{
	...
	for( int i = 0; i < 10; i++ )
	{
		...
		if( val == 0 )
			continue;
		ModifyBonuses( proto->Stats[i].Type, apply ? val : -val, apply );
	}
...
void Player::AddCalculatedRestXP(uint32 seconds)
{
	...
	// if multiplier RestXP (from FeathermoonEmu.config) is f.e 2, you only need 4hrs/bubble.
...
void Player::SendDualTalentConfirm()
{
	...
	if( GetUInt32Value(PLAYER_FIELD_COINAGE) >= talentCost )
	{
		...
		SendChatMessage(CHAT_MSG_ADDON, LANG_ADDON, "You now have Dual Talent Specialization.");
	}
	else
	{
		// Not enough money!
...
void Player::LearnTalent(uint32 talent_id, uint32 requested_rank)
{
	...
	{
		// cheater!
		printf("Player has more than 3 talent trees! Disconnecting.\n");
...
void Player::RemoveItemsFromWorld()
{
	...
	for(uint32 i = 0; i < CURRENCYTOKEN_SLOT_END; i++)
	{
		...
		if(pItem)
		{
			if(pItem->IsInWorld())
			{
				if(i < INVENTORY_SLOT_BAG_END)	  // only equipment+bag slots get mods.
				{
...
void Player::EndDuel(uint8 WinCondition)
{
	...
	if(TO_PLAYER(this)->GetHealth() < 1)
	{
		TO_PLAYER(this)->setDeathState(ALIVE);
		TO_PLAYER(this)->UnRoot();
		TO_PLAYER(this)->SetUInt32Value( UNIT_FIELD_HEALTH, 1 );
	}
}
...
bool Player::SafeTeleport(uint32 MapID, uint32 InstanceID, LocationVector vec)
{
	...
	// Update our visibility and area.
	UpdateVisibility();
	ForceAreaUpdate();
...
void Player::PvPToggle()
{
	...
	{
		char* message = ("[Hydraxis]: Hello! Certain areas have PvP disabled so that players are able to fulfill certain tasks or certain quests. These areas also exist to help uphold the RP variety that Hydraxis has. If you wish to PvP, please move to a different area and fulfill your needs. Might I suggest, Gurubashi? Have fun. ~ Crow");
...
{
	_AdvanceAllSkills(count, false);
}

void Player::_AdvanceAllSkills(uint32 count, bool skipprof)
{
	bool dirty = false;
	for(SkillMap::iterator itr = m_skills.begin(); itr != m_skills.end(); ++itr)
	{
...
{
	CharacterDatabase.Execute("UPDATE characters SET killsToday = %u, killsYesterday = %u, killsLifeTime = %u, honorToday = %u, honorYesterday = %u, honorPoints = %u, arenaPoints = %u WHERE guid = %u",
		m_killsToday, m_killsYesterday, m_killsLifetime, m_honorToday, m_honorYesterday, m_honorPoints, m_arenaPoints, m_uint32Values[OBJECT_FIELD_GUID]);
}
...
Pet* Player::GetPet() const
{
	if(uint64 pet_guid = GetPetGUID())
	{
		...
		if(Pet* pet = GetMapMgr()->GetPet(pet_guid))
			return pet;
...
void Player::SendEquipmentSetList()
{
	...
	for(EquipmentSets::iterator itr = m_EquipmentSets.begin(); itr != m_EquipmentSets.end(); ++itr)
	{
		...
		for(uint32 i = 0; i < EQUIPMENT_SLOT_END; ++i)
			data.appendPackGUID(MAKE_NEW_GUID(itr->second.Items[i], 0, HIGHGUID_TYPE_ITEM));
...
{ // Custom functions and area checks without outside scripts.
  // Got to love Hydraxis :D
	if(plr)
	{
		if(plr->GetAreaID() == 4275)
		{
			if(plr->IsPvPFlagged())
				plr->RemovePvPFlag();
		}
...
uint8 Player::AreaPvPAllow(Player* plr)
{
	/* How to use: Below is a guide on how to use this function.
	Simply create a new line, like the one below, and add a return 0 to make it so that the player cannot PvP in this area.
	Here is an example.
	if(plr->GetAreaID() == 0)
		return 0;
	*/
