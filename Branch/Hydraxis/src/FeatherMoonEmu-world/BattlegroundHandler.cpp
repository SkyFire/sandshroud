...
int32 GetBattlegroundTypeFromCreature(Creature* pCreature)
{
	...
	//Isle of Conquest
	if( string(pCreature->GetCreatureInfo()->SubName).find("Isle of Conquest") != string::npos )
		return BATTLEGROUND_ISLE_OF_CONQUEST;

	else if( string(pCreature->GetCreatureInfo()->Name).find("Isle of Conquest") != string::npos )
		return BATTLEGROUND_ISLE_OF_CONQUEST;
...
void WorldSession::HandleLeaveBattlefieldOpcode(WorldPacket &recv_data)
{
	...
	if(_player->m_bg && _player->IsInWorld() && !_player->CombatStatus.IsInCombat())
		if(!_player->m_bg->HasEnded() && !_player->m_bg->IsArena() && !_player->GetSession()->HasGMPermissions()) // Check for arena, just in case.
		{
			_player->CastSpell(_player, 26013, true); // Casting twice, in case he cheats.... Deserter!!!
