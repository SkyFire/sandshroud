...
public:
	...
	FEATHERMOONEMU_INLINE void SendPacket(WorldPacket* packet) { if(!packet) return; OutPacket(packet->GetOpcode(), packet->size(), (packet->size() ? (const void*)packet->contents() : NULL)); }
	FEATHERMOONEMU_INLINE void SendPacket(StackPacket * packet) { if(!packet) return; OutPacket(packet->GetOpcode(), packet->GetSize(), (packet->GetSize() ? (const void*)packet->GetBufferPointer() : NULL)); }
	...
	FEATHERMOONEMU_INLINE void SetSession(WorldSession * session) { mSession = session; }
	FEATHERMOONEMU_INLINE WorldSession * GetSession() { return mSession; }
	bool Authed;
...
public:
	...
	void Disconnect();
	bool IsConnected();
	FEATHERMOONEMU_INLINE string GetRemoteIP() { return string(inet_ntoa(m_address.sin_addr)); }
	FEATHERMOONEMU_INLINE uint32 GetRemotePort() { return ntohs(m_address.sin_port); }

	FEATHERMOONEMU_INLINE void SendPacket(WorldPacket* packet) { if(!packet) return; OutPacket(packet->GetOpcode(), (uint16)packet->size(), (packet->size() ? (const void*)packet->contents() : NULL)); }
	FEATHERMOONEMU_INLINE void SendPacket(StackPacket * packet) { if(!packet) return; OutPacket(packet->GetOpcode(), packet->GetSize(), (packet->GetSize() ? (const void*)packet->GetBufferPointer() : NULL)); }
