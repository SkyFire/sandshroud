...
void WorldSession::HandleCharCreateOpcode( WorldPacket & recv_data )
{
	...
	{
		SKIP_READ_PACKET(recv_data); // PACKET!!
		OutPacket(SMSG_CHAR_CREATE, 1, CHAR_CREATE_NAME_IN_USE);
		return;
	}
	...
	if(result)
	{
		if(result->Fetch()[0].GetUInt32() > 0)
		{
			// That name is banned!
			SKIP_READ_PACKET(recv_data); // PACKET!!
			OutPacket(SMSG_CHAR_CREATE, 1, CHAR_NAME_PROFANE); // You cannot use that name
			delete result;
			return;
		}
		delete result;
...
uint8 WorldSession::DeleteCharacter(uint32 guid)
{
	...
	if( inf != NULL && inf->m_loggedInPlayer == NULL )
	{
		...
		CharacterDatabase.Execute("DELETE FROM arenateams WHERE leader = %u", (uint32)guid);
...
void WorldSession::FullLogin(Player* plr)
{
	...
	WorldPacket datamotd2(SMSG_MOTD, 100);
	datamotd2 << uint32(0x04);
	datamotd2 << sWorld.GetMotd2();
	SendPacket(&datamotd2);

	/* world preload */
	packetSMSG_LOGIN_VERIFY_WORLD vwpck;
	if(HasGMPermissions() && plr->m_FirstLogin)
	{
		vwpck.MapId = 1;
		vwpck.O = 0;
		vwpck.X = 16222.6f;
		...
		plr->m_position.z = 16.2085f;
		plr->m_position.o = 0;
		plr->m_mapId = 1;
		plr->SetBindPoint(16222.6f, 16265.9f, 14.2085f, 1, 0);
	}
	else
	{
		...
		vwpck.Z = (plr->GetPositionZ() + 4); // Increase by 4, safety reasons.
	}
...
#ifdef WIN32
	_player->BroadcastMessage("Server: %sHydraxis r%u-BRANCH/%s-Win-%s", MSG_COLOR_RED, BUILD_REVISION, CONFIG, ARCH, MSG_COLOR_LIGHTBLUE);
#else
	_player->BroadcastMessage("Server: %sHydraxis r%u-BRANCH/%s-%s", MSG_COLOR_RED, BUILD_REVISION, PLATFORM_TEXT, ARCH, MSG_COLOR_LIGHTBLUE);
#endif
...
void WorldSession::HandleCharCustomizeOpcode(WorldPacket & recv_data)
{
	...
	CharacterDatabase.Execute("UPDATE characters SET gender = '%u', bytes = '%u', bytes2 = '%u' WHERE guid = '%u'", gender, skin | (face << 8) | (hairStyle << 16) | (hairColor << 24), player_bytes2, playerGuid);
