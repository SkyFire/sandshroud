...
bool ChatHandler::CreateGuildCommand(const char* args, WorldSession *m_session)
{
	...
	Guild * plrGuild = NULL;
	plrGuild = objmgr.GetGuildByGuildName(string(args));
...
bool ChatHandler::HandleDeMorphCommand(const char* args, WorldSession *m_session)
{
	...
	if(target == 0)
	{
		SystemMessage(m_session, "No selection. Auto-Targeting self.");
...
bool ChatHandler::HandleKillCommand(const char *args, WorldSession *m_session)
{
	...
	if(target->IsPlayer())
	{
		...
		m_session->GetPlayer()->Kill(plr, false);
	...
	else
	{
		...
		TO_CREATURE( target )->SendChatMessage(CHAT_MSG_MONSTER_SAY, LANG_UNIVERSAL, "Master! Forgive me!");
	}
...
bool ChatHandler::HandleGOInfo(const char *args, WorldSession *m_session)
{
	...
	SystemMessage(m_session, "%s phase:%s%u",	MSG_COLOR_GREEN,MSG_COLOR_LIGHTBLUE, GObj->GetPhaseMask() );
...
bool ChatHandler::HandleVehicleDeleteCommand(const char* args, WorldSession *m_session)
{
	uint64 guid = m_session->GetPlayer()->GetSelection();
	...
	Creature* unit = m_session->GetPlayer()->GetMapMgr()->GetVehicle(GET_LOWGUID_PART(guid));
	if( !unit )
	{
		SystemMessage(m_session, "You should select a vehicle.");
		return true;
	}
