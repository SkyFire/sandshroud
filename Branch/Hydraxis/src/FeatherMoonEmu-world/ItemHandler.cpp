...
void WorldSession::HandleSwapItemOpcode(WorldPacket& recv_data)
{
	...
	{   //check if it will go to equipment slot
		if(DstInvSlot==INVENTORY_SLOT_NOT_SET)//not bag
		{
			...
			{
				if(SrcItem->GetOwner() && SrcItem->GetOwner() != _player)
				{
					printf("Disconnecting account %u, player placed item in container that they don't own.\n", _socket->GetSession()->GetAccountId());
...
void WorldSession::HandleDestroyItemOpcode( WorldPacket & recv_data )
{
	...
	int8 SrcInvSlot, SrcSlot, count, data1, data2, data3;

	recv_data >> SrcInvSlot >> SrcSlot >> count >> data1 >> data2 >> data3;
...
void WorldSession::HandleAutoEquipItemSlotOpcode( WorldPacket& recvData )
{
	...
	if( _player->titanGrip && ( slotType == EQUIPMENT_SLOT_OFFHAND || slotType == EQUIPMENT_SLOT_MAINHAND ) )
		hastitanGrip = true;

	// Handle destination slot checking.
	if( destSlot == slotType || hastitanGrip )
	{
			...
			invType == INVTYPE_WEAPONOFFHAND || invType == INVTYPE_2HWEAPON )
		{
			...
			if( mainHand != NULL && offHand != NULL && !_player->titanGrip )
...
{ 
	...
	if(li)
		...
		namelens = strlen(itemProto->Name) + strlen(itemProto->Description) + 602;
	...
	data << uint8(0); // Set To: Item Set Name; Possibly: ?Localization?
	data << uint8(0); // Set To: Event Name; Possibly: ?Localization?
	data << uint8(0); // Set To: Comments; Possibly: ?Localization?
	...
	data << uint32(abs(0));							// 2.4.2 Item duration in seconds
...
void WorldSession::HandleBuyBackOpcode( WorldPacket & recv_data )
{
	...
	if (it)
	{
		...
		if ((error = _player->GetItemInterface()->CanReceiveItem(it->GetProto(), amount, NULL)) != 0)
		{
...
void WorldSession::HandleSellItemOpcode( WorldPacket & recv_data )
{
	...
	if((_player->GetUInt32Value(PLAYER_FIELD_COINAGE) + price) > 2147480000)
	{
...
FEATHERMOONEMU_INLINE uint32 RepairItemCost(Player* pPlayer, Item* pItem)
{
...
FEATHERMOONEMU_INLINE void RepairItem(Player* pPlayer, Item* pItem)
{
...
void WorldSession::HandleInsertGemOpcode(WorldPacket &recvPacket)
{
	...
	bool sockenchgloves = (TargetItem->HasEnchantmentOnSlot(TargetItem->HasEnchantment(3723)) && TargetItem->GetProto()->InventoryType == 10);
	bool sockenchbracer = (TargetItem->HasEnchantmentOnSlot(TargetItem->HasEnchantment(3717)) && TargetItem->GetProto()->InventoryType == 9);
	bool sockenchbelt = (TargetItem->HasEnchantmentOnSlot(TargetItem->HasEnchantment(3729)) && TargetItem->GetProto()->InventoryType == 6);
