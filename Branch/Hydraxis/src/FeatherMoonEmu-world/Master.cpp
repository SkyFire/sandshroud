...
#define BANNER "FeathermoonEmu r%u/%s-%s-%s :: World Server\n"
...
SERVER_DECL Database* Rip_Database;
...
#ifdef WIN32
        static const char* default_config_file = "FeathermoonEmu-world.conf";
        static const char* default_realm_config_file = "FeathermoonEmu-realms.conf";
#else
        static const char* default_config_file = CONFDIR "/FeathermoonEmu-world.conf";
        static const char* default_realm_config_file = CONFDIR "/FeathermoonEmu-realms.conf";
#endif
...
bool Master::Run(int argc, char ** argv)
{
	...
	struct feathermoonemu_option longopts[] =
	{
		{ "checkconf",			feathermoonemu_no_argument,			&do_check_conf,			1		},
		{ "screenloglevel",		feathermoonemu_required_argument,		&screen_log_level,		1		},
		{ "fileloglevel",		feathermoonemu_required_argument,		&file_log_level,		-1		},
		{ "version",			feathermoonemu_no_argument,			&do_version,			1		},
		{ "conf",				feathermoonemu_required_argument,		NULL,					'c'		},
		{ "realmconf",			feathermoonemu_required_argument,		NULL,					'r'		},
		{ 0, 0, 0, 0 }
	};

	char c;
	while ((c = feathermoonemu_getopt_long_only(argc, argv, ":f:", longopts, NULL)) != -1)
	{
		switch (c)
		{
		case 'c':
			config_file = new char[strlen(feathermoonemu_optarg)];
			strcpy(config_file, feathermoonemu_optarg);
			break;

		case 'r':
			realm_config_file = new char[strlen(feathermoonemu_optarg)];
			strcpy(realm_config_file, feathermoonemu_optarg);
			break;
	...
	if(geteuid() == 0 || getegid() == 0)
		Log.LargeErrorMessage( LARGERRORMESSAGE_WARNING, "You are running FeathermoonEmu as root.", "This is not needed, and may be a possible security risk.", "It is advised to hit CTRL+C now and", "start as a non-privileged user.", NULL);
#endif
	...
	if( Config.MainConfig.SetSource( config_file ) )
		Log.Success( "Config", ">> FeathermoonEmu-world.conf" );
	else
	{
		Log.Error( "Config", ">> FeathermoonEmu-world.conf" );
		return false;
	}
	...
	if(Config.RealmConfig.SetSource(realm_config_file))
		Log.Success( "Config", ">> FeathermoonEmu-realms.conf" );
	else
	{
		Log.Error( "Config", ">> FeathermoonEmu-realms.conf" );
		return false;
	}
	...
	/* write pid file */
	FILE * fPid = fopen( "FeathermoonEmu-world.pid", "w" );
	if( fPid )
	{
		uint32 pid;
#ifdef WIN32
	...
	// remove pid
	remove( "FeathermoonEmu-world.pid" );
