...
public:
	...
	FEATHERMOONEMU_INLINE ItemPrototype* GetProto() const { return m_itemProto; }
	FEATHERMOONEMU_INLINE void SetProto( ItemPrototype* pr ) { m_itemProto = pr; }

	FEATHERMOONEMU_INLINE Player* GetOwner() const { return m_owner; }
	void SetOwner( Player* owner );

	FEATHERMOONEMU_INLINE bool IsContainer(){ return ( m_objectTypeId == TYPEID_CONTAINER ) ? true : false; }
	...
	FEATHERMOONEMU_INLINE void SoulBind()
	{
		this->SetFlag( ITEM_FIELD_FLAGS, ITEM_FLAG_SOULBOUND );
	}

	FEATHERMOONEMU_INLINE void AccountBind()
	{
		this->SetFlag( ITEM_FIELD_FLAGS, ITEM_FLAG_BINDONACC );
	}

	FEATHERMOONEMU_INLINE bool IsSoulbound()
	{
		return this->HasFlag( ITEM_FIELD_FLAGS, ITEM_FLAG_QUEST | ITEM_FLAG_SOULBOUND );
	}

	FEATHERMOONEMU_INLINE uint32 GetChargesLeft()
	{
	...
	FEATHERMOONEMU_INLINE time_t GetEnchantmentApplytime( uint32 slot )
	{
	...
	FEATHERMOONEMU_INLINE void SetCount( uint32 amt ) { SetUInt32Value( ITEM_FIELD_STACK_COUNT, amt ); }
	FEATHERMOONEMU_INLINE void SetDurability( uint32 Value ) { SetUInt32Value(ITEM_FIELD_DURABILITY, Value ); };
	FEATHERMOONEMU_INLINE void SetDurabilityToMax() { SetUInt32Value( ITEM_FIELD_DURABILITY, GetUInt32Value( ITEM_FIELD_MAXDURABILITY ) ); }
	FEATHERMOONEMU_INLINE uint32 GetDurability() { return GetUInt32Value( ITEM_FIELD_DURABILITY ); }
	FEATHERMOONEMU_INLINE uint32 GetDurabilityMax() { return GetUInt32Value( ITEM_FIELD_MAXDURABILITY ); }
	FEATHERMOONEMU_INLINE bool IsAmmoBag() { return (m_itemProto->Class == ITEM_CLASS_QUIVER); }
	...
	FEATHERMOONEMU_INLINE uint32 GetItemRandomSuffixFactor() { return m_uint32Values[ITEM_FIELD_PROPERTY_SEED]; }
	static uint32 GenerateRandomSuffixFactor( ItemPrototype* m_itemProto );

	FEATHERMOONEMU_INLINE void SetRandomProperty( uint32 id )
	{
		SetUInt32Value( ITEM_FIELD_RANDOM_PROPERTIES_ID, id );
		random_prop = id;
	}

	FEATHERMOONEMU_INLINE void SetRandomSuffix( uint32 id )
	{
		int32 r_id = -(int32(id));
