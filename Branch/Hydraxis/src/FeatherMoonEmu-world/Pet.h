...
public:
	...
	FEATHERMOONEMU_INLINE uint32 GetXP(void) { return m_PetXP; }
	...
	FEATHERMOONEMU_INLINE void SetPetAction(uint32 act) { m_Action = act; }
	FEATHERMOONEMU_INLINE uint32 GetPetAction(void) { return m_Action; }

	FEATHERMOONEMU_INLINE void SetPetState(uint32 state) { m_State = state; }
	FEATHERMOONEMU_INLINE uint32 GetPetState(void) { return m_State; }

	FEATHERMOONEMU_INLINE void SetPetDiet(uint32 diet) { m_Diet = diet; }
	FEATHERMOONEMU_INLINE void SetPetDiet() { m_Diet = myFamily->petdietflags; }

	FEATHERMOONEMU_INLINE uint32 GetPetDiet(void) { return m_Diet; }
	
	FEATHERMOONEMU_INLINE AI_Spell* GetAISpellForSpellId(uint32 spellid)
	{
	...
	FEATHERMOONEMU_INLINE Player* GetPetOwner() { return m_Owner; }
	FEATHERMOONEMU_INLINE void ClearPetOwner() { m_Owner = NULLPLR; }
	...
	FEATHERMOONEMU_INLINE void RemoveSpell(uint32 SpellID)
	{
		SpellEntry * sp = dbcSpell.LookupEntry(SpellID);
		if(sp)
			RemoveSpell(sp);
	}
	FEATHERMOONEMU_INLINE void SetSpellState(uint32 SpellID, uint16 State)
	{
		SpellEntry * sp = dbcSpell.LookupEntry(SpellID);
		if(sp) SetSpellState(sp, State);
	}
	FEATHERMOONEMU_INLINE uint16 GetSpellState(uint32 SpellID)
	{
		if(SpellID == 0)
	...
	FEATHERMOONEMU_INLINE PetSpellMap* GetSpells() { return &mSpells; }
	FEATHERMOONEMU_INLINE bool IsSummonedPet() { return Summon; }

	void __fastcall SetAutoCastSpell(AI_Spell*sp);
	void Rename(string NewName);
	FEATHERMOONEMU_INLINE string& GetName() { return m_name; }
	...
	/* Pet Talents! */
	FEATHERMOONEMU_INLINE uint8 GetUnspentPetTalentPoints() { return GetByte( UNIT_FIELD_BYTES_1, 1); }
	FEATHERMOONEMU_INLINE void SetUnspentPetTalentPoints(uint8 points) { SetByte(UNIT_FIELD_BYTES_1, 1, points);}
	FEATHERMOONEMU_INLINE uint8 GetSpentPetTalentPoints() { return GetPetTalentPointsAtLevel() - GetUnspentPetTalentPoints(); }
