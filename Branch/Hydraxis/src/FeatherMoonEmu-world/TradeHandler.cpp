...
{
	// Packet is sent when player logs out as well, so we need to check if there is a player.
	if(_player)
	{
...
void WorldSession::HandleSetTradeItem(WorldPacket & recv_data)
{
	...
	recv_data >> TradeSlot2 >> SourceBag2 >> SourceSlot2;

	uint8 TradeSlot = TradeSlot2;
	int8 SourceBag = SourceBag2;
	uint8 SourceSlot = SourceSlot2;
	...
	for(uint32 i = 0; i < 7; ++i)
	{
		...
		if(_player->mTradeItems[i] == pItem || pTarget->mTradeItems[i] == pItem)
		{
			...
			printf("Tried to dupe an item through trade, disconnected.\n");
			Disconnect();
			...
			for(uint32 Index = 0; Index < 6; ++Index)
			{
				...
				if(Guid != 0)
				{
					...
					else
					{
						...
						if(GetPermissionCount()>0 || pTarget->GetSession()->GetPermissionCount()>0)
							sGMLog.writefromsession(this, "trade item %s with %s (soulbound = %d)", _player->mTradeItems[Index]->GetProto()->Name, pTarget->GetName());
					}
				}
				...
				if(Guid != 0)
				{
					...
					else
					{
						...
						if(GetPermissionCount()>0 || pTarget->GetSession()->GetPermissionCount()>0)
							sGMLog.writefromsession(this, "trade item %s with %s", pTarget->mTradeItems[Index]->GetProto()->Name, _player->GetName());
					}
				}
			}
