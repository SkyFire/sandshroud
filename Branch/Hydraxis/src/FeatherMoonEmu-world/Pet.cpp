...
void Pet::CreateAsSummon(uint32 entry, CreatureInfo *ci, Creature* created_from_creature, Unit* owner, SpellEntry* created_by_spell, uint32 type, uint32 expiretime)
{
	...
	// Create ourself	
	Create(m_name.c_str(), owner->GetMapId(), -1, owner->GetPositionX(), owner->GetPositionY(), owner->GetPositionZ(), owner->GetOrientation());
...
void Pet::LoadFromDB(Player* owner, PlayerPet * playerPetInfo)
{
	...
	Create(playerPetInfo->name.c_str(), owner->GetMapId(), -1, owner->GetPositionX() + 2 , owner->GetPositionY() +2, owner->GetPositionZ(), owner->GetOrientation());
