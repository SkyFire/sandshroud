...
void InstanceMgr::Load(TaskList * l)
{
	...
	while(!itr->AtEnd())
	{
		...
		{
			Log.Notice("InstanceMgr", "Skipped test map: %u (FeathermoonEmuConfig.h)", itr->Get()->mapid );
			itr->Inc();
			continue;
		}
#endif
...
bool InstanceMgr::_DeleteInstance(Instance * in, bool ForcePlayersOut)
{
	...
	if(in->m_mapMgr)
	{
		...
		// "ForcePlayersOut" will teleport the players in this instance to their entry point/FeathermoonEmu.
