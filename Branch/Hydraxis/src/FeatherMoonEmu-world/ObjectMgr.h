...
FEATHERMOONEMU_INLINE bool FindXinYString(std::string& x, std::string& y)
{
...
typedef std::tr1::unordered_map<uint32, PointOfInterest> PointOfInterestMap;
...
public:
	...
	FEATHERMOONEMU_INLINE void SetTextID(uint32 TID) { TextId = TID; }
...
public:
	...
	FEATHERMOONEMU_INLINE uint32 GetLeader() { return LeaderGuid; }
	FEATHERMOONEMU_INLINE uint32 GetID() { return CharterId; }

	FEATHERMOONEMU_INLINE bool IsFull() { return (SignatureCount == Slots); }
};
...
public:
	...
	FEATHERMOONEMU_INLINE bool IsSpellDisabled(uint32 spellid)
	{
	...
	FEATHERMOONEMU_INLINE GuildMap::iterator GetGuildsBegin() { return mGuild.begin(); }
	FEATHERMOONEMU_INLINE GuildMap::iterator GetGuildsEnd() { return mGuild.end(); }

	std::set<ProfessionDiscovery*> ProfessionDiscoveryTable;

	static CreatureProto const *GetCreatureTemplate( uint32 id );
