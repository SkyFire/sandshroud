...
struct FactionReputation
{
	...
	uint8 flag;
	int32 baseStanding;
	FEATHERMOONEMU_INLINE int32 CalcStanding() { return standing - baseStanding; }
	FEATHERMOONEMU_INLINE bool Positive() { return standing >= 0; }
};
...
public:
	...
	FEATHERMOONEMU_INLINE Guild * GetGuild() { return m_playerInfo->guild; }
	FEATHERMOONEMU_INLINE GuildMember * GetGuildMember() { return m_playerInfo->guildMember; }
	FEATHERMOONEMU_INLINE GuildRank * GetGuildRankS() { return m_playerInfo->guildRank; }
	FEATHERMOONEMU_INLINE int8 GetActiveSpec() { return m_talentActiveSpec; }
	...
	void _AdvanceAllSkills(uint32 count, bool skipprof);
	void _ModifySkillMaximum(uint32 SkillLine, uint32 NewMax);

	// Some WorldState crap
	void SendUpdateWorldState(uint32 Field, uint32 Value);
	// End of WorldState Crap
	// Pet crap
	Pet* GetPet() const;
	// End pet crap
...
public:
	...
	uint32 flying_aura;
	void OnLogin(Player* plr);//custom stuff on player login.
	...
	FEATHERMOONEMU_INLINE const char* GetName() { return m_name.c_str(); }
	FEATHERMOONEMU_INLINE std::string* GetNameString() { return &m_name; }
    ...
    /* Taxi                                                                 */
    /************************************************************************/
    FEATHERMOONEMU_INLINE TaxiPath*    GetTaxiPath() { return m_CurrentTaxiPath; }
    FEATHERMOONEMU_INLINE bool         GetTaxiState() { return m_onTaxi; }
    ...
    FEATHERMOONEMU_INLINE void         SetTaxiState    (bool state) { m_onTaxi = state; }
    FEATHERMOONEMU_INLINE void         SetTaximask     (uint8 index, uint32 value ) { m_taximask[index] = value; }
    FEATHERMOONEMU_INLINE void         SetTaxiPath     (TaxiPath *path) { m_CurrentTaxiPath = path; }
    FEATHERMOONEMU_INLINE void         SetTaxiPos()	{m_taxi_pos_x = m_position.x; m_taxi_pos_y = m_position.y; m_taxi_pos_z = m_position.z;}
    FEATHERMOONEMU_INLINE void         UnSetTaxiPos()	{m_taxi_pos_x = 0; m_taxi_pos_y = 0; m_taxi_pos_z = 0; }
	...
	FEATHERMOONEMU_INLINE QuestLogEntry*GetQuestLogInSlot(uint32 slot){ return m_questlog[slot]; }
	FEATHERMOONEMU_INLINE uint32 GetQuestSharer(){ return m_questSharer; }

	FEATHERMOONEMU_INLINE void	SetQuestSharer(uint32 guid){ m_questSharer = guid; }
	void				SetQuestLogSlot(QuestLogEntry *entry, uint32 slot);

	FEATHERMOONEMU_INLINE void		PushToRemovedQuests(uint32 questid)	{ m_removequests.insert(questid);}
	FEATHERMOONEMU_INLINE uint32		GetFinishedDailiesCount() { return (uint32)m_finishedDailyQuests.size(); }
	...
	FEATHERMOONEMU_INLINE uint32 GetTeam() { return m_team; }
	FEATHERMOONEMU_INLINE void SetTeam(uint32 t) { m_team = t; m_bgTeam=t; }
	FEATHERMOONEMU_INLINE void ResetTeam() { m_team = myRace->team_id==7 ? 0 : 1; m_bgTeam=m_team; }

	FEATHERMOONEMU_INLINE bool IsInFeralForm()
	{
		int s = GetShapeShift();
		if( s <= 0 )
			return false;
    ...
    /* PVP                                                                  */
    /************************************************************************/
	FEATHERMOONEMU_INLINE uint8 GetPVPRank()
	{
		return (uint8)((GetUInt32Value(PLAYER_BYTES_3) >> 24) & 0xFF);
	}
	FEATHERMOONEMU_INLINE void SetPVPRank(int newrank)
	{
	...
	FEATHERMOONEMU_INLINE uint32       GetInviter() { return m_GroupInviter; }
	FEATHERMOONEMU_INLINE bool         InGroup() { return (m_playerInfo->m_Group != NULL && !m_GroupInviter); }
	bool                IsGroupLeader()
	{
	...
	FEATHERMOONEMU_INLINE int		HasBeenInvited() { return m_GroupInviter != 0; }
	FEATHERMOONEMU_INLINE Group*	GetGroup() { return m_playerInfo != NULL ? m_playerInfo->m_Group : NULL; }
	FEATHERMOONEMU_INLINE uint32	GetGroupID() { return m_playerInfo != NULL ? m_playerInfo->m_Group != NULL ? m_playerInfo->m_Group->GetID(): NULL: NULL; }
	FEATHERMOONEMU_INLINE int8		GetSubGroup() { return m_playerInfo->subGroup; }
	FEATHERMOONEMU_INLINE bool		IsBanned()
	{
		if(m_banned)
		{
	...
	FEATHERMOONEMU_INLINE void		SetBanned() { m_banned = 4;}
	FEATHERMOONEMU_INLINE void		SetBanned(string Reason) { m_banned = 4; m_banreason = Reason;}
	FEATHERMOONEMU_INLINE void		SetBanned(uint32 timestamp, string& Reason) { m_banned = timestamp; m_banreason = Reason; }
	FEATHERMOONEMU_INLINE void		UnSetBanned() { m_banned = 0; }
	FEATHERMOONEMU_INLINE string	GetBanReason() {return m_banreason;}
	...
	/* Guilds                                                               */
	/************************************************************************/
	FEATHERMOONEMU_INLINE  bool        IsInGuild() {return (m_uint32Values[PLAYER_GUILDID] != 0) ? true : false;}
	FEATHERMOONEMU_INLINE uint32       GetGuildId() { return m_uint32Values[PLAYER_GUILDID]; }
	void                SetGuildId(uint32 guildId);
	FEATHERMOONEMU_INLINE uint32       GetGuildRank() { return m_uint32Values[PLAYER_GUILDRANK]; }
	...
	FEATHERMOONEMU_INLINE uint8        GetDuelStatus() { return m_duelStatus; }
	void                SetDuelState(uint8 state) { m_duelState = state; }
	FEATHERMOONEMU_INLINE uint8        GetDuelState() { return m_duelState; }
	// duel variables
	Player*             DuelingWith;
	...
	/* Pets                                                                 */
	/************************************************************************/
	FEATHERMOONEMU_INLINE void SetSummon(Pet* pet) { m_Summon = pet; }
	FEATHERMOONEMU_INLINE Pet* GetSummon(void) { return m_Summon; }
	uint32 GeneratePetNumber(void);
	void RemovePlayerPet(uint32 pet_number);
	FEATHERMOONEMU_INLINE void AddPlayerPet(PlayerPet* pet, uint32 index) { m_Pets[index] = pet; }
	FEATHERMOONEMU_INLINE PlayerPet* GetPlayerPet(uint8 idx)
	{
	...
	FEATHERMOONEMU_INLINE PlayerPet* GetFirstPet(void) { return GetPlayerPet(GetFirstPetNumber()); }
	FEATHERMOONEMU_INLINE void SetStableSlotCount(uint8 count) { m_StableSlotCount = count; }
	FEATHERMOONEMU_INLINE uint8 GetStableSlotCount(void) { return m_StableSlotCount; }
	...
	/* Item Interface                                                       */
	/************************************************************************/
	FEATHERMOONEMU_INLINE ItemInterface* GetItemInterface() { return m_ItemInterface; } // Player Inventory Item storage
	FEATHERMOONEMU_INLINE void         ApplyItemMods(Item* item, int8 slot, bool apply,bool justdrokedown=false) {  _ApplyItemMods(item, slot, apply,justdrokedown); }
	// item interface variables
	ItemInterface *     m_ItemInterface;

	FEATHERMOONEMU_INLINE AchievementInterface* GetAchievementInterface() { return m_achievementInterface; }
	AchievementInterface * m_achievementInterface;
	
	/************************************************************************/
	/* Loot                                                                 */
	/************************************************************************/
	FEATHERMOONEMU_INLINE const uint64& GetLootGUID() const { return m_lootGuid; }
	FEATHERMOONEMU_INLINE void         SetLootGUID(const uint64 &guid) { m_lootGuid = guid; }
	...
	/* World Session                                                        */
	/************************************************************************/
	FEATHERMOONEMU_INLINE WorldSession* GetSession() const { return m_session; }
	...
	//FEATHERMOONEMU_INLINE std::list<struct skilllines>getSkillLines() { return m_skilllines; }
	...
	uint32 GetPartyUpdateFlag() const { return m_partyUpdateMask; }
	void SetPartyUpdateFlag(uint32 flag) { m_partyUpdateMask |= flag; }

	bool canCast(SpellEntry *m_spellInfo);
	FEATHERMOONEMU_INLINE float GetBlockFromSpell() { return m_blockfromspell; }
	FEATHERMOONEMU_INLINE float GetSpellCritFromSpell() { return m_spellcritfromspell; }
	FEATHERMOONEMU_INLINE float GetHitFromMeleeSpell() { return m_hitfrommeleespell; }
	FEATHERMOONEMU_INLINE float GetHitFromSpell() { return m_hitfromspell; }
	FEATHERMOONEMU_INLINE float GetParryFromSpell() { return m_parryfromspell; }
	FEATHERMOONEMU_INLINE float GetDodgeFromSpell() { return m_dodgefromspell; }
	...
	FEATHERMOONEMU_INLINE int32 GetHealthFromSpell() { return m_healthfromspell; }
	FEATHERMOONEMU_INLINE uint32 GetManaFromSpell() { return m_manafromspell; }
	...
	FEATHERMOONEMU_INLINE void SetTalentResetTimes(uint32 value) { m_talentresettimes = value; }
	void SetPlayerStatus(uint8 pStatus) { m_status = pStatus; }
	FEATHERMOONEMU_INLINE uint8 GetPlayerStatus() { return m_status; }
	...
	FEATHERMOONEMU_INLINE uint8 GetShapeShift()
	{
	...
	FEATHERMOONEMU_INLINE bool IsVisible(Object* pObj) { return !(m_visibleObjects.find(pObj) == m_visibleObjects.end()); }
	void AddInRangeObject(Object* pObj);
	void OnRemoveInRangeObject(Object* pObj);
	void ClearInRangeSet();
	FEATHERMOONEMU_INLINE void AddVisibleObject(Object* pObj) { m_visibleObjects.insert(pObj); }
	FEATHERMOONEMU_INLINE void RemoveVisibleObject(Object* pObj) { m_visibleObjects.erase(pObj); }
	FEATHERMOONEMU_INLINE void RemoveVisibleObject(InRangeSet::iterator itr) { m_visibleObjects.erase(itr); }
	FEATHERMOONEMU_INLINE InRangeSet::iterator FindVisible(Object* obj) { return m_visibleObjects.find(obj); }
	FEATHERMOONEMU_INLINE void RemoveIfVisible(Object* obj)
	{
	...
	FEATHERMOONEMU_INLINE bool GetVisibility(Object* obj, InRangeSet::iterator *itr)
	{
		*itr = m_visibleObjects.find(obj);
		return ((*itr) != m_visibleObjects.end());
	}

	FEATHERMOONEMU_INLINE InRangeSet::iterator GetVisibleSetBegin() { return m_visibleObjects.begin(); }
	FEATHERMOONEMU_INLINE InRangeSet::iterator GetVisibleSetEnd() { return m_visibleObjects.end(); }
	...
	FEATHERMOONEMU_INLINE float res_M_crit_get(){return m_resist_critical[0];}
	FEATHERMOONEMU_INLINE void res_M_crit_set(float newvalue){m_resist_critical[0]=newvalue;}
	FEATHERMOONEMU_INLINE float res_R_crit_get(){return m_resist_critical[1];}
	FEATHERMOONEMU_INLINE void res_R_crit_set(float newvalue){m_resist_critical[1]=newvalue;}
	...
	int32 IncreaseDamageByType[14]; //mod dmg by creature type
	float IncreaseDamageByTypePCT[14];
	float IncreaseCricticalByTypePCT[14];
	int32 DetectedRange;
...
*/
	FEATHERMOONEMU_INLINE uint32* GetPlayedtime() { return m_playedtime; };
	void CalcStat(uint32 t);
	FEATHERMOONEMU_INLINE float CalcRating(uint32 index) { return CalcPercentForRating(index, m_uint32Values[index]); };
    ...
    FEATHERMOONEMU_INLINE uint32 GetSoulStoneReceiver(){return SoulStoneReceiver;}
    FEATHERMOONEMU_INLINE void SetSoulStoneReceiver(uint32 StoneGUID){SoulStoneReceiver = StoneGUID;}
    FEATHERMOONEMU_INLINE uint32 GetSoulStone(){return SoulStone;}
    FEATHERMOONEMU_INLINE void SetSoulStone(uint32 StoneID){SoulStone = StoneID;}
	...
	FEATHERMOONEMU_INLINE bool IsMounted() {return (m_MountSpellId!=0 ? true : false); }
	...
	FEATHERMOONEMU_INLINE uint32 GetAreaID() { return m_AreaID; }
	void SetAreaID(uint32 area)
	{
		...
		if(GetMapId() == 571)
		{
			...
			if( GetPositionZ() > 500.0f && (area == 4551 || area == 4553 || area == 4556 || area == 2817))
				m_AreaID = 4395;
		}
		m_areaDBC = dbcArea.LookupEntryForced(m_AreaID);
	}
	FEATHERMOONEMU_INLINE AreaTable *GetAreaDBC() { return m_areaDBC; }
	...
	FEATHERMOONEMU_INLINE void setMyCorpse(Corpse* corpse) { myCorpse = corpse; }
	FEATHERMOONEMU_INLINE Corpse* getMyCorpse() { return myCorpse; }
	...
	******************/
    ...
    FEATHERMOONEMU_INLINE void AddCoins( int32 coins ){ 
        ModUnsigned32Value( PLAYER_FIELD_COINAGE , coins );
    }
    FEATHERMOONEMU_INLINE void TakeCoins( int32 coins ){ 
        ModUnsigned32Value(PLAYER_FIELD_COINAGE, -coins);
    }

	//! Do this on /pvp off
	FEATHERMOONEMU_INLINE void ResetPvPTimer();
	//! Stop the timer for pvp off
	FEATHERMOONEMU_INLINE void StopPvPTimer() { RemoveFlag(PLAYER_FLAGS, PLAYER_FLAG_PVP_TIMER); m_pvpTimer = 0; }
	...
	FEATHERMOONEMU_INLINE uint32 LastHonorResetTime() const { return m_lastHonorResetTime; }
	FEATHERMOONEMU_INLINE void LastHonorResetTime(uint32 val) { m_lastHonorResetTime = val; }
	uint32 OnlineTime;
	bool tutorialsDirty;
	LevelInfo * lvlinfo;
	...
	FEATHERMOONEMU_INLINE void SetName(string& name) { m_name = name; }
	...
	FEATHERMOONEMU_INLINE void AddComboPoints(uint64 target, uint8 count)
	{
	...
	FEATHERMOONEMU_INLINE void NullComboPoints() { if(!m_retainComboPoints) { m_comboTarget = 0; m_comboPoints = 0; m_spellcomboPoints=0; } UpdateComboPoints(); }
	...
	FEATHERMOONEMU_INLINE Player* GetTradeTarget()
	{
	...
	/* Last Speeds */
	FEATHERMOONEMU_INLINE void UpdateLastSpeeds()
	{
...
//	void save_GuildData();
	...
	FEATHERMOONEMU_INLINE bool IsAttacking() {return m_attacking; }
...
public:
	...
	FEATHERMOONEMU_INLINE bool HasKnownTitle( int32 title )
	{
...
private:
	...
	void SendAuraListFor(uint64 guid);
public:
	...
	FEATHERMOONEMU_INLINE const uint8 *GetTalentInspectBuffer() { return m_talentInspectBuffer; }
...
public:
	...
	FEATHERMOONEMU_INLINE PlayerSkill* Grab() { return &m_itr->second; }
	FEATHERMOONEMU_INLINE bool End() { return (m_itr==m_endItr)?true:false; }
};
