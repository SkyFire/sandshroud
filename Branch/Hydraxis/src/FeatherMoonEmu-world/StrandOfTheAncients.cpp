...
StrandOfTheAncients::StrandOfTheAncients(MapMgr* mgr, uint32 id, uint32 lgroup, uint32 t) : CBattleground(mgr,id,lgroup,t)
{
	...
	if(m_endgate == NULL || !m_endgate->CreateFromProto(192549, m_mapMgr->GetMapId() ,878.555f, -108.989f, 119.835f, 0.0565f, 0, 0, 0, 0))
	{
		Log.LargeErrorMessage(LARGERRORMESSAGE_ERROR, "SOTA is being created and you are missing gameobjects. Terminating.");
		abort();
		return;
	...
	if(m_relic == NULL || !m_relic->CreateFromProto(192834, m_mapMgr->GetMapId() , 836.5f , -108.8f , 150.0f ,0.0f, 0, 0, 0, 0))
	{
		Log.LargeErrorMessage(LARGERRORMESSAGE_ERROR, "SOTA is being created and you are missing gameobjects. Terminating.");
		abort();
		return;
	...
	/*for(uint32 x = 0; x < 10; x++)
	{
		...
		if(Attackers == HORDE)
			m_cannons[x]->SetUInt32Value(UNIT_FIELD_FACTIONTEMPLATE, HORDE);
		else if(Attackers == ALLIANCE)
			m_cannons[x]->SetUInt32Value(UNIT_FIELD_FACTIONTEMPLATE, ALLIANCE);
