...
enum MOD_TYPES
{
	...
	SPELL_AURA_272 = 272,
	SPELL_AURA_273 = 273,
	SPELL_AURA_REMOVE_AMMO_COST = 274,
	SPELL_AURA_275 = 275,
	SPELL_AURA_276 = 276,
...
public:
	...
	FEATHERMOONEMU_INLINE SpellEntry* GetSpellProto() const { return m_spellProto; }
	FEATHERMOONEMU_INLINE uint32 GetSpellId() const {return m_spellProto->Id; }
	FEATHERMOONEMU_INLINE bool IsPassive(){ if(!m_spellProto) return false; return (m_spellProto->Attributes & ATTRIBUTES_PASSIVE && !m_areaAura);}

	FEATHERMOONEMU_INLINE int32 GetDuration() const { return m_duration; }
	void SetDuration(int32 duration) { m_duration = duration; }
	void SetTimeLeft(int32 time);

	FEATHERMOONEMU_INLINE uint32 GetModCount() const { return m_modcount; }
	FEATHERMOONEMU_INLINE Modifier *GetMod(uint32 x) { return &m_modList[x]; }

	FEATHERMOONEMU_INLINE uint8 GetAuraSlot() const { return m_auraSlot; }
	void SetAuraSlot(uint8 slot) { m_auraSlot = slot; }
	FEATHERMOONEMU_INLINE uint8 GetAuraFlags() const { return m_auraFlags; }
	void SetAuraFlags(uint8 flags) { m_auraFlags = flags; }
	void SetAuraFlag(uint16 flag) { if( ! (m_auraFlags & flag ) ) m_auraFlags |= flag; };
	FEATHERMOONEMU_INLINE uint8 GetAuraLevel() const { return m_auraLevel; }
	void SetAuraLevel(uint8 level) { m_auraLevel = level; }

	FEATHERMOONEMU_INLINE bool IsPositive() { return m_positive>0; }
	...
	Unit* GetUnitCaster();
	FEATHERMOONEMU_INLINE Object* GetCaster() {return TO_OBJECT(GetUnitCaster());}
	FEATHERMOONEMU_INLINE uint64 GetCasterGUID(){return m_casterGuid;}

	FEATHERMOONEMU_INLINE Object* GetTarget() {return ((m_target != NULL && m_target->IsInWorld()) ? TO_OBJECT(m_target): NULLOBJ);}
	FEATHERMOONEMU_INLINE Unit* GetUnitTarget() {return TO_UNIT(GetTarget());}
	FEATHERMOONEMU_INLINE uint64 GetTargetGUID(){return m_target->GetGUID();}
	...
	FEATHERMOONEMU_INLINE uint32 GetTimeLeft()//in sec
	{
	...
	FEATHERMOONEMU_INLINE uint32 GetMSExpiryTime()
	{ 		
		if (GetDuration() == -1)
			return -1;
...
//	void SpellAuraIncreaseHealingByInt(bool apply);
	...
	FEATHERMOONEMU_INLINE bool IsCombatStateAffecting()
	{
	...
	FEATHERMOONEMU_INLINE bool HasMechanic(uint32 MechanicsType)
	{
		return Spell::HasMechanic(m_spellProto, MechanicsType);
	}
	FEATHERMOONEMU_INLINE uint32 GetMechanic()
	{
		return Spell::GetMechanic(m_spellProto);
	}

	FEATHERMOONEMU_INLINE uint32 GetMechanicOfEffect(uint32 i)
	{
...
private:
	...
	FEATHERMOONEMU_INLINE void DurationPctMod(uint32 mechanic);

	FEATHERMOONEMU_INLINE bool IsInrange(float x1,float y1, float z1, Object* o,float square_r)
	{
		float t;
		float r;
		t=x1-o->GetPositionX();
...
public:
	...
	FEATHERMOONEMU_INLINE bool IsInterrupted() { return ( m_interrupted >= 0 ); }
