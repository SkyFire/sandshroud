...
FEATHERMOONEMU_INLINE bool CanAgroHash(uint32 spellhashname)
{
...
FEATHERMOONEMU_INLINE bool IsCastedOnFriends(SpellEntry *sp)
{
...
FEATHERMOONEMU_INLINE bool IsCastedOnEnemies(SpellEntry *sp)
{
	for( int frloop = 0; frloop < 3; frloop++ )
	{
		switch (sp->EffectImplicitTargetA[frloop])
...
/************************************************************************/
FEATHERMOONEMU_INLINE bool IsDamagingSpell(SpellEntry *sp)
{
...
FEATHERMOONEMU_INLINE bool IsHealingSpell(SpellEntry *sp)
{
	for(uint32 i = 0; i < 3; ++i)
	{
		switch( sp->Effect[i] )
...
FEATHERMOONEMU_INLINE bool IsInrange(LocationVector & location, Object* o, float square_r)
{
...
FEATHERMOONEMU_INLINE bool IsInrange(float x1,float y1, float z1,float x2,float y2, float z2,float square_r)
{
	float t;
	float r;
	t=x1-x2;
...
FEATHERMOONEMU_INLINE bool IsInrange(Object* o1,Object* o2,float square_r)
{
	return IsInrange(o1->GetPositionX(),o1->GetPositionY(),o1->GetPositionZ(),
		o2->GetPositionX(),o2->GetPositionY(),o2->GetPositionZ(),square_r);
}

FEATHERMOONEMU_INLINE bool TargetTypeCheck(Object* obj,uint32 ReqCreatureTypeMask)
{
...
FEATHERMOONEMU_INLINE bool IsFlyingSpell(SpellEntry *sp)
{
...
FEATHERMOONEMU_INLINE bool IsTargetingStealthed(SpellEntry *sp)
{
	if(	sp->Id == 3600 )
		return false;
...
public:
	...
	FEATHERMOONEMU_INLINE uint32 getState() { return m_spellState; }
	FEATHERMOONEMU_INLINE void SetUnitTarget(Unit* punit){unitTarget=punit;}
	FEATHERMOONEMU_INLINE SpellEntry *GetSpellProto() { return m_spellInfo; }
	...
	FEATHERMOONEMU_INLINE uint32 GetType() { return ( m_spellInfo->Spell_Dmg_Type == SPELL_DMG_TYPE_NONE ? SPELL_DMG_TYPE_MAGIC : m_spellInfo->Spell_Dmg_Type ); }
	FEATHERMOONEMU_INLINE Item* GetItemTarget() { return itemTarget; }
	FEATHERMOONEMU_INLINE Unit* GetUnitTarget() { return unitTarget; }
	FEATHERMOONEMU_INLINE Player* GetPlayerTarget() { return playerTarget; }
	FEATHERMOONEMU_INLINE GameObject* GetGameObjectTarget() { return gameObjTarget; }
	...
	FEATHERMOONEMU_INLINE float GetRadius(uint32 i)
	{
	...
	FEATHERMOONEMU_INLINE bool IsPassiveSpell(uint32 spellId)
	{
	...
	FEATHERMOONEMU_INLINE static uint32 GetBaseThreat(uint32 dmg)
	{
...
/*		switch(m_spellInfo->NameHash)
	...
	FEATHERMOONEMU_INLINE static bool HasMechanic(SpellEntry * sp, uint32 MechanicsType)
	{
	...
	FEATHERMOONEMU_INLINE static uint32 GetMechanic(SpellEntry * sp)
	{
		if(sp->MechanicsType)
	...
	FEATHERMOONEMU_INLINE static uint32 GetMechanicOfEffect(SpellEntry * sp, uint32 i)
	{
	...
	FEATHERMOONEMU_INLINE void safe_cancel()
	{
		m_cancelled = true;
	}
	...
	/// Spell state's
	/// Spell failed
	FEATHERMOONEMU_INLINE bool GetSpellFailed(){return m_Spell_Failed;}
	FEATHERMOONEMU_INLINE void SetSpellFailed(bool failed = true){m_Spell_Failed = failed;}

	Spell* m_reflectedParent;

	// Returns true iff spellEffect's effectNum effect affects testSpell based on EffectSpellClassMask
	FEATHERMOONEMU_INLINE static bool EffectAffectsSpell(SpellEntry* spellEffect, uint32 effectNum, SpellEntry* testSpell)
	{
