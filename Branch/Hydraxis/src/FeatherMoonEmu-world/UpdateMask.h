...
public:
	...
	FEATHERMOONEMU_INLINE uint32 GetBlockCount() const {return mBlocks;}
	
	FEATHERMOONEMU_INLINE uint32 GetLength() const { return (mBlocks *sizeof(uint32)); }
	FEATHERMOONEMU_INLINE uint32 GetCount() const { return mCount; }
	FEATHERMOONEMU_INLINE const uint8* GetMask() const { return (uint8*)mUpdateMask; }
