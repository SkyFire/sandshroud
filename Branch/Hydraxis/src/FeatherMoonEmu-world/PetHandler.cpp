			...
			case PET_ACTION_DISMISS:
				{
					...
					if(pPet->IsVehicle())
					{
						Vehicle* veh = _player->GetMapMgr()->GetVehicle(GET_LOWGUID_PART(petGuid));
...
void WorldSession::HandleTotemDestroyed(WorldPacket & recv_data)
{
	...
	if(!_player->m_SummonSlots[slotId])
		return;

	if( slotId < 4 && _player->m_SummonSlots[slotId] )
		_player->SummonExpireSlot(slotId);
}
