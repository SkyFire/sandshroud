...
_logoutTime(0), permissions(NULL), permissioncount(0), _loggingOut(false), m_playerLogout(false), instanceId(0), m_playerRecentlyLogout(false)
{
	_player = NULLPLR;
...
int WorldSession::Update(uint32 InstanceID)
{
	...
	while ((packet = _recvQueue.Pop()))
	{
		...
		else
		{
			...
			else if(Handler->status == STATUS_LOGGEDIN_OR_RECENTLY_LOGGOUT && !_player && !m_playerRecentlyLogout && Handler->handler != 0)
			{
				Log.Warning("WorldSession","Received unexpected/wrong state packet with opcode %s (0x%.4X)",
					LookupOpcodeName(packet->GetOpcode()), packet->GetOpcode());
			}
			else
			{
				...
				else
				{
					...
					{
						// This is important, so we Printf, instead of debug.
						printf("Packet size is smaller than handle size.\n");
						printf("Recieved Size: %u, handled size: %u\n", packet->wpos(), packet->rpos());
					}
					else
					{
					...
					// Send Unprocessed Opcode Data if not handled correctly, which, is bullshit.
					if((sLog.IsOutProcess() && packet->rpos() < packet->wpos()) && (packet->GetOpcode() != CMSG_GMTICKET_CREATE))
...
void WorldSession::InitPacketHandlerTable()
{
	...
	for(uint32 i = 0; i < NUM_MSG_TYPES; ++i)
	{
		...
		WorldPacketHandlers[i].handler = &WorldSession::Ignore;
	}
	// Login
	...
	WorldPacketHandlers[CMSG_CORPSE_MAP_POSITION_QUERY].handler				= &WorldSession::HandleCorpseMapPositionQuery;
	...
	// When a player logs out in a vehicle, they send this opcode.
	WorldPacketHandlers[CMSG_SET_ACTIVE_MOVER].status						= STATUS_LOGGEDIN_OR_RECENTLY_LOGGOUT;
	...
	// Sets selection to 0 if player logs out with a target.
	WorldPacketHandlers[CMSG_SET_SELECTION].status							= STATUS_LOGGEDIN_OR_RECENTLY_LOGGOUT;
	...
	WorldPacketHandlers[CMSG_HEARTH_AND_RESURRECT].handler					= &WorldSession::HandleHAndROpcode;
	...
	WorldPacketHandlers[CMSG_LEAVE_CHANNEL].status							= STATUS_LOGGEDIN_OR_RECENTLY_LOGGOUT;
	...
	WorldPacketHandlers[CMSG_LFG_SET_ROLES].handler							= &WorldSession::HandleLfgSetRoles;
	...
	// Equipment Manager
	WorldPacketHandlers[CMSG_EQUIPMENT_SET_SAVE].handler					= &HandleEquipmentSetSave;
	WorldPacketHandlers[CMSG_EQUIPMENT_SET_DELETE].handler					= &HandleEquipmentSetDelete;
	WorldPacketHandlers[CMSG_EQUIPMENT_SET_USE].handler						= &HandleEquipmentSetUse;
	...
	WorldPacketHandlers[CMSG_CANCEL_TRADE].status							= STATUS_LOGGEDIN_OR_RECENTLY_LOGGOUT;
	...
	WorldPacketHandlers[CMSG_SET_ACTIONBAR_TOGGLES].status					= STATUS_AUTHED;
	...
	// Vehicles
	WorldPacketHandlers[CMSG_DISMISS_CONTROLLED_VEHICLE].handler			= &WorldSession::HandleDismissControlledVehicle;
	WorldPacketHandlers[CMSG_CHANGE_SEATS_ON_CONTROLLED_VEHICLE].handler	= &WorldSession::HandleChangeSeatsOnControlledVehicle;
	WorldPacketHandlers[CMSG_REQUEST_VEHICLE_EXIT].handler					= &WorldSession::HandleRequestVehicleExit;

	// Ignored Opcodes TODO: make these opcodes work.
	WorldPacketHandlers[CMSG_FAR_SIGHT].handler								= &WorldSession::Ignore;
	WorldPacketHandlers[CMSG_AUCTION_LIST_PENDING_SALES].handler			= &WorldSession::Ignore;
	WorldPacketHandlers[CMSG_ITEM_REFUND_INFO].handler						= &WorldSession::Ignore;
	WorldPacketHandlers[CMSG_ITEM_REFUND_REQUEST].handler					= &WorldSession::Ignore;
}

void WorldSession::Ignore(WorldPacket & recv_data)
{
	SKIP_READ_PACKET(recv_data);

	WorldPacket *packet;
	if(sLog.m_fileLogLevel > 2)
	{
		Log.Warning("WorldSession","Received ignored packet with opcode %s (0x%.4X)",
			LookupOpcodeName(packet->GetOpcode()), packet->GetOpcode());
	}
}
...
void WorldSession::ReadMovementInfo(WorldPacket &data, MovementInfo *mi)
{
	...
	if((mi->flags & MOVEFLAG_TAXI) && (mi->flags  & MOVEFLAG_ROOT) && (data.size() == 52))
	{
		data >> mi->transSeat;
		data >> mi->transX;
		data >> mi->transY;
	...
	else if(mi->flags & MOVEFLAG_TAXI)
	{
		uint64 guid = mi->transGuid.GetOldGuid();
	...
	if((mi->flags & (MOVEFLAG_SWIMMING | MOVEFLAG_AIR_SWIMMING)) || (mi->flag16 & 0x20))
	{
		data >> mi->pitch;
	}
	...
	if(mi->flags & MOVEFLAG_JUMPING)
	{
		data >> mi->jump_zSpeed;
		data >> mi->jump_sinAngle;
		data >> mi->jump_cosAngle;
		data >> mi->jump_xySpeed;
	}

	if(mi->flags & MOVEFLAG_SPLINE_MOVER)
	{
		data >> mi->spline_unk;
	}
}
