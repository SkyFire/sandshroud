...
public:
	...
	FEATHERMOONEMU_INLINE string GetNameString() { return name; }
	FEATHERMOONEMU_INLINE const char* GetName() { return name.c_str(); }
	FEATHERMOONEMU_INLINE MapEntry* GetDBCEntry() { return me; }

	FEATHERMOONEMU_INLINE CellSpawns *GetSpawnsList(uint32 cellx,uint32 celly)
	{
		ASSERT(cellx < _sizeX);
		ASSERT(celly < _sizeY);
	...
	FEATHERMOONEMU_INLINE CellSpawns * GetSpawnsListAndCreate(uint32 cellx, uint32 celly)
	{
		ASSERT(cellx < _sizeX);
		ASSERT(celly < _sizeY);
	...
	TempSummon *SummonCreature(uint32 entry, const LocationVector &pos, SummonPropertiesEntry const *properties = NULL, uint32 duration = 0, Unit *summoner = NULL, uint32 vehId = 0);
	void LoadSpawns(bool reload);//set to true to make clean up
	uint32 CreatureSpawnCount;
	uint32 GameObjectSpawnCount;

	FEATHERMOONEMU_INLINE float  GetLandHeight(float x, float y)
	{ 
		if(_terrain)
		{
	...
	FEATHERMOONEMU_INLINE float  GetWaterHeight(float x, float y) 
	{ 
		if(_terrain)
		{ 
	...
	FEATHERMOONEMU_INLINE uint8  GetWaterType(float x, float y)
	{
		if(_terrain)
		{ 
	...
	FEATHERMOONEMU_INLINE uint8  GetWalkableState(float x, float y)
	{
		if(_terrain)
		{ 
	...
	FEATHERMOONEMU_INLINE uint16 GetAreaID(float x, float y)
	{
		if(_terrain)
		{ 
	...
	FEATHERMOONEMU_INLINE void CellGoneActive(uint32 x, uint32 y)
	{ 
		if(_terrain)
		{ 
	...
	FEATHERMOONEMU_INLINE void CellGoneIdle(uint32 x, uint32 y)
	{ 
		if(_terrain)
		{ 
