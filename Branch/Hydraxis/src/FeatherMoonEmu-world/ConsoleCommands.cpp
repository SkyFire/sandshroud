...
bool HandleInfoCommand(BaseConsole * pConsole, int argc, const char * argv[])
{
	...
	pConsole->Write("======================================================================\r\n");
	pConsole->Write("Server Revision: FeathermoonEmu r%u/%s-%s-%s\r\n", BUILD_REVISION, CONFIG, PLATFORM_TEXT, ARCH);
...
bool HandleGMsCommand(BaseConsole * pConsole, int argc, const char * argv[])
{
	...
	for (itr = objmgr._players.begin(); itr != objmgr._players.end(); itr++)
	{
		if(itr->second->GetSession()->GetPermissionCount())
		{
			pConsole->Write("| %21s | %15s | %04s |\r\n" , itr->second->GetName(), itr->second->GetSession()->GetPermissions(), itr->second->GetSession()->GetAccountName());
		}
	}
	objmgr._playerslock.ReleaseReadLock();
...
bool HandleSendToGmCommand(BaseConsole * pConsole, int argc, const char * argv[])
{
	char pAnnounce[1024];
	string outstr;
	if(argc < 2)
	...
	sWorld.SendMessageToGMs(0, pAnnounce); // send message
	pConsole->Write("%s\r\n", outstr.c_str());
...
bool HandleRestartCommand(BaseConsole * pConsole, int argc, const char * argv[])
{
	...
	pConsole->Write("Restart initiated.\r\n");
...
{
	if(sWorld.m_shutdownType = SERVER_SHUTDOWN_TYPE_SHUTDOWN)
	{
		pConsole->Write("Shutdown canceled.\r\n");
	}
	else
	{
		pConsole->Write("Restart canceled.\r\n");
	}
