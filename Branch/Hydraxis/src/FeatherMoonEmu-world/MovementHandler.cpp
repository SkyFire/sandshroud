...
FEATHERMOONEMU_INLINE uint32 mTimeStamp()
{
	return timeGetTime();
}
...
void WorldSession::HandleMoveTeleportAckOpcode( WorldPacket & recv_data )
{
	...
	if(guid == _player->GetGUID())
	{
		if(sWorld.antihack_teleport && !(HasGMPermissions() && sWorld.no_antihack_on_gm) && _player->GetPlayerStatus() != TRANSFER_PENDING)
		{
			...
			printf("Account %u, Used teleport hack, disconnecting.\n", _socket->GetSession()->GetAccountId());
			Disconnect();
			return;
		}
...
void WorldSession::HandleMovementOpcodes( WorldPacket & recv_data )
{
	...
	{
		printf("Recieved larger movement packet than 80, size is %u", recv_data.size());
		Disconnect();
		return;
	}
	...
	if( sWorld.antihack_cheatengine && _player->m_lastMovementPacketTimestamp != 0 && (int32)mstime - (int32)_player->m_lastMovementPacketTimestamp != 0)
	{
		...
		if( diff >= threshold )		// replace with threshold var
		{
			...
			if( _player->m_cheatEngineChances == 1 )
			{
				...
				_player->BroadcastMessage( "Cheat engine detected. Please contact an admin with the below information ifyou believe this is a false detection." );
	...
	{
		printf("Account %u, has incorrect co-ordinates, disconnecting.\n", _socket->GetSession()->GetAccountId());
		Disconnect();
		return;
	}
	...
	//update the detector
	if( sWorld.antihack_speed && !_player->GetTaxiState() && _player->m_TransporterGUID == 0 && !_player->GetSession()->HasGMPermissions())
	{
	...
	/************************************************************************/
	if( !_player->GetSession()->HasGMPermissions() && !movement_info.transGuid.GetOldGuid() && recv_data.GetOpcode() != MSG_MOVE_JUMP && !_player->FlyCheat && !_player->flying_aura && !(movement_info.flags & MOVEFLAG_SWIMMING || movement_info.flags & MOVEFLAG_FALLING) && movement_info.z > _player->GetPositionZ() && movement_info.x == _player->GetPositionX() && movement_info.y == _player->GetPositionY() )
	{
	...
	if( !_player->GetSession()->HasGMPermissions() && (movement_info.flags & MOVEFLAG_AIR_SWIMMING) && !(movement_info.flags & MOVEFLAG_SWIMMING) && !(_player->flying_aura || _player->FlyCheat))
	{
		WorldPacket data (SMSG_MOVE_UNSET_CAN_FLY, 13);
		data << _player->GetNewGUID();
		data << uint32(5);
	...
	else
	{
		...
		if( recv_data.GetOpcode() == MSG_MOVE_FALL_LAND )
		{
			...
			if( _player->isAlive() &&  !_player->bInvincible && !_player->GodModeCheat && falldistance > 12 && ( getMSTime() >= _player->m_fallDisabledUntil ) )
			{
...
{
	// Skip the packet data, its basically an acknowledgement opcode.
	SKIP_READ_PACKET(recvPacket);
	// ? was _player->z_axisposition = 0.0f, but lets see what this does.
	_player->z_axisposition = _player->GetMapMgr()->GetLandHeight( _player->GetPositionX(), _player->GetPositionY() );
}
