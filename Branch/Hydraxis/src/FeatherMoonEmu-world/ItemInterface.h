...
public:
	...
	*/
	...
	void ItemSets(ItemSetEntry * itemset);
	...
	//buyback stuff
	FEATHERMOONEMU_INLINE Item* GetBuyBack(int32 slot) 
	{ 
		if(slot >= 0 && slot <= 12)
...
public:
	FEATHERMOONEMU_INLINE bool VerifyBagSlots(int8 ContainerSlot, int8 Slot)
	{
	...
	FEATHERMOONEMU_INLINE bool VerifyBagSlotsWithBank(int8 ContainerSlot, int8 Slot)
	{
		if( ContainerSlot < -1 || Slot < 0 )
			return false;
	...
	FEATHERMOONEMU_INLINE bool VerifyBagSlotsWithInv(int8 ContainerSlot, int8 Slot)
	{
		if( ContainerSlot < -1 || Slot < 0 )
			return false;
...
public:
	...
	FEATHERMOONEMU_INLINE Item* Grab() { return m_currentItem; }
	FEATHERMOONEMU_INLINE bool End() { return m_atEnd; }
};
