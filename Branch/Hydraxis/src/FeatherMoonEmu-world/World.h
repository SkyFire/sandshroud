...
{
public:
	static float GetMaxVisibleDistanceOnContinents()    { return MapInfo().update_distance; }
	FEATHERMOONEMU_INLINE uint32 getHordePlayerCount() { return HordePlayers; }
	FEATHERMOONEMU_INLINE uint32 getAlliancePlayerCount() { return AlliancePlayers; }
	FEATHERMOONEMU_INLINE uint32 getPlayerCount() { return (HordePlayers + AlliancePlayers); }
	FEATHERMOONEMU_INLINE void resetPlayerCount() { HordePlayers = AlliancePlayers = 0; }
	FEATHERMOONEMU_INLINE void incrementPlayerCount(uint32 faction)
	{
		if( faction == 1 )
			HordePlayers++;
		else
	...
	FEATHERMOONEMU_INLINE void decrementPlayerCount(uint32 faction)
	{
		if( faction == 1 )
			HordePlayers--;
		else
...
public:
	...
	FEATHERMOONEMU_INLINE size_t GetSessionCount() const { return m_sessions.size(); }

	FEATHERMOONEMU_INLINE size_t GetQueueCount() { return mQueuedSessions.size(); }
	void GetStats(uint32 * GMCount, float * AverageLatency);

	FEATHERMOONEMU_INLINE uint32 GetPlayerLimit() const { return m_playerLimit; }
	void SetPlayerLimit(uint32 limit) { m_playerLimit = limit; }

	FEATHERMOONEMU_INLINE uint32 GetRequiredBuild() const { return ServerReqBuild; }
	void SetRequiredBuild(uint32 build) { ServerReqBuild = build; }

	FEATHERMOONEMU_INLINE bool getAllowMovement() const { return m_allowMovement; }
	void SetAllowMovement(bool allow) { m_allowMovement = allow; }
	FEATHERMOONEMU_INLINE bool getGMTicketStatus() { return m_gmTicketSystem; };
	...
	FEATHERMOONEMU_INLINE std::string getGmClientChannel() { return GmClientChannel; }

	void SetMotd(const char *motd) { m_motd = motd; }
	void SetMotd2(const char *motd) { m_motd2 = motd; }
	FEATHERMOONEMU_INLINE const char* GetMotd() const { return m_motd.c_str(); }
	FEATHERMOONEMU_INLINE const char* GetMotd2() const { return m_motd2.c_str(); }

	FEATHERMOONEMU_INLINE time_t GetGameTime() const { return m_gameTime; }
	...
	FEATHERMOONEMU_INLINE void SetStartTime(uint32 val) { m_StartTime = val; }
	FEATHERMOONEMU_INLINE uint32 GetUptime(void) { return (uint32)UNIXTIME - m_StartTime; }
	FEATHERMOONEMU_INLINE uint32 GetStartTime(void) { return m_StartTime; }
	...
	FEATHERMOONEMU_INLINE void setRate(int index,float value)
	{
		regen_values[index]=value;
	}

	FEATHERMOONEMU_INLINE float getRate(int index)
	{
		return regen_values[index];
	}
	
	FEATHERMOONEMU_INLINE uint32 getIntRate(int index)
	{
		return int_rates[index];
	}

	FEATHERMOONEMU_INLINE void setIntRate(int index, uint32 value)
	{
	...
	FEATHERMOONEMU_INLINE uint32 GetTimeOut(){return TimeOut;}
