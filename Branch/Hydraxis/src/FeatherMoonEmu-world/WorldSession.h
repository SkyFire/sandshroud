...
enum SessionStatus
{
	STATUS_AUTHED = 0,
	STATUS_LOGGEDIN,
	STATUS_TRANSFER, // All of the below are used for reducing spam and allowing the handle of packets.
...
{
    AccountDataEntry() : Time(0), data(""), sz(0), bIsDirty(false) {}
...
public:
	...
	FEATHERMOONEMU_INLINE void SendPacket(WorldPacket* packet)
	{
		if(_socket && _socket->IsConnected())
			_socket->SendPacket(packet);
	}

	FEATHERMOONEMU_INLINE void SendPacket(StackPacket * packet)
	{
		if(_socket && _socket->IsConnected())
			_socket->SendPacket(packet);
	}

	FEATHERMOONEMU_INLINE void OutPacket(uint16 opcode)
	{
		if(_socket && _socket->IsConnected())
	...
	FEATHERMOONEMU_INLINE uint32 GetAccountId() const { return _accountId; }
	FEATHERMOONEMU_INLINE Player* GetPlayer() { return _player; }
	...
	FEATHERMOONEMU_INLINE char* GetPermissions() { return permissions; }
	FEATHERMOONEMU_INLINE int GetPermissionCount() { return permissioncount; }
	FEATHERMOONEMU_INLINE bool HasPermissions() { return (permissioncount > 0) ? true : false; }
	FEATHERMOONEMU_INLINE bool HasGMPermissions()
	{
		if(!permissioncount)
			return false;
	...
	FEATHERMOONEMU_INLINE void SetSocket(WorldSocket *sock)
	{
		_socket = sock;
	}
	FEATHERMOONEMU_INLINE void SetPlayer(Player* plr) { _player = plr; }

	FEATHERMOONEMU_INLINE void SetAccountData(uint32 index, char* data, bool initial,uint32 sz)
	{
		ASSERT(index < 8);
	...
	FEATHERMOONEMU_INLINE AccountDataEntry* GetAccountData(uint32 index)
	{
		ASSERT(index < 8);
	...
	FEATHERMOONEMU_INLINE void QueuePacket(WorldPacket* packet)
	{
	...
	FEATHERMOONEMU_INLINE WorldSocket* GetSocket() { return _socket; }
	...
	FEATHERMOONEMU_INLINE void SetInstance(uint32 Instance) { instanceId = Instance; }
	FEATHERMOONEMU_INLINE uint32 GetLatency() { return _latency; }
	FEATHERMOONEMU_INLINE string GetAccountName() { return _accountName; }
	FEATHERMOONEMU_INLINE const char * GetAccountNameS() { return _accountName.c_str(); }

	FEATHERMOONEMU_INLINE uint32 GetClientBuild() { return client_build; }
	FEATHERMOONEMU_INLINE void SetClientBuild(uint32 build) { client_build = build; }
	bool bDeleted;
	FEATHERMOONEMU_INLINE uint32 GetInstance() { return instanceId; }
	Mutex deleteMutex;
	...
	FEATHERMOONEMU_INLINE bool IsLoggingOut() { return _loggingOut; }
...
protected:
	...
	/// Ignore!
	void Ignore(WorldPacket& recvPacket);
	...
	void HandleLfgSetRoles(WorldPacket & recvPacket);
