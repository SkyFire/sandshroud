...
void Creature::Create (const char* name, uint32 vehId, uint32 mapid, float x, float y, float z, float ang)
{
...
void Creature::RemoveFromWorld(bool addrespawnevent, bool free_guid)
{
	...
	if(IsVehicle()) // Vehicles are a bitch, they don't delete right.
	{
		if(IsInWorld())
			Unit::RemoveFromWorld(true);
...
////////////AI
	...
	// kek
	for(list<AI_Spell*>::iterator itr = proto->AIspells.begin(); itr != proto->AIspells.end(); ++itr)
	{
