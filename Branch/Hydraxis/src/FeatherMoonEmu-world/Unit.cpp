...
void CombatStatusHandler::UpdateFlag()
{
	...
	if(n_status != m_lastStatus)
	{
		...
		if(n_status)
		{
			...
			if(!m_Unit->hasUnitState(UF_ATTACKING)) m_Unit->addStateFlag(UF_ATTACKING);
		}
		else
		{
			//printf(I64FMT" is no longer in combat.\n", m_Unit->GetGUID());
			m_Unit->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_COMBAT);
			if(m_Unit->hasUnitState(UF_ATTACKING)) m_Unit->clearStateFlag(UF_ATTACKING);
...
void Unit::SetMaxPower(Powers power, uint32 val)
{
	...
	if(GetTypeId() == TYPEID_PLAYER)
	{
		if(((Player*)this)->GetGroup())
			((Player*)this)->SetPartyUpdateFlag(GROUP_UPDATE_FLAG_MAXPOWER);
	}
...
{			
	if(IsPlayer())
	{
		...
		if(IsVehicle())
		{
			...
			data << uint32(0);
			TO_PLAYER(this)->SendMessageToSet( &data, true );
		}
...
void Unit::EnterVehicle(Vehicle *vehicle, int8 seatId)
{
	...
	if(GetTypeId() == TYPEID_PLAYER)
	{
		//TO_PLAYER(this)->StopCastingCharm();
		//TO_PLAYER(this)->StopCastingBindSight();
		TO_PLAYER(this)->Dismount();
		TO_PLAYER(this)->RemoveAurasByType(SPELL_AURA_MOUNTED);
	}
...
void Unit::SendMonsterMoveTransport(Unit *vehicleOwner)
{
	...
	data << uint32(MOVEMENTFLAG_ENTER_TRANSPORT);
...
void Unit::ExitVehicle()
{
	...
	RemoveUnitMovementFlag(MOVEFLAG_TAXI);
	m_movementInfo.transX = 0;
	m_movementInfo.transY = 0;
	m_movementInfo.transZ = 0;
	m_movementInfo.transO = 0;
	m_movementInfo.transTime = 0;
	m_movementInfo.transSeat = 0;

    Relocate(vehicle->GetBase()->GetPositionX(),
		vehicle->GetBase()->GetPositionY(),
		vehicle->GetBase()->GetPositionZ());

	if(TO_PLAYER(this)->GetGroup())
		TO_PLAYER(this)->SetGroupUpdateFlag(GROUP_UPDATE_FLAG_VEHICLE_SEAT);

	//Send leave vehicle, not correct
	if(GetTypeId() == TYPEID_PLAYER)
	{
		//((Player*)this)->SetClientControl(this, 1);
		TO_PLAYER(this)->SendTeleportAckMsg();
		TO_PLAYER(this)->SetFallInformation(0, GetPositionZ());
    }
...
void Unit::BuildMovementPacket(ByteBuffer *data) const
{
	...
	case TYPEID_UNIT:
		if(isInFlight())
			const_cast<Unit*>(this)->AddUnitMovementFlag(MOVEFLAG_MOVE_FORWARD | MOVEFLAG_LEVITATE);
		break;

	case TYPEID_PLAYER:
		// remove unknown, unused etc flags for now
		const_cast<Unit*>(this)->RemoveUnitMovementFlag(MOVEFLAG_IMMOBILIZED);
		if(isInFlight())
		{
			WPAssert(const_cast<Unit*>(this)->GetCurrentMovementGeneratorType() == FLIGHT_MOTION_TYPE);
			const_cast<Unit*>(this)->AddUnitMovementFlag(MOVEFLAG_MOVE_FORWARD | MOVEFLAG_IMMOBILIZED);
		}
		break;
	}
	
	*data << uint32(GetUnitMovementFlags()); // movement flags
	*data << uint16(m_movementInfo.flag16);    // 2.3.0
	...
	*data << GetOrientation();
	...
	// 0x00000200
	if(GetUnitMovementFlags() & MOVEFLAG_TAXI)
	{
		if(m_vehicle)
	...
	*data << (uint32)m_movementInfo.FallTime;
	...
	{
		*data << (float)m_movementInfo.jump_zSpeed;
		*data << (float)m_movementInfo.jump_sinAngle;
		*data << (float)m_movementInfo.jump_cosAngle;
		*data << (float)m_movementInfo.jump_xySpeed;
	}

	// 0x04000000
	if(GetUnitMovementFlags() & MOVEFLAG_SPLINE_MOVER)
		*data << (float)m_movementInfo.spline_unk;
...
void Unit::RemoveAurasByType(uint32 auraType, uint64 casterGUID, Aura * except, bool negative, bool positive)
{
...
bool Unit::CreateVehicleKit(uint32 id)
{
	...
	VehicleEntry *vehInfo = dbcVehicle.LookupEntry(id);
	...
	m_vehicleKit = new Vehicle(this->GetGUID(), vehInfo);
...
void Unit::SetMinion(Minion *minion, bool apply)
{
	...
	if(apply)
	{
		if(!minion->SetUInt64Value(UNIT_FIELD_SUMMONEDBY, GetGUID()))
		{
		...
		{
			if(SetUInt64Value(UNIT_FIELD_SUMMON, minion->GetGUID()))
			{

			}
		}
		//else if(minion->m_Properties && minion->m_Properties->Type == SUMMON_TYPE_MINIPET)
		//    SetUInt64Value(UNIT_FIELD_CRITTER, minion->GetGUID());
	...
	else
	{
		...
		{
			sLog.outError("SetMinion: Minion %u is not the minion of owner %u", minion->GetEntry(), GetEntry());
			return;
		}
...
void Unit::SetLevel(uint32 lvl)
{
	...
	// group update
	if ((GetTypeId() == TYPEID_PLAYER) && TO_PLAYER(this)->GetGroup())
		TO_PLAYER(this)->SetGroupUpdateFlag(GROUP_UPDATE_FLAG_LEVEL);
}
