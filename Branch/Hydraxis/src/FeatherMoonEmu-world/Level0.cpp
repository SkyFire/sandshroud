...
bool ChatHandler::HandleStartCommand(const char* args, WorldSession *m_session)
{
	...
	{
		FEATHERMOONEMU_TOLOWER(argument);
...
bool ChatHandler::HandleInfoCommand(const char* args, WorldSession *m_session)
{
	...
	GreenSystemMessage(m_session, "Alliance Online: |r%u", sWorld.AlliancePlayers);
	GreenSystemMessage(m_session, "Horde Online: |r%u", sWorld.HordePlayers);
