...
void ObjectMgr::LoadPlayerCreateInfo()
{
	...
	do
	{
		...
		pPlayerCreateInfo->runic = fields[21].GetUInt32();
		pPlayerCreateInfo->attackpower = fields[22].GetUInt32();
		pPlayerCreateInfo->mindmg = fields[23].GetFloat();
		pPlayerCreateInfo->maxdmg = fields[24].GetFloat();
...
Player* ObjectMgr::GetPlayer(const char* name, bool caseSensitive)
{
	...
	if(!caseSensitive)
	{
		...
		FEATHERMOONEMU_TOLOWER(strName);
...
CreatureProto const* ObjectMgr::GetCreatureTemplate(uint32 id)
{
	return CreatureProtoStorage.LookupEntry(id);
}
