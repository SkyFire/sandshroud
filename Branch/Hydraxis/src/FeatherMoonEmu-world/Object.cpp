...
/*				// 3.2 players can turn xp off
				bool can_get_xp = true;
				if(pVictim->IsCreature())
				{
					if(TO_PLAYER(plr)->GetPlayerInfo())
						can_get_xp = (TO_PLAYER(plr)->GetPlayerInfo()->xp_off ? false : true);
				}*/
				if(can_give_xp)
				{
...
TempSummon* Object::SummonCreature(uint32 entry, const LocationVector &pos, TempSummonType spwtype, uint32 duration, uint32 vehId) const
{
    if(Map *map = FindMap())
    {
        if(TempSummon *summon = map->SummonCreature(entry, pos, NULL, duration, isType(TYPE_UNIT) ? (Unit*)this : NULL))
        {
...
TempSummon* Map::SummonCreature(uint32 entry, const LocationVector &pos, SummonPropertiesEntry const *properties, uint32 duration, Unit *summoner, uint32 vehId)
{
...
//	if(!summon->Create(summon->GetCreatureInfo()->Name, this->_mapId, vehId, pos.x, pos.y, pos.z, pos.o))
//	{
//		delete summon;
//		return NULL;
//	}
