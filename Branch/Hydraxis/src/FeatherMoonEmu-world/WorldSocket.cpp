...
void WorldSocket::OnConnect()
{
	...
	_latency = getMSTime();
	WorldPacket data (SMSG_AUTH_CHALLENGE, 24);
	data << uint32(1);			// Unk
	data << mSeed;
...
void WorldSocket::_HandleAuthSession(WorldPacket* recvPacket)
{
	...
	if(sWorld.GetRequiredBuild() != 0)
	{
		if(mClientBuild != sWorld.GetRequiredBuild())
		{
			OutPacket(SMSG_AUTH_RESPONSE, 1, AUTH_VERSION_MISMATCH);
			return;
		}
	}
