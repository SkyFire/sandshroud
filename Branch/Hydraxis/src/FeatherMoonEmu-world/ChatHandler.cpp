	...
	case CHAT_MSG_CHANNEL:
		{
			...
			{
				SystemMessage("Your voice is currently muted by Feathermoon. This will expire in %s.", ConvertTimeStampToString(m_muted - (uint32)UNIXTIME).c_str());
				return;
			}
		}break;
	}
	...
	case CHAT_MSG_SAY:
		{
			...
			{
				if(CanUseCommand('a'))
					data = sChatHandler.FillMessageData( CHAT_MSG_SAY, GetPlayer()->m_modlanguage,  ("%s%s|r", MSG_COLOR_GREEN, msg.c_str()), _player->GetGUID(), _player->bGMTagOn ? 4 : 0 );
				else
			...
			else
			{
				...
				if(CanUseCommand('a'))
					data = sChatHandler.FillMessageData( CHAT_MSG_SAY, lang, ("%s%s|r", MSG_COLOR_GREEN, msg.c_str()), _player->GetGUID(), _player->bGMTagOn ? 4 : 0 );
				else
	...
	case CHAT_MSG_CHANNEL:
		{
			...
			if(chn != NULL && chn->m_general == true && chn->pDBC->id == 1 && chn->pDBC->flags == 3)
			{
				char Message[512];
				int Permission = GetPermissionCount();

				if( m_muted && m_muted >= (uint32)UNIXTIME )
					return;

				if( HasFlag( ACCOUNT_FLAG_VIP ) && Permission == 0 )							// VIP
					snprintf( Message, 512, "[VIP][%s]: %s%s|r",				GetPlayer()->GetName(), MSG_COLOR_CYAN,		msg.c_str() );
				else
				{
					if( Permission > 0 )																// GM
					{
						if( CanUseCommand('z') )														// Admin
							snprintf( Message, 512, "[Hydraxis Staff][%s]: %s%s|r",	GetPlayer()->GetName(),	MSG_COLOR_GREEN,	msg.c_str() );
						else																			// GameMaster
							snprintf( Message, 512, "[Hydraxis Staff][%s]: %s%s|r",	GetPlayer()->GetName(),	MSG_COLOR_GREEN,		msg.c_str() );
					}
					else
