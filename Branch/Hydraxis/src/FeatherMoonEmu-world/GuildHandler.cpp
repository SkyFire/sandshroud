...
void WorldSession::HandleCharterBuy(WorldPacket & recv_data)
{
	...
	uint32 unk1;
	std::string name;
	uint64 unk2, unk3, unk4, unk5, unk6, unk7;
	uint32 unk8;
	uint16 unk9;
	uint8 arena_index;
	uint32 unk10;
	...
	recv_data >> unk, unk1;									// Find any variables.
	recv_data >> name;										// name
	recv_data >> unk2, unk3, unk4, unk5, unk6, unk7;		// TODO.
	recv_data >> unk8;										// Find any variables.
	recv_data >> unk9;										// Find any variables.
	recv_data >> arena_index;								// Index.
	recv_data >> unk10;										//
	recv_data >> unk11, unk12, unk13;
	...
	if(!crt)
	{
		printf("Disconnecting account %u, Buy charter fail!\n", _socket->GetSession()->GetAccountId());
		Disconnect();
		return;
	}
	...
	if((crt->GetCreatureInfo()->SubName == "Arena Organizer") || crt->GetEntry() == 19861 || crt->GetEntry() == 18897 || crt->GetEntry() == 19856 || crt->GetEntry() == 26012 || crt->GetEntry() == 29534 || crt->GetEntry() == 30611 || crt->GetEntry() == 32329)
	{
...
void WorldSession::HandleCharterSign( WorldPacket & recv_data )
{
	...
	Player* plr = _player->GetMapMgr()->GetPlayer( c->GetLeader() );
	if(!plr || plr->GetDistance2dSq(_player) > 1600 )
		return;
