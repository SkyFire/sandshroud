...
{
public:
	FEATHERMOONEMU_INLINE Condition(Mutex * mutex) : m_nLockCount(0), m_externalMutex(mutex)
	{
	...
	FEATHERMOONEMU_INLINE void BeginSynchronized()
	{
		m_externalMutex->Acquire();
		++m_nLockCount;
	}

	FEATHERMOONEMU_INLINE void EndSynchronized()
	{
...
{
public:
	FEATHERMOONEMU_INLINE Condition(Mutex *m)
	{
		mut=m;
		pthread_cond_init(&cond,NULL);
	}
	FEATHERMOONEMU_INLINE ~Condition()
	{
		pthread_cond_destroy(&cond);
	}

	FEATHERMOONEMU_INLINE void Signal()
	{
		pthread_cond_signal(&cond);
	}
	FEATHERMOONEMU_INLINE void Broadcast()
	{
		pthread_cond_broadcast(&cond);
	}
	FEATHERMOONEMU_INLINE void Wait()
	{
		pthread_cond_wait(&cond,&mut->mutex);
	}
	FEATHERMOONEMU_INLINE bool Wait(time_t seconds)
	{
		timespec tv;
		tv.tv_nsec = 0;
		tv.tv_sec = seconds;
