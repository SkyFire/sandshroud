...
public:
	...
	FEATHERMOONEMU_INLINE uint32 GetId() { return (uint32)thread_id; }
};
...
public:
	...
	FEATHERMOONEMU_INLINE void Gobble() { _threadsEaten=(int32)m_freeThreads.size(); }

	// gets active thread count
	FEATHERMOONEMU_INLINE uint32 GetActiveThreadCount() { return (uint32)m_activeThreads.size()+1; }

	// gets free thread count
	FEATHERMOONEMU_INLINE uint32 GetFreeThreadCount() { return (uint32)m_freeThreads.size(); }
};
