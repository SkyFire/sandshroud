...
/*
#define REPACK "Feathermoon Repack"
#define REPACK_AUTHOR "TheTrueCrow"
#define REPACK_WEBSITE "Sandshroud.servegame.org"*/
...
#ifdef WIN32
#define FEATHERMOONEMU_INLINE __forceinline
#else
#define FEATHERMOONEMU_INLINE inline
#endif
...
#ifdef WIN32
#define CROW_INLINE __forceinline
#else
#define CROW_INLINE inline
#endif
...
#include "feathermoonemuConfig.h"
...
#	define HEARTHSTONE_DLL_SPEC __declspec(dllexport)
#	ifndef DECLSPEC_NORETURN
#		define DECLSPEC_NORETURN __declspec(noreturn)
#	endif //DECLSPEC_NORETURN
#else //PLATFORM != PLATFORM_WINDOWS
#	define HEARTHSTONE_DLL_SPEC
...
FEATHERMOONEMU_INLINE uint32 now()
{	
#ifdef WIN32
	return GetTickCount();
#else
...
FEATHERMOONEMU_INLINE void reverse_array(uint8 * pointer, size_t count)
{
	size_t x;
...
FEATHERMOONEMU_INLINE void FEATHERMOONEMU_TOLOWER(std::string& str)
{
	for(size_t i = 0; i < str.length(); ++i)
		str[i] = (char)tolower(str[i]);
};

FEATHERMOONEMU_INLINE void FEATHERMOONEMU_TOUPPER(std::string& str)
{
	for(size_t i = 0; i < str.length(); ++i)
