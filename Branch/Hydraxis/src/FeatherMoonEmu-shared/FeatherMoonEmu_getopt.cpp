...
#include "Common.h"
#include "feathermoonemu_getopt.h"

int arg_counter = 1;
char feathermoonemu_optarg[514];
...
//	int return_val = 0;
	...
	for (; __longopts[i].name != 0; ++i)
	{
		if (!strnicmp(__longopts[i].name, opt, strlen(__longopts[i].name)))
		{
			...
			if (__longopts[i].has_arg == feathermoonemu_required_argument)
			{
				if(!par)
					return 1;
			...
			if (par)
				strncpy(feathermoonemu_optarg, par, 514);
