...
/*	uint32 ssdMultiplier[4];			// 3-6 Multiplier for ScalingStatDistribution
	uint32 armorMod[4];					// 7-10 Armor for level
	uint32 dpsMod[6];					// 11-16 DPS mod for level
	uint32 spellBonus;					// 17 spell power for level
	uint32 ssdMultiplier2;				// 18 there's data from 3.1 dbc ssdMultiplier[3]
	//uint32 unk1;						// 19 all fields equal to 0
	//uint32 unk2;						// 20 unk, probably also Armor for level
	uint32 armorMod2[4];				// 21-24 Armor for level*/
};

#pragma pack(pop)

FEATHERMOONEMU_INLINE float GetScale(CreatureDisplayInfo *Scale)
{
	return Scale->Scale;
}

FEATHERMOONEMU_INLINE float GetRadius(SpellRadius *radius)
{
    return radius->Radius;
}
FEATHERMOONEMU_INLINE uint32 GetCastTime(SpellCastTime *time)
{
    return time->CastTime;
}
FEATHERMOONEMU_INLINE float GetMaxRange(SpellRange *range)
{
    return range->maxRange;
}
FEATHERMOONEMU_INLINE float GetMinRange(SpellRange *range)
{
    return range->minRange;
}
FEATHERMOONEMU_INLINE uint32 GetDuration(SpellDuration *dur)
{
...
public:
	...
	FEATHERMOONEMU_INLINE uint32 GetNumRows()
	{
		return m_numrows;
	}
