...
/* Sending Operations */
	...
	FEATHERMOONEMU_INLINE void BurstBegin() { m_writeMutex.Acquire(); }
	...
	FEATHERMOONEMU_INLINE void BurstEnd() { m_writeMutex.Release(); }

/* Client Operations */

	// Get the client's ip in numerical form.
	string GetRemoteIP();
	FEATHERMOONEMU_INLINE uint32 GetRemotePort() { return ntohs(m_client.sin_port); }
	FEATHERMOONEMU_INLINE SOCKET GetFd() { return m_fd; }
...
/* Platform-specific methods */
	...
	FEATHERMOONEMU_INLINE bool IsDeleted() { return m_deleted; }
	FEATHERMOONEMU_INLINE bool IsConnected() { return m_connected; }
	FEATHERMOONEMU_INLINE sockaddr_in & GetRemoteStruct() { return m_client; }
	FEATHERMOONEMU_INLINE CircularBuffer& GetReadBuffer() { return readBuffer; }
	FEATHERMOONEMU_INLINE CircularBuffer& GetWriteBuffer() { return writeBuffer; }

/* Deletion */
	void Delete();

	FEATHERMOONEMU_INLINE in_addr GetRemoteAddress() { return m_client.sin_addr; }
...
public:
	...
	FEATHERMOONEMU_INLINE void SetCompletionPort(HANDLE cp) { m_completionPort = cp; }
	
	// Atomic wrapper functions for increasing read/write locks
	FEATHERMOONEMU_INLINE void IncSendLock() { InterlockedIncrement(&m_writeLock); }
	FEATHERMOONEMU_INLINE void DecSendLock() { InterlockedDecrement(&m_writeLock); }
	FEATHERMOONEMU_INLINE bool AcquireSendLock()
	{
		if(m_writeLock)
			return false;
		else
...
public:
	...
	// Atomic wrapper functions for increasing read/write locks
	FEATHERMOONEMU_INLINE void IncSendLock() { __sync_add_and_fetch(&m_writeLock, 1); }
	FEATHERMOONEMU_INLINE void DecSendLock() { __sync_sub_and_fetch(&m_writeLock, 1); }
	FEATHERMOONEMU_INLINE bool HasSendLock() { return (m_writeLock != 0); }
