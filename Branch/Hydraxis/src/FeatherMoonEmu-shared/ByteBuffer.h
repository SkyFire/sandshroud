...
class SERVER_DECL ByteBufferException
{
public:
	ByteBufferException(bool _add, size_t _pos, size_t _esize, size_t _size) : add(_add), pos(_pos), esize(_esize), size(_size)
	{
		PrintPosError();
	}
	...
	{
		printf("ERROR: Packet size is smaller than what we are trying to skip\n");
		printf("ERROR: Server has crashed due to unable to handle the new size\n");
	}
private:
	bool add;
	size_t pos;
...
public:
	...
	FEATHERMOONEMU_INLINE size_t size() const { return _storage.size(); };
	...
	FEATHERMOONEMU_INLINE void reverse()
	{
