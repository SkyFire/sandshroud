...
#include "onyx_packet.h"
#include "onyx_opcodes.h"
#include "voice_channel.h"
#include "voice_channelmgr.h"

#define onyxSOCKET_RBUF_LEN 5000
...
onyx_socket * onyxsocket_allocate(network_socket *s)
{
	onyx_socket * ret = (onyx_socket*)vc_malloc(sizeof(onyx_socket));
	ret->read_buf = (char*)vc_malloc(onyxSOCKET_RBUF_LEN);
	ret->read_buf_len = 0;
	ret->read_buf_sz = onyxSOCKET_RBUF_LEN;
	ret->sock = s;
	ret->channelcount = 0;
	++g_serverCount;
	return ret;
...
void onyxsocket_free(onyx_socket* s)
{
	if( s->channelcount > 0 )
...
void onyxsocket_send(onyx_socket *s, onyx_packet *p)
{
...
int voicechat_onyx_listen_socket_read_handler(network_socket *s, int act)
{
	...
	ns->event_handler = voicechat_onyx_socket_read_handler;
	ns->miscdata = (void*)onyxsocket_allocate(ns);		// used for voicechat struct later
	...
	if( network_add_socket(ns) < 0 )
	{
		...
		onyxsocket_free((onyx_socket*)ns->miscdata);
		network_close(ns);
	}
...
// client handlers
void vc_handler_createchannel(onyx_socket *s, onyx_packet *p)
{
	uint8 type;
	uint32 request_id;
	onyx_packet reply;
	...
	type = onyxpacket_readu8(p);
	request_id = onyxpacket_readu32(p);
	...
	if( chn == NULL )
	{
		...
		onyxpacket_init(VOICECHAT_SMSG_CHANNEL_CREATED, 5, &reply);
		onyxpacket_writeu32(&reply, request_id);
		onyxpacket_writeu8(&reply, error);

		// send reply
		onyxsocket_send(s, &reply);

		// free buffer
		onyxpacket_free(&reply);
	}
	else
	{
		...
		onyxpacket_init(VOICECHAT_SMSG_CHANNEL_CREATED, 7, &reply);
		onyxpacket_writeu32(&reply, request_id);
		onyxpacket_writeu8(&reply, error);
		onyxpacket_writeu16(&reply, reply_id);
...
void vc_handler_deletechannel(onyx_socket *s, onyx_packet *p)
{
	uint16 channel_id;
	voice_channel *ch;

	channel_id = onyxpacket_readu16(p);
...
void vc_handler_addmember(onyx_socket *s, onyx_packet *p)
{
	uint16 channel_id;
	uint8 member_id;
	voice_channel *ch;
...
void vc_handler_deletemember(onyx_socket *s, onyx_packet *p)
{
	uint16 channel_id;
	uint8 member_id;
	voice_channel *ch;
...
void vc_handler_ping(onyx_socket *s, onyx_packet *p)
{
	onyx_packet bp;
	uint32 pi;

	printf("ping!\n");
	pi = onyxpacket_readu32(p);

	onyxpacket_init(VOICECHAT_SMSG_PONG, 4, &bp);
	onyxpacket_writeu32(&bp, pi);
	onyxsocket_send(s, &bp);
	onyxpacket_free(&bp);
}

// handler table
typedef void(*vc_handler)(onyx_socket*,onyx_packet*);
...
{
	onyx_socket * mysock = (onyx_socket*)s->miscdata;
	uint16 opcode;
	uint16 len;
	int readcount;
	onyx_packet * pck;
	...
	if( readcount <= 0 )
	{
		log_write(DEBUG, "onyx socket %u failed reading. probably dead.", s->fd);
	...
	for(;;)
	{
		...
		// create onyxpacket structure
		pck = onyxpacket_create(opcode, len);
