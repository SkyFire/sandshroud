...
/* Sending Operations */
	...
	ONYX_INLINE void BurstBegin() { m_writeMutex.Acquire(); }
	...
	ONYX_INLINE void BurstEnd() { m_writeMutex.Release(); }

/* Client Operations */

	// Get the client's ip in numerical form.
	string GetRemoteIP();
	ONYX_INLINE uint32 GetRemotePort() { return ntohs(m_client.sin_port); }
	ONYX_INLINE SOCKET GetFd() { return m_fd; }
...
/* Platform-specific methods */
	...
	ONYX_INLINE bool IsDeleted() { return m_deleted; }
	ONYX_INLINE bool IsConnected() { return m_connected; }
	ONYX_INLINE sockaddr_in & GetRemoteStruct() { return m_client; }
	ONYX_INLINE CircularBuffer& GetReadBuffer() { return readBuffer; }
	ONYX_INLINE CircularBuffer& GetWriteBuffer() { return writeBuffer; }

/* Deletion */
	void Delete();

	ONYX_INLINE in_addr GetRemoteAddress() { return m_client.sin_addr; }
...
public:
	...
	ONYX_INLINE void SetCompletionPort(HANDLE cp) { m_completionPort = cp; }
	
	// Atomic wrapper functions for increasing read/write locks
	ONYX_INLINE void IncSendLock() { InterlockedIncrement(&m_writeLock); }
	ONYX_INLINE void DecSendLock() { InterlockedDecrement(&m_writeLock); }
	ONYX_INLINE bool AcquireSendLock()
	{
		if(m_writeLock)
			return false;
		else
...
public:
	...
	// Atomic wrapper functions for increasing read/write locks
	ONYX_INLINE void IncSendLock() { m_writeLockMutex.Acquire(); m_writeLock++; m_writeLockMutex.Release(); }
	ONYX_INLINE void DecSendLock() { m_writeLockMutex.Acquire(); m_writeLock--; m_writeLockMutex.Release(); }
	ONYX_INLINE bool HasSendLock() { bool res; m_writeLockMutex.Acquire(); res = (m_writeLock != 0); m_writeLockMutex.Release(); return res; }
...
public:
	...
	// Atomic wrapper functions for increasing read/write locks
	ONYX_INLINE void IncSendLock() { __sync_add_and_fetch(&m_writeLock, 1); }
	ONYX_INLINE void DecSendLock() { __sync_sub_and_fetch(&m_writeLock, 1); }
	ONYX_INLINE bool HasSendLock() { return (m_writeLock != 0); }
	bool AcquireSendLock()
	{
		if( m_writeLock != 0 )
			return false;
