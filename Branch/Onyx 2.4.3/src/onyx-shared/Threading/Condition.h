...
protected:
	HANDLE create_anonymous_semaphore(long cur,long max)
	{
		...
		ONYX_INLINE void shift_generations_down()
		{
		...
		ONYX_INLINE void dispose_entry(list_entry& entry)
		{
		...
		ONYX_INLINE HANDLE duplicate_handle(HANDLE source)
		{
...
{
public:
	ONYX_INLINE Condition(Mutex * mutex) : m_nLockCount(0), m_externalMutex(mutex)
	{
	...
	ONYX_INLINE void BeginSynchronized()
	{
		m_externalMutex->Acquire();
		++m_nLockCount;
	}

	ONYX_INLINE void EndSynchronized()
	{
...
{
public:
	ONYX_INLINE Condition(Mutex *m)
	{
		mut=m;
		pthread_cond_init(&cond,NULL);
	}
	ONYX_INLINE ~Condition()
	{
		pthread_cond_destroy(&cond);
	}

	ONYX_INLINE void Signal()
	{
		pthread_cond_signal(&cond);
	}
	ONYX_INLINE void Broadcast()
	{
		pthread_cond_broadcast(&cond);
	}
	ONYX_INLINE void Wait()
	{
		pthread_cond_wait(&cond,&mut->mutex);
	}
	ONYX_INLINE bool Wait(time_t seconds)
	{
		timespec tv;
		tv.tv_nsec = 0;
		tv.tv_sec = seconds;
