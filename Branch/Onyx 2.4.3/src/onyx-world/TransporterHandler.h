...
public:
	...
	ONYX_INLINE void SetLength(const unsigned int sz)
	{
		i_nodes.resize( sz );
	}

	ONYX_INLINE size_t Size(void) const { return i_nodes.size(); }
	ONYX_INLINE void Resize(unsigned int sz) { i_nodes.resize(sz); }
	ONYX_INLINE void Clear(void) { i_nodes.clear(); }
	ONYX_INLINE PathNode* GetNodes(void) { return static_cast<PathNode *>(&i_nodes[0]); }
...
public:
	...
	ONYX_INLINE void AddPlayer(Player *pPlayer) { mPassengers[pPlayer->GetLowGUID()] = pPlayer; }
	ONYX_INLINE void RemovePlayer(Player *pPlayer) {mPassengers.erase(pPlayer->GetLowGUID()); }
	ONYX_INLINE bool HasPlayer(Player* pPlayer) { return mPassengers.find(pPlayer->GetLowGUID()) != mPassengers.end(); }
	ONYX_INLINE void SetPeriod(uint32 val) { m_period = val; }
