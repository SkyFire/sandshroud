...
#define BANNER "onyx r%u/%s-%s-%s :: World Server\n"
...
bool Master::Run(int argc, char ** argv)
{
	...
	struct onyx_option longopts[] =
	{
		{ "checkconf",			onyx_no_argument,				&do_check_conf,			1		},
		{ "version",			onyx_no_argument,				&do_version,			1		},
		{ "conf",				onyx_required_argument,		NULL,					'c'		},
		{ "realmconf",			onyx_required_argument,		NULL,					'r'		},
		{ 0, 0, 0, 0 }
	};

	char c;
	while ((c = onyx_getopt_long_only(argc, argv, ":f:", longopts, NULL)) != -1)
	{
		switch (c)
		{
		case 'c':
			config_file = new char[strlen(onyx_optarg)];
			strcpy(config_file, onyx_optarg);
			break;

		case 'r':
			realm_config_file = new char[strlen(onyx_optarg)];
	...
	if( Config.MainConfig.SetSource( config_file ) )
		Log.Success( "Config", ">> onyx-world.conf" );
	else
	{
		Log.Error( "Config", ">> onyx-world.conf" );
		return false;
	}
	...
	{
		Log.Error( "Config", ">> onyx-realms.conf" );
		return false;
	}
	...
	int wsport = Config.RealmConfig.GetIntDefault( "Listen", "Port", DEFAULT_WORLDSERVER_PORT );
