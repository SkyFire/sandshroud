...
/*	if(qst_giver->GetTypeId() == TYPEID_UNIT && !ScriptSystem->OnQuestRequireEvent(qst, static_cast< Creature* >( qst_giver ), _player, QUEST_EVENT_CAN_ACCEPT))
	...
	for(uint32 i = 0; i < 4; ++i)
	{
		...
		{
			proto = ItemPrototypeStorage.LookupEntry(qst->receive_items[i]);
			if(proto != NULL)
			{
				if(proto->Unique)
				{
					if(_player->GetItemInterface()->GetItemCount(qst->srcitem) < 1) // We don't have the item.
					{
	...
	{
		// Crow: I am claiming this shit.
		proto = ItemPrototypeStorage.LookupEntry(qst->srcitem);
		if(proto != NULL)
		{
			if(proto->Unique) // Limit 1.
			{
				if(_player->GetItemInterface()->GetItemCount(qst->srcitem) < 1) // We don't have the item.
				{
			...
			else // We can have as many as we want, so fuck it :|
			{
				Item * item = objmgr.CreateItem( qst->srcitem, _player );
				if(item)
				{
...
void WorldSession::HandleQuestlogRemoveQuestOpcode(WorldPacket& recvPacket)
{
	...
	for(uint32 i = 0; i < 4; ++i)
	{
		if(qPtr->receive_items[i])
			if(qPtr->receive_items[i] != qPtr->srcitem)
