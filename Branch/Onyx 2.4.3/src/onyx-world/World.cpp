...
bool World::SetInitialWorldSettings()
{
	...
	CharacterDatabase.WaitExecute("DELETE FROM guild_logs WHERE timestamp <= %u", uint32(UNIXTIME - UNIX_TWO_WEEKS));
	CharacterDatabase.WaitExecute("DELETE FROM guild_banklogs WHERE timestamp <= %u", uint32(UNIXTIME - UNIX_TWO_WEEKS));
...
void World::Rehash(bool load)
{
	...
	{
		#ifdef WIN32
		Config.MainConfig.SetSource("onyx-world.conf", true);
		#else
		Config.MainConfig.SetSource((char*)CONFDIR "/onyx-world.conf", true);
		#endif
	}
