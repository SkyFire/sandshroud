...
public:
	...
	ONYX_INLINE CreatureInfo* GetCreatureInfo() { return creature_info; }

	/// Creature inventory
	ONYX_INLINE uint32 GetItemIdBySlot(uint32 slot) { return m_SellItems->at(slot).itemid; }
	ONYX_INLINE uint32 GetItemAmountBySlot(uint32 slot) { return m_SellItems->at(slot).amount; }

	ONYX_INLINE bool HasItems() { return ((m_SellItems != NULL) ? true : false); }
	...
	ONYX_INLINE void GetSellItemBySlot(uint32 slot, CreatureItem &ci)
	{
	...
	ONYX_INLINE std::vector<CreatureItem>::iterator GetSellItemBegin() { return m_SellItems->begin(); }
	ONYX_INLINE std::vector<CreatureItem>::iterator GetSellItemEnd()   { return m_SellItems->end(); }
	ONYX_INLINE size_t GetSellItemCount() { return m_SellItems->size(); }
	...
	ONYX_INLINE bool isQuestGiver() { return HasFlag( UNIT_NPC_FLAGS, UNIT_NPC_FLAG_QUESTGIVER ); };
	...
	ONYX_INLINE Player *GetTotemOwner() { return totemOwner; }
	ONYX_INLINE void SetTotemOwner(Player *owner) { totemOwner = owner; }
	ONYX_INLINE uint32 GetTotemSlot() { return totemSlot; }
	ONYX_INLINE void SetTotemSlot(uint32 slot) { totemSlot = slot; }

	ONYX_INLINE bool IsPickPocketed() { return m_PickPocketed; }
	ONYX_INLINE void SetPickPocketed(bool val = true) { m_PickPocketed = val; }

	ONYX_INLINE CreatureAIScript * GetScript() { return _myScriptClass; }
	void LoadScript();
	...
	ONYX_INLINE CreatureInfo *GetCreatureName()
	{
		return creature_info; 
	}
	ONYX_INLINE void SetCreatureName(CreatureInfo *ci) { creature_info = ci; }
	ONYX_INLINE Trainer* GetTrainer() { return mTrainer; }
	void RegenerateFocus();

	CreatureFamilyEntry * myFamily;
	ONYX_INLINE bool IsTotem() { return totemOwner != 0 && totemSlot != -1; }
	void TotemExpire();
	...
	CreatureProto * proto;
	ONYX_INLINE CreatureProto *GetProto() { return proto; }
