...
enum RankTitles
{
	...
	TITLE_OF_THE_SHATTERED_SUN	= 38,
	TITLE_HAND_OF_A_DAL			= 39,
...
struct FactionReputation
{
	...
	uint8 flag;
	int32 baseStanding;
	ONYX_INLINE int32 CalcStanding() { return standing - baseStanding; }
	ONYX_INLINE bool Positive() { return standing >= 0; }
};
...
public:
	...
	ONYX_INLINE Guild * GetGuild() { return m_playerInfo->guild; }
	ONYX_INLINE GuildMember * GetGuildMember() { return m_playerInfo->guildMember; }
	ONYX_INLINE GuildRank * GetGuildRankS() { return m_playerInfo->guildRank; }
...
public:
	...
	ONYX_INLINE std::string* GetNameString() { return &m_name; }
	void Die();
	...
	ONYX_INLINE bool IsInFeralForm()
	{
		int s = GetShapeShift();
		if( s <= 0 )
			return false;
	...
	ONYX_INLINE void SetPVPRank(int newrank)
	{
	...
	ONYX_INLINE void			AddPlayerPet(PlayerPet* pet, uint32 index) { m_Pets[index] = pet; }
	ONYX_INLINE PlayerPet*	GetPlayerPet(uint32 idx)
	{
	...
	ONYX_INLINE PlayerPet*	GetFirstPet(void) { return GetPlayerPet(GetFirstPetNumber()); }
	ONYX_INLINE void			SetStableSlotCount(uint8 count) { m_StableSlotCount = count; }
	ONYX_INLINE uint8			GetStableSlotCount(void) { return m_StableSlotCount; }
	...
	//ONYX_INLINE std::list<struct skilllines>getSkillLines() { return m_skilllines; }
	...
	ONYX_INLINE float GetBlockFromSpell() { return m_blockfromspell; }
	ONYX_INLINE float GetSpellCritFromSpell() { return m_spellcritfromspell; }
	ONYX_INLINE float GetHitFromMeleeSpell() { return m_hitfrommeleespell; }
	ONYX_INLINE float GetHitFromSpell() { return m_hitfromspell; }
	ONYX_INLINE float GetParryFromSpell() { return m_parryfromspell; }
	ONYX_INLINE float GetDodgeFromSpell() { return m_dodgefromspell; }
	...
	ONYX_INLINE uint32 GetHealthFromSpell() { return m_healthfromspell; }
	ONYX_INLINE uint32 GetManaFromSpell() { return m_manafromspell; }
	...
	ONYX_INLINE void SetTalentResetTimes(uint32 value) { m_talentresettimes = value; }
	void SetPlayerStatus(uint8 pStatus) { m_status = pStatus; }
	ONYX_INLINE uint8 GetPlayerStatus() { return m_status; }
	...
	ONYX_INLINE uint8 GetShapeShift()
	{
	...
	ONYX_INLINE bool IsVisible(Object* pObj) { return !(m_visibleObjects.find(pObj) == m_visibleObjects.end()); }
	void AddInRangeObject(Object* pObj);
	void OnRemoveInRangeObject(Object* pObj);
	void ClearInRangeSet();
	ONYX_INLINE void AddVisibleObject(Object* pObj) { m_visibleObjects.insert(pObj); }
	ONYX_INLINE void RemoveVisibleObject(Object* pObj) { m_visibleObjects.erase(pObj); }
	ONYX_INLINE void RemoveVisibleObject(InRangeSet::iterator itr) { m_visibleObjects.erase(itr); }
	ONYX_INLINE InRangeSet::iterator FindVisible(Object * obj) { return m_visibleObjects.find(obj); }
	ONYX_INLINE void RemoveIfVisible(Object * obj)
	{
	...
	ONYX_INLINE bool GetVisibility(Object * obj, InRangeSet::iterator *itr)
	{
		*itr = m_visibleObjects.find(obj);
		return ((*itr) != m_visibleObjects.end());
	}

	ONYX_INLINE InRangeSet::iterator GetVisibleSetBegin() { return m_visibleObjects.begin(); }
	ONYX_INLINE InRangeSet::iterator GetVisibleSetEnd() { return m_visibleObjects.end(); }
	...
	ONYX_INLINE float res_M_crit_get(){return m_resist_critical[0];}
	ONYX_INLINE void res_M_crit_set(float newvalue){m_resist_critical[0]=newvalue;}
	ONYX_INLINE float res_R_crit_get(){return m_resist_critical[1];}
	ONYX_INLINE void res_R_crit_set(float newvalue){m_resist_critical[1]=newvalue;}
...
*/
	ONYX_INLINE uint32* GetPlayedtime() { return m_playedtime; };
	...
	ONYX_INLINE uint32 GetAreaID() { return m_AreaID; }
	void SetAreaID(uint32 area) { m_AreaID = area; m_areaDBC = dbcArea.LookupEntryForced(m_AreaID); }
	ONYX_INLINE AreaTable *GetAreaDBC() { return m_areaDBC; }
	...
	ONYX_INLINE void setMyCorpse(Corpse * corpse) { myCorpse = corpse; }
	ONYX_INLINE Corpse * getMyCorpse() { return myCorpse; }
	...
	******************/
	...
	//! Is PVP flagged?
	ONYX_INLINE bool IsPvPFlagged() { return HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP); }
	ONYX_INLINE void SetPvPFlag()
	{
	...
	//! Removal
	ONYX_INLINE void RemovePvPFlag()
	{
		StopPvPTimer();
		RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP);
	}
	//! Do this on /pvp off
	ONYX_INLINE void ResetPvPTimer();
	//! Stop the timer for pvp off
	ONYX_INLINE void StopPvPTimer() { m_pvpTimer = 0; }
	...
	ONYX_INLINE uint32 LastHonorResetTime() const { return m_lastHonorResetTime; }
	ONYX_INLINE void LastHonorResetTime(uint32 val) { m_lastHonorResetTime = val; }
	uint32 OnlineTime;
	bool tutorialsDirty;
	LevelInfo * lvlinfo;
	...
	ONYX_INLINE void SetName(string& name) { m_name = name; }
	...
	ONYX_INLINE void NullComboPoints() { if(!m_retainComboPoints) { m_comboTarget = 0; m_comboPoints = 0; m_spellcomboPoints=0; } UpdateComboPoints(); }
	...
	ONYX_INLINE Player * GetTradeTarget()
	{
	...
	/* Last Speeds */
	ONYX_INLINE void UpdateLastSpeeds()
	{
	...
	ONYX_INLINE bool IsAttacking() {return m_attacking; }
...
public:
	...
	ONYX_INLINE bool HasKnownTitle( int32 title )
	{
	...
	void SetKnownTitle( int32 title, bool add );
	void SetChosenTitle( int32 title )
	{
		SetUInt32Value( PLAYER_CHOSEN_TITLE, title );
		if(title <= TITLE_HIGH_WARLORD)
		{
			if(title > TITLE_GRAND_MARSHAL)
				title -= TITLE_GRAND_MARSHAL;
			SetPVPRank(title);
		}
	}
...
public:
	ONYX_INLINE const uint8 *GetTalentInspectBuffer() { return m_talentInspectBuffer; }
