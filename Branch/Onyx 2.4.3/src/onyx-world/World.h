...
public:
	...
	ONYX_INLINE size_t GetSessionCount() const { return m_sessions.size(); }
	uint32 GetNonGmSessionCount();
	ONYX_INLINE size_t GetQueueCount() { return mQueuedSessions.size(); }
	void GetStats(uint32 * GMCount, float * AverageLatency);

	ONYX_INLINE uint32 GetPlayerLimit() const { return m_playerLimit; }
	void SetPlayerLimit(uint32 limit) { m_playerLimit = limit; }

	ONYX_INLINE bool getAllowMovement() const { return m_allowMovement; }
	void SetAllowMovement(bool allow) { m_allowMovement = allow; }
	ONYX_INLINE bool getGMTicketStatus() { return m_gmTicketSystem; };
	...
	ONYX_INLINE std::string getGmClientChannel() { return GmClientChannel; }

	void SetMotd(const char *motd) { m_motd = motd; }
	ONYX_INLINE const char* GetMotd() const { return m_motd.c_str(); }

	ONYX_INLINE time_t GetGameTime() const { return m_gameTime; }
	...
	ONYX_INLINE void SetStartTime(uint32 val) { m_StartTime = val; }
	ONYX_INLINE uint32 GetUptime(void) { return (uint32)UNIXTIME - m_StartTime; }
	ONYX_INLINE uint32 GetStartTime(void) { return m_StartTime; }
	...
	ONYX_INLINE void setRate(int index,float value)
	{
		regen_values[index]=value;
	}

	ONYX_INLINE float getRate(int index)
	{
		return regen_values[index];
	}
	
	ONYX_INLINE uint32 getIntRate(int index)
	{
		return int_rates[index];
	}

	ONYX_INLINE void setIntRate(int index, uint32 value)
	{
	...
	ONYX_INLINE uint32 GetTimeOut(){return TimeOut;}
