...
public:
	...
	ONYX_INLINE GroupMembersSet::iterator GetGroupMembersBegin(void) { return m_GroupMembers.begin(); }
	ONYX_INLINE GroupMembersSet::iterator GetGroupMembersEnd(void)   { return m_GroupMembers.end();   }

	bool AddPlayer(PlayerInfo * info);
	void RemovePlayer(PlayerInfo * info);
	
	ONYX_INLINE bool IsFull(void)				{ return m_GroupMembers.size() >= MAX_GROUP_SIZE_PARTY; }
	ONYX_INLINE size_t GetMemberCount(void)		{ return m_GroupMembers.size(); }
	
	ONYX_INLINE uint32 GetID(void)			   { return m_Id; }
	ONYX_INLINE void SetID(uint32 newid)		 { m_Id = newid; }

	ONYX_INLINE void   SetParent(Group* parent)  { m_Parent = parent; }
	ONYX_INLINE Group* GetParent(void)		   { return m_Parent; }
...
public:
	...
	ONYX_INLINE void SendPacketToAll(WorldPacket *packet) { SendPacketToAllButOne(packet, NULL); }
	void SendPacketToAllButOne(WorldPacket *packet, Player *pSkipTarget);

	ONYX_INLINE void SendPacketToAll(StackPacket *packet) { SendPacketToAllButOne(packet, NULL); }
	void SendPacketToAllButOne(StackPacket *packet, Player *pSkipTarget);

	ONYX_INLINE void OutPacketToAll(uint16 op, uint16 len, const void* data) { OutPacketToAllButOne(op, len, data, NULL); }
	...
	// Accessing functions
	ONYX_INLINE SubGroup* GetSubGroup(uint32 Id)
	{
		if(Id >= 8)
			return 0;
	...
	ONYX_INLINE uint32 GetSubGroupCount(void) { return m_SubGroupCount; }

	ONYX_INLINE uint8 GetMethod(void) { return m_LootMethod; }
	ONYX_INLINE uint16 GetThreshold(void) { return m_LootThreshold; }
	ONYX_INLINE PlayerInfo* GetLeader(void) { return m_Leader; }
	ONYX_INLINE PlayerInfo* GetLooter(void) { return m_Looter; }

	void MovePlayer(PlayerInfo* info, uint8 subgroup);

	bool HasMember(Player *pPlayer);
	bool HasMember(PlayerInfo * info);
	ONYX_INLINE uint32 MemberCount(void) { return m_MemberCount; }
	ONYX_INLINE bool IsFull() { return ((m_GroupType == GROUP_TYPE_PARTY && m_MemberCount >= MAX_GROUP_SIZE_PARTY) || (m_GroupType == GROUP_TYPE_RAID && m_MemberCount >= MAX_GROUP_SIZE_RAID)); }
	...
	ONYX_INLINE uint8 GetGroupType() { return m_GroupType; }
	ONYX_INLINE uint32 GetID() { return m_Id; }
	...
	ONYX_INLINE Mutex& getLock() { return m_groupLock; }
	ONYX_INLINE void Lock() { m_groupLock.Acquire(); }
	ONYX_INLINE void Unlock() { return m_groupLock.Release(); }
	bool m_isqueued;
	...
	ONYX_INLINE PlayerInfo * GetAssistantLeader() { return m_assistantLeader; }
	ONYX_INLINE PlayerInfo * GetMainTank() { return m_mainTank; }
