...
pSpellAura SpellAuraHandler[TOTAL_SPELL_AURAS]={
	...
	&Aura::SpellAuraModAOEDodge,//missing = 160 //Apply Aura: Mod Side/Rear PBAE Damage Taken %//used http://www.thottbot.com/?sp=23198
...
/*
ONYX_INLINE void ApplyFloatSM(float ** m,float v,uint32 mask, float def)
{
	if(*m == 0)
	{
...
/*
ONYX_INLINE void ApplyFloatPSM(float ** m,int32 v,uint32 mask, float def)
{
	if(*m == 0)
	{
		*m = new float[SPELL_GROUPS];
...
{
	if( m_target->IsPlayer() && mechanic < 27 && GetDuration() > 0 )
		SetDuration( int32(float(GetDuration()) * TO_PLAYER(m_target)->MechanicDurationPctMod[mechanic]));
}
...
void Aura::SpellAuraModAOEDodge(bool apply)
{
	if( m_target == NULL )
		return;
	...
	if( apply )
	{
		m_target->AreaOfAffectDodgeMod += mod->m_amount / 100.0f;
	}
	else
		m_target->AreaOfAffectDodgeMod -= mod->m_amount / 100.0f;
}
