...
uint8 Spell::_DidHit(const Unit *target)
{
	...
	if(m_spellInfo->spell_coef_flags & SPELL_FLAG_AOE_SPELL)
		resistchance += u_victim->AreaOfAffectDodgeMod;

	if (m_spellInfo->DispelType < 10)
...
void Spell::Heal(int32 amount)
{
	...
	if( u_caster != NULL )
	{
		...
		amount *= int32(u_caster->HealDonePctMod[m_spellInfo->School]);
		amount *= int32(unitTarget->HealTakenPctMod[m_spellInfo->School]);
