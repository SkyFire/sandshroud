...
ONYX_INLINE std::string MyConvertIntToString(const int arg)
{
	stringstream out;
	out << arg;
	return out.str();
...
ONYX_INLINE std::string MyConvertFloatToString(const float arg)
{
	stringstream out;
	out << arg;
	return out.str();
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					{
						if(plr->GetItemInterface()->GetItemCount(qst->srcitem) < 1)
						{
