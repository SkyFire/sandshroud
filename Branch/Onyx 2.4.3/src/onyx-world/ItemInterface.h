...
public:
	...
	//buyback stuff
	ONYX_INLINE Item* GetBuyBack(int32 slot) 
	{ 
		if(slot >= 0 && slot <= 12)
...
public:
	ONYX_INLINE bool VerifyBagSlots(int8 ContainerSlot, int8 Slot)
	{
	...
	ONYX_INLINE bool VerifyBagSlotsWithBank(int8 ContainerSlot, int8 Slot)
	{
		if( ContainerSlot < -1 || Slot < 0 )
			return false;
	...
	ONYX_INLINE bool VerifyBagSlotsWithInv(int8 ContainerSlot, int8 Slot)
	{
		if( ContainerSlot < -1 || Slot < 0 )
			return false;
...
public:
	...
	ONYX_INLINE Item* Grab() { return m_currentItem; }
	ONYX_INLINE bool End() { return m_atEnd; }
};
