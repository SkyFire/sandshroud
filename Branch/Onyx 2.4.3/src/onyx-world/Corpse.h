...
public:
   ...
   // void Create();
	...
	void SaveToDB();
	void DeleteFromDB();
	ONYX_INLINE void SetCorpseState(uint32 state) { m_state = state; }
	ONYX_INLINE uint32 GetCorpseState() { return m_state; }
	void Despawn();
	
	ONYX_INLINE void SetLoadedFromDB(bool value) { _loadedfromdb = value; }
	ONYX_INLINE bool GetLoadedFromDB(void) { return _loadedfromdb; }
