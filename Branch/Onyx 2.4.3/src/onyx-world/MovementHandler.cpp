...
void WorldSession::HandleMovementOpcodes( WorldPacket & recv_data )
{
	...
	else
	{
		if( recv_data.GetOpcode() == MSG_MOVE_FALL_LAND )
		{
			...
			if( _player->isAlive() && !_player->HasNoFallDamageAura() && !_player->GodModeCheat && falldistance > 12 && ( UNIXTIME >= _player->m_fallDisabledUntil ) && movement_info.FallTime > 1000 )
			{
				...
				uint32 manashield = _player->ManaShieldAbsorb(health_loss); // Yay, mana shield bullcrap.

				if(manashield)
					health_loss -= manashield;
