...
public:
  ...
  //! True if object exists in world
	...
	ONYX_INLINE bool IsInWorld() { return m_mapMgr != NULL; }
	...
	ONYX_INLINE const uint64& GetGUID() const { return *((uint64*)m_uint32Values); }
	ONYX_INLINE const WoWGuid& GetNewGUID() const { return m_wowGuid; }
	ONYX_INLINE uint32 GetEntry(){return m_uint32Values[OBJECT_FIELD_ENTRY];}
	
	ONYX_INLINE const uint32 GetEntryFromGUID() const	{ return uint32( (*(uint64*)m_uint32Values >> 24) & 0xFFFFFFFF ); }
	ONYX_INLINE const uint32 GetTypeFromGUID() const { return (m_uint32Values[1] & HIGHGUID_TYPE_MASK); }
	ONYX_INLINE const uint32 GetUIdFromGUID() const { return (m_uint32Values[0] & LOWGUID_ENTRY_MASK); }
	ONYX_INLINE const uint32 GetLowGUID() const { return (m_uint32Values[0]); }

	// type
	ONYX_INLINE const uint8& GetTypeId() const { return m_objectTypeId; }
	ONYX_INLINE bool IsUnit()	{ return ( m_objectTypeId == TYPEID_UNIT || m_objectTypeId == TYPEID_PLAYER ); }
	ONYX_INLINE bool IsPlayer() { return m_objectTypeId == TYPEID_PLAYER; }
	ONYX_INLINE bool IsCreature() { return m_objectTypeId == TYPEID_UNIT; }
	bool IsPet();
	...
	ONYX_INLINE const float& GetPositionX( ) const { return m_position.x; }
	ONYX_INLINE const float& GetPositionY( ) const { return m_position.y; }
	ONYX_INLINE const float& GetPositionZ( ) const { return m_position.z; }
	ONYX_INLINE const float& GetOrientation( ) const { return m_position.o; }
	ONYX_INLINE void SetOrientation( float &o ) { m_position.o = o; }

	ONYX_INLINE const float& GetSpawnX( ) const { return m_spawnLocation.x; }
	ONYX_INLINE const float& GetSpawnY( ) const { return m_spawnLocation.y; }
	ONYX_INLINE const float& GetSpawnZ( ) const { return m_spawnLocation.z; }
	ONYX_INLINE const float& GetSpawnO( ) const { return m_spawnLocation.o; }
	ONYX_INLINE void SetSpawnX(float x) { m_spawnLocation.x = x; }
	ONYX_INLINE void SetSpawnY(float y) { m_spawnLocation.y = y; }
	ONYX_INLINE void SetSpawnZ(float z) { m_spawnLocation.z = z; }
	ONYX_INLINE void SetSpawnO(float o) { m_spawnLocation.o = o; }

	ONYX_INLINE const LocationVector & GetPosition() { return m_position; }
	ONYX_INLINE LocationVector & GetPositionNC() { return m_position; }
	ONYX_INLINE LocationVector * GetPositionV() { return &m_position; }
	...
	//! Only for MapMgr use
	ONYX_INLINE MapCell* GetMapCell() const { return m_mapCell; }
	//! Only for MapMgr use
	ONYX_INLINE void SetMapCell(MapCell* cell) { m_mapCell = cell; }
	//! Only for MapMgr use
	ONYX_INLINE MapMgr* GetMapMgr() const { return m_mapMgr; }

	ONYX_INLINE void SetMapId(uint32 newMap) { m_mapId = newMap; }
	void SetZoneId(uint32 newZone);

	ONYX_INLINE const uint32 GetMapId( ) const { return m_mapId; }
	ONYX_INLINE const uint32& GetZoneId( ) const { return m_zoneId; }

	//! Get uint32 property
	ONYX_INLINE const uint32& GetUInt32Value( uint32 index ) const { return m_uint32Values[ index ]; }

	//! Get uint64 property
	ONYX_INLINE const uint64& GetUInt64Value( uint32 index ) const { return *((uint64*)&(m_uint32Values[ index ])); }

	//! Get float property
	ONYX_INLINE const float& GetFloatValue( uint32 index ) const { return m_floatValues[ index ]; }
	...
	ONYX_INLINE uint8 GetByte(uint32 i,uint32 i1) { return ((uint8*)m_uint32Values)[i*4+i1]; }
	
	ONYX_INLINE void SetNewGuid(uint32 Guid)
	{
	...
	ONYX_INLINE bool HasFlag( const uint32 index, uint32 flag ) const { return (m_uint32Values[ index ] & flag) != 0;	}
	...
	ONYX_INLINE const float GetDistanceSq(Object* obj)
	{
		if(obj->GetMapId() != m_mapId) return 40000.0f; //enough for out of range
		return m_position.DistanceSq(obj->GetPosition());
	}

	ONYX_INLINE float GetDistanceSq(LocationVector & comp)
	{
		return comp.DistanceSq(m_position);
	}

	ONYX_INLINE float CalcDistance(LocationVector & comp)
	{
		return comp.Distance(m_position);
	}

	ONYX_INLINE const float GetDistanceSq(float x, float y, float z)
	{
		return m_position.DistanceSq(x, y, z);
	}

	ONYX_INLINE const float GetDistance2dSq( Object* obj )
	{
	...
	ONYX_INLINE bool IsInRangeSet( Object* pObj )
	{
	...
	ONYX_INLINE void RemoveInRangeObject( Object* pObj )
	{
		if( pObj == NULL )
			return;
	...
	ONYX_INLINE bool HasInRangeObjects()
	{
	...
	ONYX_INLINE size_t GetInRangeCount() { return m_objectsInRange.size(); }
	ONYX_INLINE size_t GetInRangePlayersCount() { return m_inRangePlayers.size();}
	ONYX_INLINE InRangeSet::iterator GetInRangeSetBegin() { return m_objectsInRange.begin(); }
	ONYX_INLINE InRangeSet::iterator GetInRangeSetEnd() { return m_objectsInRange.end(); }
	ONYX_INLINE InRangeSet::iterator FindInRangeSet(Object * obj) { return m_objectsInRange.find(obj); }
	...
	ONYX_INLINE bool RemoveIfInRange( Object * obj )
	{
	...
	ONYX_INLINE void AddInRangePlayer( Object * obj )
	{
		m_inRangePlayers.insert( reinterpret_cast< Player* >( obj ) );
	}

	ONYX_INLINE void RemoveInRangePlayer( Object * obj )
	{
		m_inRangePlayers.erase( reinterpret_cast< Player* >( obj ) );
	}
	...
	ONYX_INLINE std::set<Object*>::iterator GetInRangeOppFactsSetBegin() { return m_oppFactsInRange.begin(); }
	ONYX_INLINE std::set<Object*>::iterator GetInRangeOppFactsSetEnd() { return m_oppFactsInRange.end(); }
	ONYX_INLINE std::set<Player*>::iterator GetInRangePlayerSetBegin() { return m_inRangePlayers.begin(); }
	ONYX_INLINE std::set<Player*>::iterator GetInRangePlayerSetEnd() { return m_inRangePlayers.end(); }
	ONYX_INLINE std::set<Player*> * GetInRangePlayerSet() { return &m_inRangePlayers; };

	void __fastcall SendMessageToSet(WorldPacket *data, bool self,bool myteam_only=false);
	ONYX_INLINE void SendMessageToSet(StackPacket * data, bool self) { OutPacketToSet(data->GetOpcode(), (uint16)data->GetSize(), data->GetBufferPointer(), self); }
	...
	ONYX_INLINE uint16 GetValuesCount() const { return m_valuesCount; }
	...
	ONYX_INLINE void SetInstanceID(int32 instance) { m_instanceId = instance; }
	ONYX_INLINE int32 GetInstanceID() { return m_instanceId; }
	...
	bool m_inQueue;
	ONYX_INLINE void SetMapMgr(MapMgr * mgr) { m_mapMgr = mgr; }
	...
	ONYX_INLINE size_t GetInRangeOppFactCount() { return m_oppFactsInRange.size(); }
...
public:
	...
	ONYX_INLINE uint32 GetAgility() { return m_uint32Values[UNIT_FIELD_AGILITY]; }
	ONYX_INLINE uint32 GetStamina() { return m_uint32Values[UNIT_FIELD_STAMINA]; }
	ONYX_INLINE uint32 GetIntellect() { return m_uint32Values[UNIT_FIELD_INTELLECT]; }
	ONYX_INLINE uint32 GetSpirit() { return m_uint32Values[UNIT_FIELD_SPIRIT]; }

	// Health
	ONYX_INLINE uint32 GetHealth() { return m_uint32Values[UNIT_FIELD_HEALTH]; }
