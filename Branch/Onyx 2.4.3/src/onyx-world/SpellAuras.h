...
public:
    ...
    ONYX_INLINE SpellEntry* GetSpellProto() const { return m_spellProto; }
    ONYX_INLINE uint32 GetSpellId() const {return m_spellProto->Id; }
    ONYX_INLINE bool IsPassive(){ if(!m_spellProto) return false; return (m_spellProto->Attributes & ATTRIBUTES_PASSIVE && !m_areaAura);}

    ONYX_INLINE int32 GetDuration() const { return m_duration; }
    void SetDuration(int32 duration) { m_duration = duration; }
	void UpdateAuraDuration(uint32 new_duration);

	ONYX_INLINE uint32 GetModCount() const { return m_modcount; }
	ONYX_INLINE Modifier *GetMod(uint32 x) { return &m_modList[x]; }

    ONYX_INLINE uint8 GetAuraSlot() const { return m_auraSlot; }
	void SetAuraSlot(uint8 slot) { m_auraSlot = slot; }

	ONYX_INLINE bool IsPositive() { return m_positive>0; }
	void SetNegative(signed char value=1) { m_positive -= value; }
	void SetPositive(signed char value=1) { m_positive += value; }

	Object* GetCaster();
	ONYX_INLINE uint64 GetCasterGUID(){return m_casterGuid;}
	Unit* GetUnitCaster();
	ONYX_INLINE Unit* GetTarget() { return m_target; }
	...
	ONYX_INLINE uint32 GetTimeLeft()//in sec
	{
	...
	void SpellAuraModAOEDodge(bool apply);
	...
	ONYX_INLINE void SendPeriodicHealAuraLog(uint32 amt)
	{
		WorldPacket data(32);
	...
	// log message's
	ONYX_INLINE void SendPeriodicAuraLog(Unit * Caster, Unit * Target, uint32 SpellID, uint32 School, uint32 Amount, uint32 abs_dmg, uint32 resisted_damage, uint32 Flags)
	{
	...
	ONYX_INLINE bool IsCombatStateAffecting()
	{
	...
	ONYX_INLINE bool HasMechanic(uint32 MechanicsType)
	{
		return Spell::HasMechanic(m_spellProto, MechanicsType);
	}

	ONYX_INLINE uint32 GetMechanic()
	{
		return Spell::GetMechanic(m_spellProto);
	}

	ONYX_INLINE uint32 GetMechanicOfEffect(uint32 i)
	{
...
private:
	...
	ONYX_INLINE void DurationPctMod(uint32 mechanic);

	ONYX_INLINE bool IsInrange(float x1,float y1, float z1, Object * o,float square_r)
	{
		float t;
		float r;
		t=x1-o->GetPositionX();
...
public:
	...
	ONYX_INLINE bool IsInterrupted() { return ( m_interrupted >= 0 ); }
};
