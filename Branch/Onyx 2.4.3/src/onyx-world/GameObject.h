...
/*	typedef struct { const char *name; int(*mfunc)(lua_State*,GameObject*); } RegType;
	...
	ONYX_INLINE GameObjectInfo* GetInfo() { return pInfo; }
	ONYX_INLINE void SetInfo(GameObjectInfo * goi) { pInfo = goi; }
   ...
   // void Create ( uint32 mapid, float x, float y, float z, float ang);
	...
	ONYX_INLINE GameObjectAIScript* GetScript() { return myScript; }

	void TrapSearchTarget();	// Traps need to find targets faster :P

	ONYX_INLINE bool HasAI() { return spell != 0; }
	GOSpawn * m_spawn;
	void OnPushToWorld();
	void OnRemoveInRangeObject(Object* pObj);
	void RemoveFromWorld(bool free_guid);

	ONYX_INLINE bool CanMine(){return (mines_remaining > 0);}
	ONYX_INLINE void UseMine(){ if(mines_remaining) mines_remaining--;}
	...
	ONYX_INLINE void SetScript(GameObjectAIScript *pScript) { myScript = pScript; }
