...
/////////////////////////////////////////////
	...
	ONYX_INLINE uint32 GenerateGameobjectGuid()
	{
		m_GOHighGuid &= 0x00FFFFFF;
		return ++m_GOHighGuid;
	}

	ONYX_INLINE GameObject * GetGameObject(uint32 guid)
	{
...
////////////////////////////////////////////
	...
	ONYX_INLINE DynamicObject * GetDynamicObject(uint32 guid)
	{
...
///////////////////////////////////
	...
	// Terrain Functions
	ONYX_INLINE float  GetLandHeight(float x, float y) { return GetBaseMap()->GetLandHeight(x, y); }
	ONYX_INLINE float  GetWaterHeight(float x, float y) { return GetBaseMap()->GetWaterHeight(x, y); }
	ONYX_INLINE uint8  GetWaterType(float x, float y) { return GetBaseMap()->GetWaterType(x, y); }
	ONYX_INLINE uint8  GetWalkableState(float x, float y) { return GetBaseMap()->GetWalkableState(x, y); }
	ONYX_INLINE uint16 GetAreaID(float x, float y) { return GetBaseMap()->GetAreaID(x, y); }

	ONYX_INLINE uint32 GetMapId() { return _mapId; }

	void PushToProcessed(Player* plr);

	ONYX_INLINE bool HasPlayers() { return (m_PlayerStorage.size() > 0); }
	ONYX_INLINE bool IsCombatInProgress() { return (_combatProgress.size() > 0); }
	void TeleportPlayers();

	ONYX_INLINE uint32 GetInstanceID() { return m_instanceID; }
	ONYX_INLINE MapInfo *GetMapInfo() { return pMapInfo; }

	ONYX_INLINE MapScriptInterface * GetInterface() { return ScriptInterface; }
	virtual int32 event_GetInstanceID() { return m_instanceID; }

	void LoadAllCells();
	ONYX_INLINE size_t GetPlayerCount() { return m_PlayerStorage.size(); }
...
public:
	...
	// get!
	ONYX_INLINE WorldStateManager& GetStateManager() { return m_stateManager; }
