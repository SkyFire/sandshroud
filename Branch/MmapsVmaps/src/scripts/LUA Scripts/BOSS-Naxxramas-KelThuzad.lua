...
function kt_OnCombat(Unit, Event)
	...
	local fbtime=math.random(15000,25000)
		if agrochoice==1 then
			...
			Unit:SendChatMessage(CHAT_MSG_MONSTER_YELL,LANG_UNIVERSAL,"Pray For Mercy!")
		end
		if agrochoice==3 then
			Unit:PlaySoundToSet(8811)
			Unit:SendChatMessage(CHAT_MSG_MONSTER_YELL,LANG_UNIVERSAL,"The end is Upon you!")
		end
		if agrochoice==2 then
			Unit:PlaySoundToSet(8810)
			Unit:SendChatMessage(CHAT_MSG_MONSTER_YELL,LANG_UNIVERSAL,"Scream your dying breath!")
		end
...
function kt_OnKilledTarget(Unit)
	local KilledChoice = math.random(1,2)
		if KilledChoice==1 then
			...
			Unit:SendChatMessage(CHAT_MSG_MONSTER_YELL,LANG_UNIVERSAL,"The dark void awaits	you!")
		else
...
function kt_OnDied(Unit)
	...
	Unit:SendChatMessage(CHAT_MSG_MONSTER_YELL,LANG_UNIVERSAL,"Do not rejoice... your victory is a hollow one... for I shall return	with powers beyond your	imagining!")
	Unit:RemoveEvents()	
end
...
function Fissure(pUnit)
	...
	pUnit:SpawnCreature(16129,plr:GetX(),plr:GetY(),plr:GetZ(),plr:GetO(),14,10000)
end
...
function VoidBlast(Unit, event)
	Unit:SendChatMessage(CHAT_MSG_MONSTER_YELL,LANG_UNIVERSAL,"Void Blast")
	Unit:CastSpell(27812)
end

function FrostBlast(pUnit)
	pUnit:PlaySoundToSet(8815)
	pUnit:SendChatMessage(CHAT_MSG_MONSTER_YELL,LANG_UNIVERSAL,"I shall freeze the blood in your veins!")
...
function Phase3(Unit, Event)
		if Unit:GetHealthPct() < 41 then
			if phase3==0 then
				...
				Unit:SendChatMessage(CHAT_MSG_MONSTER_YELL,LANG_UNIVERSAL, "Master! I require aid!")
