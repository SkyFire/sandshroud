...
public:
	...
	ONYX_INLINE string GetNameString() { return name; }
	ONYX_INLINE const char* GetName() { return name.c_str(); }
	ONYX_INLINE MapEntry* GetDBCEntry() { return me; }

	ONYX_INLINE CellSpawns *GetSpawnsList(uint32 cellx,uint32 celly)
	{
		ASSERT(cellx < _sizeX);
		ASSERT(celly < _sizeY);
	...
	ONYX_INLINE CellSpawns * GetSpawnsListAndCreate(uint32 cellx, uint32 celly)
	{
		ASSERT(cellx < _sizeX);
		ASSERT(celly < _sizeY);
	...
	ONYX_INLINE float  GetLandHeight(float x, float y)
	{ 
		if(_terrain)
		{
	...
	ONYX_INLINE float  GetWaterHeight(float x, float y) 
	{ 
		if(_terrain)
		{ 
	...
	ONYX_INLINE uint8  GetWaterType(float x, float y)
	{
		if(_terrain)
		{ 
	...
	ONYX_INLINE uint8  GetWalkableState(float x, float y)
	{
		if(_terrain)
		{ 
	...
	ONYX_INLINE uint16 GetAreaID(float x, float y)
	{
		if(_terrain)
		{ 
	...
	ONYX_INLINE void CellGoneActive(uint32 x, uint32 y)
	{ 
		if(_terrain)
		{ 
	...
	ONYX_INLINE void CellGoneIdle(uint32 x,uint32 y)
	{ 
		if(_terrain)
		{ 
