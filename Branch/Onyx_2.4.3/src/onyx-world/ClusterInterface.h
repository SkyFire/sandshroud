...
public:
	...
	ONYX_INLINE WorldSession * GetSession(uint32 sid) { return _sessions[sid]; }
	...
	ONYX_INLINE void QueuePacket(WorldPacket * pck) { _pckQueue.Push(pck); }

	void Update();
	void DestroySession(uint32 sid);

	ONYX_INLINE void SendPacket(WorldPacket * data) { if(_clientSocket) _clientSocket->SendPacket(data); }
	ONYX_INLINE void SetSocket(WSClient * s) { _clientSocket = s; }
