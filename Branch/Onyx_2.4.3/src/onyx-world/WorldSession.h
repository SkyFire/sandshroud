...
public:
	...
	ONYX_INLINE void SendPacket(WorldPacket* packet)
	{
		if(_socket && _socket->IsConnected())
			_socket->SendPacket(packet);
	}

	ONYX_INLINE void SendPacket(StackBufferBase * packet)
	{
		if(_socket && _socket->IsConnected())
			_socket->SendPacket(packet);
	}

	ONYX_INLINE void OutPacket(uint16 opcode)
	{
		if(_socket && _socket->IsConnected())
	...
	ONYX_INLINE uint32 GetAccountId() const { return _accountId; }
	ONYX_INLINE Player* GetPlayer() { return _player; }
	...
	ONYX_INLINE char* GetPermissions() { return permissions; }
	ONYX_INLINE int GetPermissionCount() { return permissioncount; }
	ONYX_INLINE bool HasPermissions() { return (permissioncount > 0) ? true : false; }
	ONYX_INLINE bool HasGMPermissions()
	{
		if(!permissioncount)
			return false;
	...
	ONYX_INLINE void SetSocket(WorldSocket *sock)
	{
		_socket = sock;
	}
	ONYX_INLINE void SetPlayer(Player *plr) { _player = plr; }
	
	ONYX_INLINE void SetAccountData(uint32 index, char* data, bool initial,uint32 sz)
	{
		ASSERT(index < 8);
	...
	ONYX_INLINE AccountDataEntry* GetAccountData(uint32 index)
	{
		ASSERT(index < 8);
	...
	ONYX_INLINE void QueuePacket(WorldPacket* packet)
	{
	...
	ONYX_INLINE WorldSocket* GetSocket() { return _socket; }
	...
	ONYX_INLINE void SetInstance(uint32 Instance) { instanceId = Instance; }
	ONYX_INLINE uint32 GetLatency() { return _latency; }
	ONYX_INLINE string GetAccountName() { return _accountName; }
	ONYX_INLINE const char * GetAccountNameS() { return _accountName.c_str(); }

	ONYX_INLINE uint32 GetClientBuild() { return client_build; }
	ONYX_INLINE void SetClientBuild(uint32 build) { client_build = build; }
	bool bDeleted;
	ONYX_INLINE uint32 GetInstance() { return instanceId; }
	Mutex deleteMutex;
	...
	ONYX_INLINE bool IsLoggingOut() { return _loggingOut; }
