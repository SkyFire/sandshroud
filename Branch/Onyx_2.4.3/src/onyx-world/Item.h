...
public:
	...
	ONYX_INLINE ItemPrototype* GetProto() const { return m_itemProto; }
	ONYX_INLINE void SetProto( ItemPrototype* pr ) { m_itemProto = pr; }

	ONYX_INLINE Player* GetOwner() const { return m_owner; }
	void SetOwner( Player* owner );

	ONYX_INLINE bool IsContainer(){ return ( m_objectTypeId == TYPEID_CONTAINER ) ? true : false; }
	...
	ONYX_INLINE time_t GetEnchantmentApplytime( uint32 slot )
	{
	...
	ONYX_INLINE void SetCount( uint32 amt ) { SetUInt32Value( ITEM_FIELD_STACK_COUNT, amt ); }
	ONYX_INLINE void SetDurability( uint32 Value ) { SetUInt32Value(ITEM_FIELD_DURABILITY, Value ); };
	ONYX_INLINE void SetDurabilityToMax() { SetUInt32Value( ITEM_FIELD_DURABILITY, GetUInt32Value( ITEM_FIELD_MAXDURABILITY ) ); }
	ONYX_INLINE uint32 GetDurability() { return GetUInt32Value( ITEM_FIELD_DURABILITY ); }
	ONYX_INLINE uint32 GetDurabilityMax() { return GetUInt32Value( ITEM_FIELD_MAXDURABILITY ); }
	ONYX_INLINE bool IsAmmoBag() { return (m_itemProto->Class == ITEM_CLASS_QUIVER); }
	...
	ONYX_INLINE uint32 GetItemRandomSuffixFactor() { return m_uint32Values[ITEM_FIELD_PROPERTY_SEED]; }
	static uint32 GenerateRandomSuffixFactor( ItemPrototype* m_itemProto );

	ONYX_INLINE void SetRandomProperty( uint32 id )
	{
		SetUInt32Value( ITEM_FIELD_RANDOM_PROPERTIES_ID, id );
		random_prop = id;
	}

	ONYX_INLINE void SetRandomSuffix( uint32 id )
	{
		int32 r_id = -(int32(id));
