...
public:
	...
	ONYX_INLINE uint32 GetBlockCount() const {return mBlocks;}
	
	ONYX_INLINE uint32 GetLength() const { return (mBlocks *sizeof(uint32)); }
	ONYX_INLINE uint32 GetCount() const { return mCount; }
	ONYX_INLINE const uint8* GetMask() const { return (uint8*)mUpdateMask; }
