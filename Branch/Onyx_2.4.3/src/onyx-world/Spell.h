...
enum SpellIsFlags
{
	...
	SPELL_FLAG_IS_FORCEDDEBUFF			= 0x00001000,
	SPELL_FLAG_IS_FORCEDBUFF			= 0x00002000,
};
...
ONYX_INLINE bool CanAgroHash(uint32 spellhashname)
{
...
/************************************************************************/
ONYX_INLINE bool IsDamagingSpell(SpellEntry *sp)
{
    switch (sp->Effect[0])
    {
...
ONYX_INLINE bool IsHealingSpell(SpellEntry *sp)
{
    switch( sp->Effect[0] )
    {
        case SPELL_EFFECT_HEALTH_LEECH:
...
ONYX_INLINE bool IsInrange(LocationVector & location, Object * o, float square_r)
{
...
ONYX_INLINE bool IsInrange(float x1,float y1, float z1,float x2,float y2, float z2,float square_r)
{
    float t;
    float r;
    t=x1-x2;
...
ONYX_INLINE bool IsInrange(Object * o1,Object * o2,float square_r)
{
    return IsInrange(o1->GetPositionX(),o1->GetPositionY(),o1->GetPositionZ(),
        o2->GetPositionX(),o2->GetPositionY(),o2->GetPositionZ(),square_r);
}

ONYX_INLINE bool TargetTypeCheck(Object *obj,uint32 ReqCreatureTypeMask)
{
...
ONYX_INLINE bool IsTargetingStealthed(SpellEntry *sp)
{
	if(
...
public:
    ...
    ONYX_INLINE uint32 getState() { return m_spellState; }
    ONYX_INLINE void SetUnitTarget(Unit *punit){unitTarget=punit;}
	...
	ONYX_INLINE uint32 GetType() { return ( m_spellInfo->Spell_Dmg_Type == SPELL_DMG_TYPE_NONE ? SPELL_DMG_TYPE_MAGIC : m_spellInfo->Spell_Dmg_Type ); }

    std::vector<uint64> UniqueTargets;
    SpellTargetsList    ModeratedTargets;

    ONYX_INLINE Item* GetItemTarget() { return itemTarget; }
    ONYX_INLINE Unit* GetUnitTarget() { return unitTarget; }
    ONYX_INLINE Player* GetPlayerTarget() { return playerTarget; }
    ONYX_INLINE GameObject* GetGameObjectTarget() { return gameObjTarget; }
    ...
    ONYX_INLINE float GetRadius(uint32 i)
    {
    ...
    ONYX_INLINE static uint32 GetBaseThreat(uint32 dmg)
    {
...
/*        switch(m_spellInfo->NameHash)
    ...
    /// Spell state's
    /// Spell failed
    ONYX_INLINE bool GetSpellFailed(){return m_Spell_Failed;}
    ONYX_INLINE void SetSpellFailed(bool failed = true){m_Spell_Failed = failed;}

    ONYX_INLINE bool IsReflected() {return m_IsReflected;}
    ONYX_INLINE void SetReflected(bool reflected = true) {m_IsReflected = reflected;}
    
    /// Spell possibility's
    ONYX_INLINE bool GetCanReflect() {return m_CanRelect;}
    ONYX_INLINE void SetCanReflect(bool reflect = true) {m_CanRelect = reflect;}
...
protected:
	...
	ONYX_INLINE bool HasTarget(const uint64& guid, TargetsList* tmpMap)
	{
