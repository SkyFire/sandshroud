...
#else
	ONYX_INLINE const uint64 GetGUID() const { return GetUInt64Value(0); }
#endif

	ONYX_INLINE const WoWGuid& GetNewGUID() const { return m_wowGuid; }
	ONYX_INLINE uint32 GetEntry(){return m_uint32Values[3];}
	
	ONYX_INLINE const uint32 GetEntryFromGUID() const
	{
/*		uint64 entry = *(uint64*)m_uint32Values;
	...
	//! Get uint32 property
	ONYX_INLINE const uint32& GetUInt32Value( uint32 index ) const
	{
...
#else
	ONYX_INLINE const uint64& GetUInt64Value( uint32 index ) const
#endif
	{
	...
	//! Get float property
	ONYX_INLINE const float& GetFloatValue( uint32 index ) const
	{
		ASSERT( index < m_valuesCount );
	...
	ONYX_INLINE uint8 GetByte(uint32 i,uint32 i1)
	{
	...
	ONYX_INLINE bool HasFlag( const uint32 index, uint32 flag ) const
	{
		ASSERT( index < m_valuesCount );
	...
	ONYX_INLINE float GetDistance2dSq(float x, float y)
	{
	...
	ONYX_INLINE void SendMessageToSet(StackBufferBase * data, bool self) { OutPacketToSet(data->GetOpcode(), data->GetSize(), data->GetBufferPointer(), self); }
