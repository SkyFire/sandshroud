...
public:
	...
	ONYX_INLINE void SendPacket(WorldPacket* packet) { if(!packet) return; OutPacket(packet->GetOpcode(), packet->size(), (packet->size() ? (const void*)packet->contents() : NULL)); }
	ONYX_INLINE void SendPacket(StackBufferBase * packet) { if(!packet) return; OutPacket(packet->GetOpcode(), packet->GetSize(), (packet->GetSize() ? (const void*)packet->GetBufferPointer() : NULL)); }
	...
	ONYX_INLINE void SetSession(WorldSession * session) { mSession = session; }
	ONYX_INLINE WorldSession * GetSession() { return mSession; }
	bool Authed;
