...
public:
	...
	ONYX_INLINE uint32 GetXP(void) { return m_PetXP; }
	...
	ONYX_INLINE void SetPetAction(uint32 act) { m_Action = act; }
	ONYX_INLINE uint32 GetPetAction(void) { return m_Action; }

	ONYX_INLINE void SetPetState(uint32 state) { m_State = state; }
	ONYX_INLINE uint32 GetPetState(void) { return m_State; }

	ONYX_INLINE void SetPetDiet(uint32 diet) { m_Diet = diet; }
	ONYX_INLINE void SetPetDiet() { m_Diet = myFamily->petdietflags; }

	ONYX_INLINE uint32 GetPetDiet(void) { return m_Diet; }
	
	ONYX_INLINE AI_Spell* GetAISpellForSpellId(uint32 spellid)
	{
	...
	ONYX_INLINE Player* GetPetOwner() { return m_Owner; }
	ONYX_INLINE void ClearPetOwner() { m_Owner = 0; }
	...
	ONYX_INLINE void RemoveSpell(uint32 SpellID)
	{
		SpellEntry * sp = dbcSpell.LookupEntry(SpellID);
		if(sp) RemoveSpell(sp);
	}
	ONYX_INLINE void SetSpellState(uint32 SpellID, uint16 State)
	{
		SpellEntry * sp = dbcSpell.LookupEntry(SpellID);
		if(sp) SetSpellState(sp, State);
	}
	ONYX_INLINE uint16 GetSpellState(uint32 SpellID)
	{
		if(SpellID == 0)
	...
	ONYX_INLINE PetSpellMap* GetSpells() { return &mSpells; }
	ONYX_INLINE bool IsSummon() { return Summon; }

	void __fastcall SetAutoCastSpell(AI_Spell * sp);
	void Rename(string NewName);
	ONYX_INLINE string& GetName() { return m_name; }
