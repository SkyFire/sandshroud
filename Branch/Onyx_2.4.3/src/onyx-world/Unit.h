...
public:
	...
	ONYX_INLINE bool IsInCombat() { return m_lastStatus; }				// checks if we are in combat or not.

	void OnRemoveFromWorld();										// called when we are removed from world, kills all references to us.
	
	ONYX_INLINE void Vanished()
	{
		ClearAttackers();
		ClearHealers();
	}

	ONYX_INLINE const uint64& GetPrimaryAttackTarget() { return m_primaryAttackTarget; }
	ONYX_INLINE void SetUnit(Unit * p) { m_Unit = p; }
...
/*	typedef struct { const char *name; int(*mfunc)(lua_State*,Unit*); } RegType;
  ...
  //void StrikeWithAbility( Unit* pVictim, Spell* spell, uint32 addspelldmg, uint32 weapon_damage_type );
	...
	ONYX_INLINE void addStateFlag(uint32 f) { m_state |= f; };
	ONYX_INLINE bool hasStateFlag(uint32 f) { return (m_state & f ? true : false); }
	ONYX_INLINE void clearStateFlag(uint32 f) { m_state &= ~f; };

	/// Stats
	ONYX_INLINE uint32 getLevel() { return m_uint32Values[ UNIT_FIELD_LEVEL ]; };
	ONYX_INLINE uint8 getRace() { return GetByte(UNIT_FIELD_BYTES_0,0); }
	ONYX_INLINE uint8 getClass() { return GetByte(UNIT_FIELD_BYTES_0,1); }
	ONYX_INLINE void setRace(uint8 race) { SetByte(UNIT_FIELD_BYTES_0,0,race); }
	ONYX_INLINE void setClass(uint8 class_) { SetByte(UNIT_FIELD_BYTES_0,1, class_ ); }
	ONYX_INLINE uint32 getClassMask() { return 1 << (getClass() - 1); }
	ONYX_INLINE uint32 getRaceMask() { return 1 << (getRace() - 1); }
	ONYX_INLINE uint8 getGender() { return GetByte(UNIT_FIELD_BYTES_0,2); }
	ONYX_INLINE void setGender(uint8 gender) { SetByte(UNIT_FIELD_BYTES_0,2,gender); }
	ONYX_INLINE uint8 getStandState() { return ((uint8)m_uint32Values[UNIT_FIELD_BYTES_1]); }
...
//	void PeriodicAuraLog(Unit *pVictim, SpellEntry* spellID, uint32 damage, uint32 damageType);
	...
	void HandleProc(uint32 flag, Unit* Victim, SpellEntry* CastingSpell,uint32 dmg=-1,uint32 abs=0, uint32 weapon_damage_type=0);
...
//	void HandleProcSpellOnSpell(Unit* Victim,uint32 damage,bool critical);//nasty, some spells proc other spells
	...
	ONYX_INLINE void setPRegenTimer(uint32 time) {m_P_regenTimer = time; }
	ONYX_INLINE void DelayPowerRegeneration(uint32 time) { m_P_regenTimer = time; if (!m_interruptedRegenTime) m_interruptedRegenTime = 2000; }
	void DeMorph();
	...
	// Stealth  
	ONYX_INLINE int32 GetStealthLevel() { return m_stealthLevel; }
	ONYX_INLINE int32 GetStealthDetectBonus() { return m_stealthDetectBonus; }
	ONYX_INLINE void SetStealth(uint32 id) { m_stealth = id; }
	ONYX_INLINE bool IsStealth() { return (m_stealth!=0 ? true : false); }
	float detectRange;

	// Invisibility
	ONYX_INLINE void SetInvisibility(uint32 id) { m_invisibility = id; }
	ONYX_INLINE bool IsInvisible() { return (m_invisible!=0 ? true : false); }
	...
	/// Combat / Death Status
	ONYX_INLINE bool isAlive() { return m_deathState == ALIVE; };
	ONYX_INLINE bool isDead() { return  m_deathState !=ALIVE; };
...
//	std::map<uint32,ProcTriggerSpellOnSpellList> m_procSpellonSpell; //index is namehash
	...
	ONYX_INLINE void SetOnMeleeSpell(uint32 spell ) { m_meleespell = spell; }
	ONYX_INLINE uint32 GetOnMeleeSpell() { return m_meleespell; }
	...
	void WipeHateList();
	void WipeTargetList();
	ONYX_INLINE void setAItoUse(bool value){m_useAI = value;}
	...
	// DK:Affect
	ONYX_INLINE uint32 IsPacified() { return m_pacified; }
	ONYX_INLINE uint32 IsStunned() { return m_stunned; }
	ONYX_INLINE uint32 IsFeared() { return m_fearmodifiers; }
	ONYX_INLINE uint32 GetResistChanceMod() { return m_resistChance; }
	ONYX_INLINE void SetResistChanceMod(uint32 amount) { m_resistChance=amount; }
	
	ONYX_INLINE uint16 HasNoInterrupt() { return m_noInterrupt; }
	...
	void EmoteExpire();
	ONYX_INLINE void setEmoteState(uint8 emote) { m_emoteState = emote; };
	ONYX_INLINE uint32 GetOldEmote() { return m_oldEmote; }
	...
	ONYX_INLINE StandState GetStandState()
	{
	...
	ONYX_INLINE int GetManaPct() { return (int)(GetUInt32Value(UNIT_FIELD_POWER1) * 100 / GetUInt32Value(UNIT_FIELD_MAXPOWER1)); };
		
	uint32 GetResistance(uint32 type);	
	
	//Pet
	ONYX_INLINE void SetIsPet(bool chck) { m_isPet = chck; }
	...
	ONYX_INLINE Spell * GetCurrentSpell(){return m_currentSpell;}
	ONYX_INLINE void SetCurrentSpell(Spell* cSpell) { m_currentSpell = cSpell; }

	uint32 m_CombatUpdateTimer;

	ONYX_INLINE void setcanperry(bool newstatus){can_parry=newstatus;}
	...
	ONYX_INLINE uint32 GetPowerType(){ return (GetUInt32Value(UNIT_FIELD_BYTES_0)>> 24);}
	...
	ONYX_INLINE uint32 GetCharmTempVal() { return m_charmtemp; }
	ONYX_INLINE void SetCharmTempVal(uint32 val) { m_charmtemp = val; }
	set<uint32> m_SpellList;

	ONYX_INLINE void DisableAI() { m_useAI = false; }
	ONYX_INLINE void EnableAI() { m_useAI = true; }

	ONYX_INLINE void SetPowerType(uint8 type)
	{
		SetByte(UNIT_FIELD_BYTES_0,3,type);
	}

	ONYX_INLINE bool IsSpiritHealer()
	{
		switch(GetEntry())
		{
