...
struct SpellEntry
{
	...
	uint32 field114;                        //124
	uint32 dummy;                           //125
	uint32 CoSpell;                         //126   activeIconID;
...
ONYX_INLINE float GetRadius(SpellRadius *radius)
{
    return radius->Radius;
}
ONYX_INLINE uint32 GetCastTime(SpellCastTime *time)
{
    return time->CastTime;
}
ONYX_INLINE float GetMaxRange(SpellRange *range)
{
    return range->maxRange;
}
ONYX_INLINE float GetMinRange(SpellRange *range)
{
    return range->minRange;
}
ONYX_INLINE uint32 GetDuration(SpellDuration *dur)
{
