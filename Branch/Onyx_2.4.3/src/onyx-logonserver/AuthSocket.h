...
public:
	...
	ONYX_INLINE sAuthLogonChallenge_C * GetChallenge() { return &m_challenge; }
	ONYX_INLINE void SendPacket(const uint8* data, const uint16 len) { Send(data, len); }
	void OnDisconnect();
	ONYX_INLINE time_t GetLastRecv() { return last_recv; }
	bool removedFromSet;
	ONYX_INLINE uint32 GetAccountID() { return m_account ? m_account->AccountId : 0; }
