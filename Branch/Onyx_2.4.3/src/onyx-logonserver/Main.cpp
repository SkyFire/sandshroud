...
#define BANNER "Onyx %s r%u/%s-%s (%s) :: Logon Server"
...
void LogonServer::Run(int argc, char ** argv)
{
...
#ifdef WIN32
	char * config_file = "onyx-logonserver.conf";
#else
	char * config_file = (char*)CONFDIR "/onyx-logonserver.conf";
#endif
	...
	{
		Log.Success("Config", ">> onyx-logonserver.conf", config_file);
	}
	else
	{
		Log.Error("Config", ">> onyx-logonserver.conf", config_file);
		return;
	}
