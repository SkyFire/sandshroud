...
HEARTHSTONE_INLINE uint32 GetCurrencyId(std::string name)
{
	for(uint32 i = 0; i < dbcCurrencyTypesStore.GetMax(); ++i)
	{
		CurrencyTypesEntry* currency = dbcCurrencyTypesStore.LookupEntry(i);
		if(currency != NULL)
		{
			if(name.find(currency->name) != std::string::npos)
				return i;
		}
	}
	return 0;
