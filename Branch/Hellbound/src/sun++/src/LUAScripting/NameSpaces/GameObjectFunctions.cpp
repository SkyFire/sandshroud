...
namespace luaGameObject
{
	...
	int GossipCreateMenu(lua_State * L, GameObject *ptr)
	{
		...
		if(target->GetTypeId() != TYPEID_PLAYER) return 0;
		objmgr.CreateGossipMenuForPlayer(&Menu, ptr->GetGUID(), text_id, (Player *)target);
		if(autosend)
			Menu->SendTo((Player *)target);
		return 1;
	}
	...
	int GossipComplete(lua_State * L, GameObject *ptr)
	{
		Unit *target = Lunar<Unit>::check(L, 1);
		if(target->GetTypeId() != TYPEID_PLAYER) return 0;
		TO_PLAYER(target)->Gossip_Complete();
		return 0;
	}
	int GossipSendPOI(lua_State * L, GameObject *ptr)
	{
		...
		TO_PLAYER(target)->Gossip_SendPOI(x, y, icon, flags, data, name);
		return 0;
	}
	...
	int SetSpawnPosition(lua_State * L, GameObject *ptr)
	{
		...
		//static_ *_cast<Object>(ptr)->SetSpawnPosition(pvect);
		return 0;
	}
