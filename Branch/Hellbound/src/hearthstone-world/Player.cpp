	...
	if(result)
	{
		do 
		{
			...
			if(spProto)
				...
				CharacterDatabase.Execute("DELETE FROM playerspells WHERE spellid = %u", fields[0].GetInt32());
		} while(result->NextRow());
	}
}
...
void Player::_ApplyItemMods(Item* item, int16 slot, bool apply, bool justdrokedown /* = false */, bool skip_stat_apply /* = false  */)
{
	...
	if( proto->Damage.Min )
	{
		...
		{	
			BaseRangedDamage[0] += apply ? proto->Damage.Min : -proto->Damage.Min;
			BaseRangedDamage[1] += apply ? proto->Damage.Max : -proto->Damage.Max;
		}
		else
		{
			if( slot == EQUIPMENT_SLOT_OFFHAND )
			{
				BaseOffhandDamage[0] = apply ? proto->Damage.Min : 0;
				BaseOffhandDamage[1] = apply ? proto->Damage.Max : 0;
			}
			else
			{
				BaseDamage[0] = apply ? proto->Damage.Min : 0;
				BaseDamage[1] = apply ? proto->Damage.Max : 0;
			}
		}
	}
	...
	case DRUID:
		{
				...
				|| GetShapeShift() == FORM_BEAR || GetShapeShift() == FORM_DIREBEAR )
			{
				...
				if(it)
				{
					...
					if(ip)
					{
						float wpndmg = ((ip->Damage.Max + ip->Damage.Min)/2);
...
/*	ItemPrototype* proto = ItemPrototypeStorage.LookupEntry(itemId);
	...
	if(CurrencyTypesEntry const* ctEntry = dbcCurrencyTypesStore.LookupEntry(GetCurrencyId(proto->Name1)))
	{
		if(ctEntry)
		{
			if(apply)
