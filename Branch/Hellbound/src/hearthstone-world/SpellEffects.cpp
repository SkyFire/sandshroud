...
void Spell::SpellEffectSchoolDMG(uint32 i) // dmg school
{
	...
	if(m_spellInfo->EffectChainTarget[i])//chain
	{
		if( m_spellInfo->Id == 53595 )
		{
			...
			if( it && it->GetProto() )
			{
				WMIN = it->GetProto()->Damage.Min;
				WMAX = it->GetProto()->Damage.Max;
		...
		case SPELL_HASH_STEADY_SHOT:
			{
				if( p_caster != NULL )
				{
					...
					if( it )
						dmg += float2int32((it->GetProto()->Damage.Min + it->GetProto()->Damage.Max) * 0.2f); //+unmodified weapon damage
				}
			}break;
...
void Spell::SpellEffectDummy(uint32 i) // Dummy(Scripted events)
{
	...
	case 47475:
			...
			damage = ((itm->GetProto()->Damage.Min + itm->GetProto()->Damage.Max) * 0.2f) + m_spellInfo->EffectBasePoints[i];
			}
		}break;
