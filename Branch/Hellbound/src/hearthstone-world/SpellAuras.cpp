			...
			case SPELL_HASH_REND:
				{
					if( m_caster->IsPlayer() )
					{
						...
						if( weapon != NULL && weapon->GetProto() )
							dmg += float2int32( ( weapon->GetProto()->Delay * m_caster->GetAP() / 14 + (weapon->GetProto()->Damage.Min + weapon->GetProto()->Damage.Max)/2 ) * (GetDuration() / 1000) * (743 / 300000 ) );
					}
				}break;
					...
					if( m_caster->IsPlayer() && pSpellId )
					{
						...
						if(multiplyer)
						{
							if(TO_PLAYER(m_caster) != NULL && TO_PLAYER(m_caster)->GetItemInterface())
							{
								...
								{
									dmg = 0;
									if(it->GetProto()->Damage.Type==SCHOOL_NORMAL)
										dmg += int32((it->GetProto()->Damage.Min + it->GetProto()->Damage.Max) / 2);
