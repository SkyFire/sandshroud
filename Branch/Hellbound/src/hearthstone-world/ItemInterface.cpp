...
int16 ItemInterface::GetInventorySlotById(uint32 ID)
{
	...
	{
		CurrencyTypesEntry* store = dbcCurrencyTypesStore.LookupEntry(GetCurrencyId(proto->Name1));
		if(store)
		{
...
AddItemResult ItemInterface::AddItemToFreeSlot(Item* item)
{
	...
	if( item->GetProto()->BagFamily )
	{
		...
		{
			CurrencyTypesEntry* currency = dbcCurrencyTypesStore.LookupEntry(GetCurrencyId(item->GetProto()->Name1));
			if(currency)
			{
