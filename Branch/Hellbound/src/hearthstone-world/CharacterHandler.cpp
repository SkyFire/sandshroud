...
void WorldSession::FullLogin(Player* plr)
{
	...
	OutPacket( SMSG_LOGIN_VERIFY_WORLD, sizeof(vwpck), &vwpck );
	...
	*/
	...
	// Anti max level hack.
	if(plr->getLevel() > sWorld.GetMaxLevel(plr))
		plr->SetUInt32Value(UNIT_FIELD_LEVEL, sWorld.GetMaxLevel(plr));
...
void WorldSession::HandleEquipmentSetUse(WorldPacket &recv_data)
{
	...
	for(uint32 i = 0; i < EQUIPMENT_SLOT_END; ++i)
	{
		...
		// Crow: Oh god.... Let the torment begin...
