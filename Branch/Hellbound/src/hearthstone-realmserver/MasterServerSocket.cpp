...
MasterServerSocket::MasterServerSocket(SOCKET fd, const sockaddr_in * peer) : TcpSocket(fd, 100000, 100000, false, peer)
{
...
void MasterServerSocket::OnRead()
{
	...
	for(;;)
	{
		size = (uint32)GetReadBuffer()->GetSize();
		if(!_cmd)
		{
			if(size < 6)
				break;
		...
		if(_cmd == IMSMSG_WOW_PACKET)
		{
			...
			pck->resize(sz);
			GetReadBuffer()->Read((uint8*)pck->contents(), sz);
...
void MasterServerSocket::SendPacket(uint16 opcode, uint32 size, const void* data)
{
	...
	// Pass the rest of the packet to our send buffer (if there is any)
	if(size > 0 && rv)
		rv = Write((const uint8*)data, uint32(size));
	else if(rv)
		rv = ForceSend();
...
void MasterServerSocket::SendWoWPacket(Session * from, WorldPacket * pck)
{
	...
	// Pass the header to our send buffer
	WriteButHold((const uint8*)&RSOpcode, 2);
	WriteButHold((const uint8*)&RSOpcodeSize, 4);
	WriteButHold((const uint8*)&SessionId, 4);
	WriteButHold((const uint8*)&WoWOpcode, 2);
	rv = WriteButHold((const uint8*)&WoWOpcodeSize, 4);	

	// Pass the rest of the packet to our send buffer (if there is any)
	if(WoWOpcodeSize > 0 && rv)
		rv = Write(pck->contents(), uint32(WoWOpcodeSize));
	else if(rv)
		rv = ForceSend();
