...
void DarkPortalEventTriggerAI::AIUpdate()
{
	...
	// Conditions written to set special unit  *s to null
	if ( mJusti != NULL )
	{
	...
	if ( mMelgr != NULL )
	{
		...
		// If Melgromm's  *is valid and unit is not alive or is not in world, but was resurrected we NULL it's  *
...
void DarkPortalEventTriggerAI::PitCommanderInfernalSummon()
{
	...
	// We check if Pit Commander  *is valid and if he's alive and we have on battlefield less than 3 infernals
...
void DarkPortalEventTriggerAI::PitCommanderAttackCmd()
{
	...
	// If  *is valid, Pit Commander is not in combat we randomize emoting
