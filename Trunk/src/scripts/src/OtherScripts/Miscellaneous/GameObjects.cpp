...
public:
	...
	void AIUpdate()
	{
		...
		if( _gameobject->CalcDistance( _gameobject, plr ) <= 1.5f && plr->GetMapId() == 609)
		{
			...
			EventMgr::getSingleton().AddEvent(plr, &Player::EventTeleport, (uint32)609, (float)2390.18, (float)-5640.28, (float)420.84, (float)0.57, 1, 109, 1000, 1, 1);
		}
...
public:
	...
	void AIUpdate()
	{
		...
		if( _gameobject->CalcDistance( _gameobject, plr ) <= 2.0f && plr->GetMapId() == 609)
			EventMgr::getSingleton().AddEvent(plr, &Player::EventTeleport, (uint32)609, (float)2397.57, (float)-5636.03, (float)377.05, (float)0.57, 1, 109, 1000, 1, 1);
