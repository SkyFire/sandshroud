			...
			case 1:
				{
					if( GetHealthPercent() <= 65 ){
						SetAllowMelee(false);
						Emote("This meaningless exertion bores me. I'll incinerate you all from above!", Text_Yell, 0, EMOTE_ONESHOT_LIFTOFF);
						SetFlyMode(true);
						SetWaypointToMove(0);
						SetPhase(2);
					}
			...
			case 2:
				{
					if( GetHealthPercent() <= 40 ){
						SetAllowMelee(true);
						Emote("It seems you'll need another lesson, mortals!", Text_Yell, 0, EMOTE_ONESHOT_LAND);
						SetFlyMode(false);
						SetPhase(3);
						StopWaypointMovement();
...
class GuardAI : public MoonScriptCreatureAI
{
    ...
    {	
		...
		AddSpell(GUARD_IGNITE_WEAPON, Target_Self, 15, 0, 10, 0.0f, 5.0f);
	}
};
