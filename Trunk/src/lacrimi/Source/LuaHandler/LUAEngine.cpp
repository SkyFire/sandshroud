...
#ifdef WIN32
static HANDLE Lua_stdout_handle = GetStdHandle(STD_OUTPUT_HANDLE);
#endif
...
void LuaEngine::LoadScripts()
{
	...
	for(set<string>::iterator itr = rtn.luaFiles.begin(); itr != rtn.luaFiles.end(); ++itr)
	{
		...
		{
#ifdef WIN32
				SetConsoleTextAttribute(Lua_stdout_handle, (WORD)TRED);
				report(L);
				SetConsoleTextAttribute(Lua_stdout_handle, (WORD)TWHITE);
#else
				printf("\033[22;31m");
				report(L);
				printf("\033[01;37m");
