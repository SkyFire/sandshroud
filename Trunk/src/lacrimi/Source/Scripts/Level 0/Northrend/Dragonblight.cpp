...
void Lacrimi::SetupDragonblight()
{
	sMgr->register_creature_script(26421, &WoodlandWalker::Create);

	sMgr->register_gossip_script(26421, ((GossipScript*)(new WoodlandWalkerGossip())));

	QuestScript *WrathGateCinema = ((QuestScript*)(new WrathGateQuestCinema()));
	sMgr->register_quest_script(12499, WrathGateCinema);
	sMgr->register_quest_script(12500, WrathGateCinema);

	GossipScript * WyrmrestTempleFlightGossip = ((GossipScript*)(new WyrmrestTemple_FlightGossip()));
	sMgr->register_gossip_script(CN_TOP, WyrmrestTempleFlightGossip);		// Torastrasza <Majordomo to the Ruling Council>
	sMgr->register_gossip_script(CN_MIDDLE, WyrmrestTempleFlightGossip);	// Lord Afrasastrasz <Commander of Wyrmrest Temple Defenses>
	sMgr->register_gossip_script(CN_BOTTOM, WyrmrestTempleFlightGossip);	// Tariolstrasz <Steward of Wyrmrest Temple>
}
