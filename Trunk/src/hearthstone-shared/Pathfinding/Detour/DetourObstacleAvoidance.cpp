...
dtObstacleAvoidanceDebugData* mallocObstacleAvoidanceDebugData()
{
	void* mem = malloc(sizeof(dtObstacleAvoidanceDebugData));
	if (!mem) return 0;
	return new(mem) dtObstacleAvoidanceDebugData;
}

void freeObstacleAvoidanceDebugData(dtObstacleAvoidanceDebugData* ptr)
{
	if (!ptr) return;
...
bool dtObstacleAvoidanceDebugData::init(const int maxSamples)
{
	...
	m_vel = (float*)malloc(sizeof(float)*3*m_maxSamples);
	if (!m_vel)
		return false;
	m_pen = (float*)malloc(sizeof(float)*m_maxSamples);
	if (!m_pen)
		return false;
	m_ssize = (float*)malloc(sizeof(float)*m_maxSamples);
	if (!m_ssize)
		return false;
	m_vpen = (float*)malloc(sizeof(float)*m_maxSamples);
	if (!m_vpen)
		return false;
	m_vcpen = (float*)malloc(sizeof(float)*m_maxSamples);
	if (!m_vcpen)
		return false;
	m_spen = (float*)malloc(sizeof(float)*m_maxSamples);
	if (!m_spen)
		return false;
	m_tpen = (float*)malloc(sizeof(float)*m_maxSamples);
	if (!m_tpen)
		return false;
...
dtObstacleAvoidanceQuery* mallocObstacleAvoidanceQuery()
{
	void* mem = malloc(sizeof(dtObstacleAvoidanceQuery));
	if (!mem) return 0;
	return new(mem) dtObstacleAvoidanceQuery;
}

void freeObstacleAvoidanceQuery(dtObstacleAvoidanceQuery* ptr)
{
	if (!ptr) return;
...
bool dtObstacleAvoidanceQuery::init(const int maxCircles, const int maxSegments)
{
	...
	m_ncircles = 0;
	m_circles = (dtObstacleCircle*)malloc(sizeof(dtObstacleCircle)*m_maxCircles);
	if (!m_circles)
		return false;
	...
	m_nsegments = 0;
	m_segments = (dtObstacleSegment*)malloc(sizeof(dtObstacleSegment)*m_maxSegments);
	if (!m_segments)
		return false;
