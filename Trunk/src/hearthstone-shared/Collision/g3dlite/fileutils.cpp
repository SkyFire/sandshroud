...
void parseFilename(
	...
	// Pull the extension off
	{
		...
		if (i != std::string::npos) {
			...
			size_t j = iMax((int)f.rfind('/'), (int)f.rfind('\\'));
	...
	{
		// Find the last slash
		size_t i = iMax((int)f.rfind('/'), (int)f.rfind('\\'));
