...
#include "hearthstone_packet.h"
#include "hearthstone_opcodes.h"
...
hearthstone_socket * ascentsocket_allocate(network_socket *s)
{
	hearthstone_socket * ret = (hearthstone_socket*)vc_malloc(sizeof(hearthstone_socket));
...
void ascentsocket_free(hearthstone_socket* s)
{
	if( s->channelcount > 0 )
...
void ascentsocket_send(hearthstone_socket *s, hearthstone_packet *p)
{
...
int voicechat_hearthstone_listen_socket_read_handler(network_socket *s, int act)
{
	...
	if( network_add_socket(ns) < 0 )
	{
		...
		ascentsocket_free((hearthstone_socket*)ns->miscdata);
		network_close(ns);
	}
...
// client handlers
void vc_handler_createchannel(hearthstone_socket *s, hearthstone_packet *p)
{
	uint8 type;
	uint32 request_id;
	hearthstone_packet reply;
	voice_channel * chn;
	uint8 error;
	uint16 reply_id;
...
void vc_handler_deletechannel(hearthstone_socket *s, hearthstone_packet *p)
{
	uint16 channel_id;
	voice_channel *ch;
...
void vc_handler_addmember(hearthstone_socket *s, hearthstone_packet *p)
{
	uint16 channel_id;
	uint8 member_id;
	voice_channel *ch;
...
void vc_handler_deletemember(hearthstone_socket *s, hearthstone_packet *p)
{
	uint16 channel_id;
	uint8 member_id;
	voice_channel *ch;
...
void vc_handler_ping(hearthstone_socket *s, hearthstone_packet *p)
{
	hearthstone_packet bp;
	uint32 pi;
...
// handler table
typedef void(*vc_handler)(hearthstone_socket*,hearthstone_packet*);
...
// client reader
int voicechat_hearthstone_socket_read_handler(network_socket *s, int act)
{
	hearthstone_socket * mysock = (hearthstone_socket*)s->miscdata;
	uint16 opcode;
	uint16 len;
	int readcount;
	hearthstone_packet * pck;
