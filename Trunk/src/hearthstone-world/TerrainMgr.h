...
protected:
	...
	/* Gets the offset for the specified cell from the cached offset index.
	  ...
	  */
	HEARTHSTONE_INLINE uint32 GetCellInformationOffset(uint32 x, uint32 y)
	{
	...
	/* Gets a cell information pointer so that another function can access its data.
	  ...
	  */
	HEARTHSTONE_INLINE CellTerrainInformation* GetCellInformation(uint32 x, uint32 y)
	{
	...
	*/
	HEARTHSTONE_INLINE float ConvertInternalXCoordinate(float x, uint32 cellx)
	{
		float X = (_maxX - x);
		X -= (cellx * _cellSize);
		return X;
	...
	*/
	HEARTHSTONE_INLINE float ConvertInternalYCoordinate(float y, uint32 celly)
	{
		float Y = (_maxY - y);
		Y -= (celly * _cellSize);
		return Y;
	...
	/* Checks whether a cell information is loaded or not.
	  */
	HEARTHSTONE_INLINE bool CellInformationLoaded(uint32 x, uint32 y)
	{
	...
	/* Checks that the co-ordinates are within range.
	  */
	HEARTHSTONE_INLINE bool AreCoordinatesValid(float x, float y)
	{
		if(x > _maxX || x < _minX)
			return false;
		if(y > _maxY || y < _minY)
