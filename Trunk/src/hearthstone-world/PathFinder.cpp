 ...
 * Copyright (C) 2008-2010 Rebirth <http://UntoldRealms.net/>
 *
...
////////////////// PathInfo //////////////////
PathInfo::PathInfo(Object* from, const float x, const float y, const float z) :
	...
	m_pointPathPointer(0), m_sourceObject(from), m_type(PATHFIND_BLANK)
{
	...
	if(m_navMesh = NavMeshInterface.GetNavmesh(m_sourceObject->GetMapId()))
	{
		DEBUG_LOG("Pathfinder", "Navmesh found!");
		m_navMeshQuery = dtAllocNavMeshQuery();
		m_navMeshQuery->init(m_navMesh, MESH_MAX_NODES*64*64);
		...
		shortcut();
	DEBUG_LOG("Pathfinder", "Navmesh not found!");
}
...
{
	DEBUG_LOG("Pathfinder", "Building a path!");

	if(startPoly == INVALID_POLYREF || endPoly == INVALID_POLYREF)
	{
		DEBUG_LOG("Pathfinder", "Unable to retrieve polygons!");
	...
	{
		DEBUG_LOG("Pathfinder", "Both polygons are the same!");
		shortcut();
	...
	{
		DEBUG_LOG("Pathfinder", "Found both polygons!");
	...
	{
		DEBUG_LOG("Pathfinder", "Found polygon(1) but not polygon(2)");
		...
		if(m_navMesh->getTileAndPolyByRef(suffixStartPoly, &tile, &poly))
		{
			...
			else
			{
				...
				DEBUG_LOG("Pathfinder", "%u's Path Build failed: 0 length path", m_sourceObject->GetGUID());
			}
		}
		else
		{
			// suffixStartPoly is invalid somehow, or the navmesh is broken => error state
			DEBUG_LOG("Pathfinder", "%u's Path Build failed: invalid polyRef in path", m_sourceObject->GetGUID());
	...
	{
		DEBUG_LOG("Pathfinder", "Failed to find both polygons!", m_sourceObject->GetGUID());
...
{
	DEBUG_LOG("Pathfinder", "Unable to build path, taking a shortcut!");
	clear();
...
bool PathInfo::canFly()
{
	...
	Creature* creature = TO_CREATURE(m_sourceObject);
...
{
	switch(m_sourceObject->GetMapMgr()->GetWaterType(x, y))
	{
