...
void Vehicle::ChangeSeats(Unit* unit, uint8 seatid)
{
	...
	uint64 guid = Vehicleguid.GetOldGuid();
	Vehicle * v = GetPlayer()->GetMapMgr()->GetVehicle(GET_LOWGUID_PART(guid));
...
bool Unit::IsVehicleMountOwner()
{ 
	if(m_vehiclemount == NULL)
		return false;
	return ( m_vehiclemount->GetOwner()->GetGUID() == GetGUID() );
}

Mount::Mount(Unit * owner, uint32 vehicle_entry, uint32 spellId) : m_capacity(0), m_vehicle_entry(vehicle_entry), m_spellId(spellId)
{
	memset(&m_passengers, 0, sizeof(Unit*) * 4);
	m_mountowner = owner;
	m_passengers[0] = owner;
}
...
int8 Mount::GetFreeSeat()
{
	int8 seat = -1;
	for (int8 i = 1; i < m_capacity; ++i) // skip first seat since that belongs to the owner
	{
		if(m_passengers[i] == NULL)
			seat = i;
	}
...
void Mount::AddPassenger(Unit * passenger, int8 seat)
{
	ASSERT(seat < 4);
	if(passenger == NULL)
		return;
	...
	passenger->SetVehicleMount(this, seat);

	if(passenger->IsPlayer())
	{
		Player * player = static_cast<Player*>(passenger);

		WorldPacket data(CMSG_BOARD_PLAYER_VEHICLE, 0);
		player->GetSession()->SendPacket(&data);
	}

	SendRidePacket(passenger);
	passenger->Root();
}

void Mount::RemovePassenger(Unit * passenger)
{
	if(passenger == NULL)
		return;
	...
	for (uint8 i = 0; i < m_capacity; ++i)
	{
		...
		if(m_passengers[i]->GetGUID() == passenger->GetGUID())
		{
			SendHeartbeatPacket(passenger);
			m_passengers[i] = NULL;
			passenger->ResetVehicleMount();
			break;
		}
	}
}

void Mount::RemovePassenger(uint8 seat)
{
	ASSERT(seat < 4);
	if(seat >= 4)
		return;
	...
	Unit * passenger = m_passengers[seat];
	SendHeartbeatPacket(passenger);
...
void WorldSession::HandleEnterVehicleOpcode(WorldPacket & recv_data)
{
	...
	Player * pl = GetPlayer();
	Player * owner = pl->GetMapMgr()->GetPlayer( GET_LOWGUID_PART(vehicleGuid) );
	if(owner == NULL)
	{
		return;
	}

	if(pl->GetGroup() == NULL || pl->GetGroup() != owner->GetGroup())
	{
		return;
	}

	owner->GetVehicleMount()->AddPassenger(pl);
}

void WorldSession::ConvertToVehicle(WorldPacket & recv_data)
{
	Player* plr;
	Vehicle* v;
	WorldPacket pkt(SMSG_CONVERT_VEHICLE, 8+4);
	pkt << plr->GetNewGUID();
	pkt << v->GetVehicleEntry();
	plr->SendMessageToSet(&pkt, true);

	plr->SetUInt32Value(UNIT_DYNAMIC_FLAGS, 0);
	plr->SetFlag(UNIT_NPC_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
	plr->SetFlag(UNIT_FIELD_FLAGS , UNIT_FLAG_MOUNT);
}

void Mount::SendRidePacket(Unit * passenger)
{	
	Vehicle* pPassenger;
	uint8 slot = pPassenger->m_inVehicleSeatId;
	WorldPacket data(SMSG_MONSTER_MOVE_TRANSPORT, 100);
	...
	data << slot;									// Vehicle Seat ID
	data << uint8(0);										// Unknown
	data << m_vehicleSeats[slot]->m_attachmentOffsetX;		// OffsetTransporterX
	data << m_vehicleSeats[slot]->m_attachmentOffsetY;		// OffsetTransporterY
	data << m_vehicleSeats[slot]->m_attachmentOffsetZ;		// OffsetTransporterZ
	data << getMSTime();									// Timestamp
	data << uint8(0x04);									// Flags
...
void Mount::SendHeartbeatPacket(Unit * passenger)
{
	WorldPacket data;
...
void Mount::ConvertToVehicle(uint32 vehicle_entry)
{
	...
	if(vehicle_entry)
	{
		...
		m_mountowner->SetFlag(UNIT_NPC_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
		m_mountowner->SetFlag(UNIT_FIELD_FLAGS , UNIT_FLAG_MOUNT);
	}
	else
	{
		m_mountowner->SetUInt32Value(UNIT_DYNAMIC_FLAGS, 0);
		m_mountowner->RemoveFlag(UNIT_NPC_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
		m_mountowner->RemoveFlag(UNIT_FIELD_FLAGS , UNIT_FLAG_MOUNT);
	}
}