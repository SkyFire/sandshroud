...
void AIInterface::UpdateMove()
{
	...
	if(sWorld.PathFinding && NavMeshInterface.IsNavmeshLoadedAtPosition(m_Unit->GetMapId(), m_Unit->GetPositionX(), m_Unit->GetPositionY()))
	{
		...
		{	// Use next instead of best, because we want as many steps as we can get.
			PathLocation = new LocationVector(NavMeshInterface.getNextPositionOnPathToLocation(m_Unit->GetMapId(), m_nextPosX, m_nextPosY, m_nextPosZ, m_destinationX, m_destinationY, m_destinationZ));
		...
		if(PathMap != NULL)
		{
			...
			{	// This works, but maybe a 200ms update time would be better for pathfinding.
				if(m_timeMoved == m_timeToMove)
				{
					...
					m_nextPosX = m_nextPosY = m_nextPosZ = m_nextPosO = 0;
					m_timeMoved = 0;
					m_timeToMove = 0;
				}
				else
				{
					...
					if(!positionchanged)
					{
						float q = ((float)(m_timeMoved-sexyteim[0])) / ((float)(sexyteim[1]-sexyteim[0]));
						float x = x1 + ((x1 - x2) * q);
						float y = y1 + ((y1 - y2) * q);
						float z = z1 + ((z1 - z2) * q);
						m_Unit->SetPosition(x, y, z, m_Unit->GetOrientation());
					}
					m_moveTimer = (UNIT_MOVEMENT_INTERPOLATE_INTERVAL/2 < m_timeToMove) ? UNIT_MOVEMENT_INTERPOLATE_INTERVAL/2 : m_timeToMove;
				}
